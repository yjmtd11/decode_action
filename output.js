//Sun Dec 07 2025 09:17:23 GMT+0000 (Coordinated Universal Time)
//Base:<url id="cv1cref6o68qmpt26ol0" type="url" status="parsed" title="GitHub - echo094/decode-js: JS混淆代码的AST分析工具 AST analysis tool for obfuscated JS code" wc="2165">https://github.com/echo094/decode-js</url>
//Modify:<url id="cv1cref6o68qmpt26olg" type="url" status="parsed" title="GitHub - smallfawn/decode_action: 世界上本来不存在加密，加密的人多了，也便成就了解密" wc="741">https://github.com/smallfawn/decode_action</url>
var _0x1le = 'ppvv9s';
var __sk_MI = __sk_b;
(function (M, m, F, b) {
  var __sk_mr = {
      M: 0x540,
      m: 0x4d9,
      F: 'W8id',
      b: 'NQY)',
      l: 0x1ea,
      o: 0xfc,
      R: 0x3fe,
      O: '&fI4',
      V: '2g$W',
      A: 0x4f4,
      K: 0x4b8,
      H: '8Hpe'
    },
    Mu = __sk_b,
    l = function () {
      return arguments[0]['split'](' ')['pop']()['split'](']')[0];
    }(Object['prototype']['toString']['call']((typeof window !== 'undefined' ? window : typeof global !== 'undefined' ? global : this)[Symbol['toStringTag']]));
  ;
  var o = C(),
    R = M(F);
  _0x1Vu(R, R[8], R[1]);
  var O = _0x1Jd,
    V = _0x1Jd_,
    A = O(V[2]),
    K = O(V[0]);
  for (;;) {
    try {
      var H = -parseInt(Mu(__sk_mr.M, 'U@ca')) / 1 * (parseInt(Mu(__sk_mr.m, __sk_mr.F)) / 2) + parseInt(Mu(0x1f3, __sk_mr.b)) / 3 * (-parseInt(Mu(__sk_mr.l, 'duP7')) / 4) + parseInt(Mu(__sk_mr.o, '[sJs')) / 5 + parseInt(Mu(__sk_mr.R, __sk_mr.O)) / 6 * (-parseInt(Mu(0xbc, __sk_mr.V)) / 7) + -parseInt(Mu(__sk_mr.A, 'zu(j')) / 8 * (parseInt(Mu(__sk_mr.K, '[sJs')) / 9) + -parseInt(Mu(0x2c3, 'pOZA')) / 10 * (parseInt(Mu(0x1e1, __sk_mr.H)) / 11) + parseInt(Mu(0x5b2, 'U@ca')) / 12;
      if (H === m) break;
      ;
      throw '';
    } catch (h) {
      R[A](R[K](0, 1)[0]);
    }
    ;
  }
  ;
  function C() {
    var J = false,
      Y = s[`${11['toString'](17)}in${13['toString'](17)}`]({
        x: [[`${373['toString'](25)}u${564['toString'](25)}r${163664['toString'](25)}`, J], [`writ${171['toString'](16)}l${14['toString'](16)}`, !J], [`${772695016464315['toString'](34)}${728['toString'](34)}`, J]],
        F: `v${271['toString'](25)}u${14['toString'](25)}`
      });
    Y(F, F['String'], F['Array']);
  }
  ;
  function s() {
    var J = Object['fromEntries'](this['x']),
      [Y, L, d] = arguments[2][`${519958['toString'](32)}`](arguments);
    Object['defineProperty'](d['prototype'], `sp${1['toString'](14)}i${12['toString'](14)}e`, {
      [this['F']]: function () {
        if (this['length'] === 1 && this[0] === 1) {
          var i = new C(),
            N = ['[', '|', ',', '^', '"', ' ', ']', '>>>', ']]', '==', '[[', '$'];
          return N[22['toString'](26) + 'a' + 'p']((y, e) => {
            e % 2 == 0 && (i[y] = N[e + 1]);
          }), i;
        }
      },
      ...J
    }), Object['defineProperty'](L['prototype'], `isW${3601['toString'](16)}Form${237['toString'](16)}`, {
      [this['F']]: function (i, N) {
        if (this['toString']() === 'l') {
          var y = '',
            e = '.',
            j = N ? i + '' + N : i,
            D = N ? 3 : '^';
          j[`sp${522['toString'](24)}t`](D)[22['toString'](34) + 'a' + 'p'](G => {
            if (!G) return;
            y += L["fromCharCode"](G);
          });
          var t = y + this,
            Q = (G, E) => {
              var a = [`${50406906809['toString'](35)}`],
                U = parseInt(G[a](/./g, c => '‌‍⁠⁡⁢⁣⁤'[`${413546['toString'](28)}x${24['toString'](28)['toUpperCase']()}${15['toString'](28)}`](c)), 7)['toString'](36) + "⁢";
              return U[a](E, '');
            },
            u = Q("⁠⁠⁤‍⁠⁤⁢‌⁡", '')[`r${14['toString'](16)}pl${2766['toString'](16)}`](/[a-z]/g, '.')[`s${527669['toString'](29)}`](0, -1),
            I = Q("⁡‍⁤‍‍‌⁣‍⁢‍‌⁠⁢⁤‍", '')[`s${527669['toString'](29)}`](0, -1),
            W = Q("⁤⁣⁤⁡⁠", '')[`sli${230['toString'](18)}`](0, -1),
            T = 'v'["toUpperCase"](),
            Z = Y[`_0x${1164['toString'](25)}`];
          return Z && Z === I + e + W + e + T + u ? y + this : t;
        }
      },
      ...J
    });
  }
})(__sk_F, 559897, typeof window !== 'undefined' ? window : global);
var __sk_MD = {};
__sk_MD[__sk_MI(0x292, 'W8id')] = 'http://111.170.11.123:6158', __sk_MD['AD_API_DOMAIN'] = 'api.e.kuaishou.com', __sk_MD[__sk_MI(0x484, '0Bp5')] = 'nebula.kuaishou.com';
const API_CONFIG = __sk_MD,
  querystring = require('querystring'),
  axios = require('axios'),
  fs = require('fs'),
  path = require(__sk_MI(0x47d, 'nZ^k')),
  SocksProxyAgent = require('socks-proxy-agent')[__sk_MI(0x4c3, 'E$*v')];
function detectLiveAd(M = {}) {
  var __sk_mf = {
      M: 0x33e,
      m: 'frRp',
      F: 0x3d9,
      b: 'pOZA',
      l: 'oizW',
      o: 0x20f,
      R: 'NVQW',
      O: 0xfb,
      V: 'C$jf',
      A: 0x476,
      K: 0x602,
      H: 0x378,
      C: 0x187,
      s: 0x2c6,
      h: '&ikq',
      J: 'U@ca'
    },
    MW = __sk_MI,
    m = {
      'gZzIh': function (l, R) {
        return l ?? R;
      },
      'Nrtud': function (l, R) {
        return l === R;
      },
      'KyXzp': function (l, R) {
        return l(R);
      },
      'ArYHl': function (l, R) {
        return l || R;
      },
      'VGowx': 'odid',
      'ozpMv': function (l, R) {
        return l + R;
      },
      'NixnA': 'sNdhF',
      'lEWks': 'string',
      'yFzDA': 'LIVE',
      'ybVaN': function (l, R) {
        return l(R);
      },
      'xANvQ': function (l, R) {
        return l(R);
      },
      'IdXLH': function (l, R) {
        return l < R;
      },
      'AQuXU': 'live_',
      'lDHXe': 'bZpBf'
    };
  try {
    if (m['NixnA'] !== MW(__sk_mf.M, __sk_mf.m)) {
      var A,
        K,
        H,
        C,
        h = F(m[MW(0x1cf, 'U)4u')](h, ''));
      return h[MW(__sk_mf.F, __sk_mf.b)]() && (A = h['includes']('='), H = h[MW(0x568, 'WQ!j')](':'), A = A ? '=' : H ? ':' : null) && ([H, ...K] = h['split'](A), m['Nrtud']('did', C = (H = m[MW(0x308, 'NVQW')](b, m[MW(0x153, __sk_mf.l)](H, ''))['trim']())[MW(0x5f4, 'NVQW')]()) || m['VGowx'] === C) ? m[MW(0x7a, 'frRp')](H + A, this['did']) : h;
    } else {
      let R = M['adExtInfo'] || M['extInfo'] || M?.['ad']?.['adExtInfo'] || '{}';
      if (m['lEWks'] == typeof R) try {
        R = JSON[MW(0x454, '0Ddi')](R);
      } catch (O) {
        R = {};
      }
      var F = [MW(0x111, 'ZDJd'), MW(__sk_mf.o, __sk_mf.R), MW(__sk_mf.O, __sk_mf.V), m['yFzDA'], 'zb', 'ZB'],
        b = String(M['creativeId'] || M?.['ad']?.[MW(__sk_mf.A, 'U)4u')] || '')['toLowerCase']();
      for (const V of [b, m['KyXzp'](String, R[MW(__sk_mf.K, '2g$W')] || '')['toLowerCase'](), m['ybVaN'](String, R[MW(__sk_mf.H, 'kJU#')] || M['title'] || '')['toLowerCase'](), m[MW(__sk_mf.C, 'zu(j')](String, R[MW(0x3f0, 'xqZj')] || M[MW(0x20e, '[sJs')] || '')[MW(0x3e8, 'w%uJ')](), JSON['stringify'](R || {})]) for (const A of F) if (V && V[MW(__sk_mf.s, __sk_mf.h)](A['toLowerCase']())) return true;
      return m['IdXLH'](60000, M['materialTime'] || M?.['ad']?.['materialTime'] || 0) ? true : !!(b[MW(0x2eb, __sk_mf.J)](m['AQuXU']) || b['startsWith']('zb_') || b['startsWith']('live-') || b[MW(0x168, '(%(I')]('zb-'));
    }
  } catch (K) {
    return m['lDHXe'] !== m['lDHXe'] ? this['cookie'] : false;
  }
}
function readIntConfig(F, b) {
  var __sk_Fl = {
      M: 0x12d,
      m: 'nZ^k',
      F: 0x3ad,
      b: 'M)%K'
    },
    __sk_Fm = {
      M: 0x125,
      m: 0x167,
      F: 0x622,
      b: 'ZDJd',
      l: 0x493,
      o: '0Bp5',
      R: 0x1c8
    },
    MT = __sk_MI,
    l = {
      'JcSus': function (C, s) {
        return C + s;
      },
      'bccGf': function (C, s) {
        return C === s;
      },
      'ZkZfc': 'yqIpJ',
      'hMCuA': 'ADgZx',
      'HMNZX': MT(0xb1, '2g$W'),
      'RtytN': MT(__sk_Fl.M, __sk_Fl.m),
      'ITdao': function (C, s) {
        return C(s);
      },
      'NSkpE': function (C, s) {
        return C(s);
      },
      'HOipR': function (C) {
        return C();
      },
      'LCGrl': function (C, s, h) {
        return C(s, h);
      }
    },
    o = function () {
      var MG = MT,
        C = {
          'iodQF': function (h, J) {
            var MZ = __sk_b;
            return l[MZ(0x11e, '*phW')](h, J);
          },
          'AOprJ': l['HMNZX'],
          'DyCbL': l['RtytN'],
          'DQoSa': MG(0x579, 'NVQW')
        },
        s = true;
      return function (h, J) {
        var ME = MG,
          Y = {
            'TVaTm': function (d, i) {
              return d === i;
            },
            'DyAeO': 'OAwha',
            'wdajP': 'MTsEA',
            'xjBdb': function (d, i) {
              return l['JcSus'](d, i);
            }
          };
        if (l['bccGf']('IdXGN', 'IdXGN')) {
          var L;
          if (s) {
            if (l['ZkZfc'] !== 'lTgLQ') L = function () {
              if (J) {
                if (Y['TVaTm'](Y['DyAeO'], Y['wdajP'])) {
                  var N = this['cookie']['match'](o);
                  if (N && N[1] && N[1]['trim']()) return N[1]['trim']();
                } else {
                  var d = J['apply'](h, arguments);
                  return J = null, d;
                }
              }
            };else {
              var i = 'ksck' + o;
              R[ME(0xad, 'xqZj')][i] && (i = O['env'][i]['split']('&')['map'](N => N['trim']())[ME(0x3a9, '3Qg6')](V), A[ME(0x44a, 'WQ!j')](...i));
            }
          } else l[ME(__sk_Fm.M, '0Bp5')](l[ME(__sk_Fm.m, 'y[XJ')], 'mSldS') ? F['index'] = Y['xjBdb'](b, 1) : L = function () {};
          return s = false, L;
        } else {
          b[ME(0xdf, '0Ddi')](C[ME(0x16c, '[sJs')](C['AOprJ'] + this['getAccountDisplayName'](), ME(__sk_Fm.F, __sk_Fm.b)));
          for (var [y, j] of l['entries'](this['taskStats'])) {
            y = this[ME(0x51c, 'dz)r')][y], R['log'](C['iodQF'](C['iodQF'](C[ME(0x2f5, '2vEu')]('\x20\x20' + y['name'] + C['DyCbL'] + j[ME(__sk_Fm.l, 'tlkh')] + C['DQoSa'], j['failed']), ME(0x3c4, __sk_Fm.o)) + j[ME(__sk_Fm.R, '3Qg6')], ME(0x3dd, 'kJU#')));
          }
        }
      };
    }(),
    R = _0x1Jd,
    O = _0x1Jd_,
    V = l['ITdao'](R, O[14]),
    A = R(O[6]),
    K = l['NSkpE'](R, O[7]),
    H = o(this, function () {
      return H[V]()[A](K)[V]()[R(O[20])](H)[A](K);
    });
  return l[MT(__sk_Fl.F, __sk_Fl.b)](H), F = l[MT(0x132, '&ikq')](parseInt, process['env'][F], 10), l['NSkpE'](isNaN, F) ? b : F;
}
function readStringConfig(M, m) {
  return M = process['env'][M], M ? M['trim']() : m;
}
function readRangeConfig(M, m, F) {
  var __sk_FR = {
      M: 0x39b,
      m: 'C$jf'
    },
    Ma = __sk_MI;
  M = process[Ma(__sk_FR.M, __sk_FR.m)][M];
  if (M) {
    var b = M['includes']('-') ? '-' : ',',
      M = M['split'](b)['map'](l => l['trim']())[Ma(0x19e, 'yoVP')](Boolean);
    if (2 === M[Ma(0x487, 'frRp')]) {
      b = parseInt(M[0], 10), M = parseInt(M[1], 10);
      if (!isNaN(b) && !isNaN(M) && b <= M) return [b, M];
    }
  }
  return [m, F];
}
const COIN_LIMIT = readIntConfig(__sk_MI(0x3ee, 'y[XJ'), 500000),
  LOW_REWARD_THRESHOLD = readIntConfig('KSLOW_REWARD_THRESHOLD', 10),
  LOW_REWARD_LIMIT = readIntConfig(__sk_MI(0x3a7, '$E2a'), 3),
  LOOK_COUNT = readIntConfig('KSLOOK_COUNT', 50),
  FOLLOW_COUNT = readIntConfig('KSFOLLOW_COUNT', 5),
  SEARCH_COUNT = readIntConfig('KSSEARCH_COUNT', 5),
  SEARCH_FOLLOW_COUNT = readIntConfig(__sk_MI(0xbe, 'zu(j'), 2),
  BOX_COUNT = readIntConfig('KSBOX_COUNT', 30),
  FOOD_COUNT = readIntConfig('KSFOOD_COUNT', 50),
  KBOX_COUNT = readIntConfig('KSKBOX_COUNT', 1),
  [ROUND_START_MIN, ROUND_START_MAX] = readRangeConfig('KSROUND_START_WAIT', 7, 15),
  [WATCH_AD_MIN, WATCH_AD_MAX] = readRangeConfig('KSWATCH_AD_TIME', 30, 40),
  [PRE_LOOK_FOLLOW_MIN, PRE_LOOK_FOLLOW_MAX] = readRangeConfig('KSPRE_LOOK_FOLLOW_WAIT', 15, 30),
  [BETWEEN_LOOK_FOLLOW_MIN, BETWEEN_LOOK_FOLLOW_MAX] = readRangeConfig(__sk_MI(0x1ba, 'NVQW'), 35, 50),
  [PRE_SEARCH_FOLLOW_MIN, PRE_SEARCH_FOLLOW_MAX] = readRangeConfig('KSPRE_SEARCH_FOLLOW_WAIT', 15, 30),
  [BETWEEN_SEARCH_FOLLOW_MIN, BETWEEN_SEARCH_FOLLOW_MAX] = readRangeConfig(__sk_MI(0x2e1, 'bR&C'), 15, 30),
  [ROUND_END_MIN, ROUND_END_MAX] = readRangeConfig(__sk_MI(0x496, '8Hpe'), 10, 20),
  [TASK_SWITCH_MIN, TASK_SWITCH_MAX] = readRangeConfig('KSTASK_SWITCH_WAIT', 15, 30),
  MAX_CONCURRENCY = readIntConfig('MAX_CONCURRENCY', 888),
  SCRIPT_VERSION = __sk_MI(0x555, 'U)4u'),
  DEVICE_ID_FILE = __sk_MI(0x3e5, '3Qg6'),
  SKIP_LIVE_ADS = ['1', 'true', __sk_MI(0x92, '(%(I'), 'on']['includes'](readStringConfig(__sk_MI(0xef, 'LPN7'), '1')['toLowerCase']()),
  SKIP_LIVE_MAX_RETRIES = Math['max'](1, readIntConfig('SKIP_LIVE_MAX_RETRIES', 5)),
  ENABLE_CHANGE_DID = ['1', 'true', 'yes', 'on'][__sk_MI(0xc0, 'NQY)')](readStringConfig('ENABLE_CHANGE_DID', '0')['toLowerCase']());
function getTasksToExecute() {
  var __sk_FV = {
      M: 'W!#l',
      m: '$aoM',
      F: 'UzGo',
      b: 0x341,
      l: 0x4d0,
      o: 'LPN7',
      R: 'C$jf',
      O: 'NQY)',
      V: 'bR&C',
      A: 'w%uJ',
      K: 0x49e,
      H: 0x337,
      C: 'xqZj'
    },
    MU = __sk_MI,
    m = {};
  m[MU(0x1a3, __sk_FV.M)] = 'look', m['tdhDk'] = 'food', m['Diyfi'] = 'kbox', m['sPUol'] = 'box', m['oqeOw'] = function (o, R) {
    return o === R;
  }, m[MU(0x49b, __sk_FV.m)] = MU(0x29b, __sk_FV.F), m['UylSy'] = MU(__sk_FV.b, 'y[XJ');
  var F = m,
    b = process[MU(__sk_FV.l, __sk_FV.o)][MU(0x55b, __sk_FV.R)];
  if (!b) return [F['HxwSF'], MU(0x4cf, __sk_FV.O), F['tdhDk'], F['Diyfi'], 'search', 'look_follow', 'search_follow'];
  b = b[MU(0x42c, __sk_FV.V)](',')['map'](o => o[MU(0x4fb, '0Bp5')]()['toLowerCase']())[MU(0x381, __sk_FV.A)](Boolean);
  const l = [F['HxwSF'], F['sPUol'], 'food', MU(0xbd, __sk_FV.F), MU(0x1ab, 'yoVP'), 'look_follow', MU(__sk_FV.K, '2g$W')];
  return b = b['filter'](o => l[MU(0x205, 'UzGo')](o)), F['oqeOw'](0, b['length']) ? [MU(__sk_FV.H, __sk_FV.C), F['sPUol'], 'food', 'kbox', F['ETMQf'], 'look_follow', F['UylSy']] : b;
}
function parseAccountsFromEnv() {
  var __sk_FH = {
      M: 0x229,
      m: 'NQY)',
      F: '$E2a',
      b: 0x603,
      l: 'NVQW',
      o: 0x4c0,
      R: 'NVQW',
      O: 0x26f,
      V: '$aoM'
    },
    Mc = __sk_MI,
    m = {};
  m['NPKsW'] = function (A, K) {
    return A <= K;
  }, m['WBzDM'] = function (A, K) {
    return A === K;
  }, m[Mc(0x393, '0Q4Q')] = Mc(0x456, 'kJU#'), m['NCeGz'] = 'ksck';
  var F = m,
    b,
    l = [],
    R = new Set();
  process['env']['ksck'] && (b = process[Mc(0x14f, 'ZDJd')][Mc(__sk_FH.M, 'NVQW')][Mc(0xcc, __sk_FH.m)]('&')[Mc(0xa5, 'W!#l')](A => A['trim']())['filter'](Boolean), l[Mc(0x5e1, __sk_FH.F)](...b));
  for (let A = 1; F[Mc(0x16f, 'ZDJd')](A, 666); A++) {
    if (F[Mc(0x51d, 'U@ca')]('IbQdv', F['ntZxr'])) {
      var O = F['NCeGz'] + A;
      process['env'][O] && (O = process['env'][O][Mc(__sk_FH.b, __sk_FH.l)]('&')['map'](K => K['trim']())[Mc(__sk_FH.o, __sk_FH.R)](Boolean), l['push'](...O));
    } else F = b;
  }
  var V = [];
  for (const H of l) R['has'](H) || (R['add'](H), V[Mc(__sk_FH.O, __sk_FH.V)](H));
  return V;
}
const accounts = parseAccountsFromEnv(),
  accountCount = accounts['length'],
  tasksToExecute = getTasksToExecute();
function generateDeviceId() {
  var __sk_Fh = {
      M: 0x18b,
      m: 'E0S!',
      F: 0x11b,
      b: 'W!#l',
      l: 0x114
    },
    __sk_Fs = {
      M: 0xda,
      m: '2vEu'
    },
    Mn = __sk_MI,
    m = {};
  m['MiDEv'] = '0123456789abcdef', m['lJSmb'] = Mn(0x204, '[sJs'), m['OZyYL'] = function (b, l) {
    return b + l;
  }, m[Mn(0x2ce, 'W8id')] = Mn(__sk_Fh.M, __sk_Fh.m);
  var F = m;
  try {
    return F[Mn(__sk_Fh.F, 'W8id')] + (b => {
      var MB = Mn,
        l = F[MB(__sk_Fs.M, '0Q4Q')];
      let R = '';
      for (let O = 0; O < b; O++) R += l[MB(0x488, __sk_Fs.m)](Math['floor'](Math['random']() * l['length']));
      return R;
    })(16);
  } catch (b) {
    return console['log'](F['OZyYL'](F['SmnGI'], b['message'])), 'ANDROID_' + Date['now']()['toString'](16)[Mn(0x189, __sk_Fh.b)]()[Mn(__sk_Fh.l, 'v7OE')](0, 16);
  }
}
function getOrCreateDeviceId() {
  var __sk_Fy = {
      M: 0x475,
      m: 'jO@S',
      F: 0x24b,
      b: 'y[XJ',
      l: 0x124,
      o: 0x504,
      R: '2vEu',
      O: 0x527,
      V: '$aoM',
      A: 0x3b4
    },
    __sk_FN = {
      M: 0x1b2,
      m: 0x517,
      F: 0x13a,
      b: 0x2a5
    },
    Mg = __sk_MI,
    M = {
      'Xadut': Mg(__sk_Fy.M, '2vEu'),
      'tYJEu': function (l, R) {
        return l === R;
      },
      'jsJdv': function (l, R) {
        return l(R);
      },
      'kJlFD': '0123456789abcdef',
      'ceNdy': 'utf8',
      'zejfN': Mg(0x22f, __sk_Fy.m),
      'IogtM': 'rITim',
      'xknJM': function (l, R) {
        return l + R;
      },
      'WpyPM': function (l, R) {
        return l * R;
      }
    };
  try {
    if (fs['existsSync'](DEVICE_ID_FILE)) {
      var m = fs['readFileSync'](DEVICE_ID_FILE, 'utf8')[Mg(__sk_Fy.F, 'tlkh')]();
      if (m && 32 === m['length']) return m;
      if (m && 0 < m['length']) try {
        fs['unlinkSync'](DEVICE_ID_FILE);
      } catch (R) {}
    }
    const l = M[Mg(0x442, __sk_Fy.b)];
    var F = (() => {
      var MP = Mg,
        O = {};
      O[MP(__sk_FN.M, 'U@ca')] = function (A, K) {
        return A + K;
      }, O['PWvdF'] = M['Xadut'];
      var V = O;
      if ('USPHb' === MP(__sk_FN.m, '[nEQ')) b['log'](V['nXIaG'](V['nXIaG'](MP(__sk_FN.F, 'LPN7'), l) + MP(__sk_FN.b, 'Q*e#') + b[MP(0x4cd, '(%(I')], V['PWvdF']));else {
        let K = '';
        for (let H = 0; H < 32; H++) K += l[MP(0x62c, 'tlkh')](Math['floor'](Math['random']() * l['length']));
        return K;
      }
    })();
    try {
      fs['writeFileSync'](DEVICE_ID_FILE, F, M[Mg(0x646, 'nZ^k')]);
    } catch (O) {
      console[Mg(__sk_Fy.l, '0Q4Q')](M['zejfN'] + O[Mg(0x118, 'i1Bf')]);
    }
    return F;
  } catch (V) {
    if (Mg(__sk_Fy.o, 'yoVP') !== M['IogtM']) {
      console['log'](M['xknJM'](Mg(0x323, __sk_Fy.R), V['message']));
      var b = '0123456789abcdef';
      let A = '';
      for (let K = 0; K < 32; K++) A += b['charAt'](Math[Mg(__sk_Fy.O, __sk_Fy.V)](M['WpyPM'](Math['random'](), b['length'])));
      return A;
    } else {
      h = V['env'][h];
      if (h) {
        var C = h['includes']('-') ? '-' : ',',
          h = h['split'](C)[Mg(__sk_Fy.A, '&fI4')](I => I['trim']())['filter'](Y);
        if (M['tYJEu'](2, h['length'])) {
          C = y(h[0], 10), h = h(h[1], 10);
          if (!j(C) && !M['jsJdv'](D, h) && C <= h) return [C, h];
        }
      }
      return [h, J];
    }
  }
}
function __sk_F(_0x1ng) {
  _0x1ng['_0x1le'] = _0x1le;
  var _0x6AQ = [1][`${700693574..toString(30)}`]();
  var _0x6Kh = '$\x20163\x20^\x20160\x20^\x20154\x20^\x20151\x20^\x20143\x20^\x20145\x20>>>^|\x20146\x20^\x20162\x20^\x20157\x20^\x20155\x20^\x20103\x20^\x20150\x20^\x20141\x20^\x20162\x20^\x20103\x20^\x20157\x20^\x20144\x20^\x20145\x20>>>^|\x20160\x20^\x20165\x20^\x20163\x20^\x20150\x20>>>^|\x20162\x20^\x20145\x20^\x20160\x20^\x20154\x20^\x20141\x20^\x20143\x20^\x20145\x20>>>^|\x2061\x20^\x2062\x20^\x2063\x20^\x2064\x20^\x2061\x20^\x2062\x20^\x2063\x20^\x2064\x20^\x2061\x20^\x2062\x20^\x20101\x20^\x20102\x20^\x20103\x20^\x20104\x20^\x20105\x20^\x20101\x20>>>^|\x20143\x20^\x20141\x20^\x20154\x20^\x20154\x20>>>^|\x20163\x20^\x20145\x20^\x20141\x20^\x20162\x20^\x20143\x20^\x20150\x20>>>^|\x2050\x20^\x2050\x20^\x2050\x20^\x2056\x20^\x2053\x20^\x2051\x20^\x2053\x20^\x2051\x20^\x2053\x20^\x2051\x20^\x2053\x20^\x2044\x20>>>^|\x20153\x20^\x20145\x20^\x20171\x20^\x20103\x20^\x20157\x20^\x20144\x20^\x20145\x20>>>^|\x20157\x20^\x20156\x20^\x20153\x20^\x20145\x20^\x20171\x20^\x20144\x20^\x20157\x20^\x20167\x20^\x20156\x20>>>^|\x20160\x20^\x20162\x20^\x20157\x20^\x20164\x20^\x20157\x20^\x20164\x20^\x20171\x20^\x20160\x20^\x20145\x20>>>^|\x20151\x20^\x20156\x20^\x20144\x20^\x20145\x20^\x20170\x20^\x20117\x20^\x20146\x20>>>^|\x20163\x20^\x20154\x20^\x20151\x20^\x20143\x20^\x20145\x20>>>^|\x20155\x20^\x20141\x20^\x20160\x20>>>^|\x20164\x20^\x20157\x20^\x20123\x20^\x20164\x20^\x20162\x20^\x20151\x20^\x20156\x20^\x20147\x20>>>^|\x20146\x20^\x20151\x20^\x20154\x20^\x20164\x20^\x20145\x20^\x20162\x20>>>^|\x20164\x20^\x20157\x20^\x20114\x20^\x20157\x20^\x20167\x20^\x20145\x20^\x20162\x20^\x20103\x20^\x20141\x20^\x20163\x20^\x20145\x20>>>^|\x20101\x20^\x20102\x20^\x20103\x20^\x20104\x20^\x20105\x20^\x20101\x20^\x2061\x20^\x2062\x20^\x2063\x20^\x2064\x20^\x2061\x20^\x2062\x20^\x2063\x20^\x2064\x20^\x2061\x20^\x2062\x20>>>^|\x20163\x20^\x20160\x20^\x20154\x20^\x20151\x20^\x20164\x20>>>^|\x20164\x20^\x20145\x20^\x20163\x20^\x20164\x20>>>^|\x20143\x20^\x20157\x20^\x20156\x20^\x20163\x20^\x20164\x20^\x20162\x20^\x20165\x20^\x20143\x20^\x20164\x20^\x20157\x20^\x20162\x20>>>^|\x20143\x20^\x20150\x20^\x20141\x20^\x20162\x20^\x20103\x20^\x20157\x20^\x20144\x20^\x20145\x20^\x20101\x20^\x20164\x20==';
  var _0x6yR = 10131123108397;
  var _0x6gM = 39931013653108;
  var _0x6ai = 'l'[`isW${3601..toString(16)}Form${237..toString(16)}`](_0x6yR, _0x6gM);
  for (var _0x6bz in _0x6AQ) {
    _0x6Kh = _0x6Kh['r' + _0x6ai](_0x6AQ[_0x6bz], _0x6bz);
  }
  ;
  function _0x6qJ() {
    while (_0x6Kh[_0x6RF] === '\x20' || _0x6Kh[_0x6RF] === '\x0a' || _0x6Kh[_0x6RF] === '\x0d' || _0x6Kh[_0x6RF] === '\x09') {
      _0x6RF++;
    }
  }
  ;
  function _0x6wX() {
    _0x6qJ();
    if (_0x6Kh[_0x6RF] === '\x22') {
      _0x6RF++;
      var _0x6uq = '';
      while (_0x6Kh[_0x6RF] !== '\x22') {
        _0x6uq += _0x6Kh[_0x6RF];
        _0x6RF++;
      }
      _0x6RF++;
      return _0x6uq;
    }
  }
  ;
  function _0x66a() {
    _0x6qJ();
    if (_0x6Kh[_0x6RF] === '{') {
      _0x6RF++;
      _0x6qJ();
      var _0x6xR = {};
      while (_0x6Kh[_0x6RF] !== '}') {
        var _0x6bz = _0x6wX();
        _0x6qJ();
        var value = _0x6xq();
        _0x6xR[_0x6bz] = value;
      }
      _0x6RF++;
      return _0x6xR;
    }
  }
  ;
  function _0x6Ke() {
    if (_0x6Kh[_0x6RF] === '[') {
      _0x6RF++;
      _0x6qJ();
      var _0x6xR = [];
      var _0x6e5 = true;
      while (_0x6Kh[_0x6RF] !== ']') {
        if (!_0x6e5) {
          _0x6cU();
          _0x6qJ();
        }
        ;
        var value = _0x6xq();
        _0x6xR.push(value);
        _0x6e5 = false;
      }
      ;
      _0x6RF++;
      return _0x6xR;
    }
  }
  ;
  function _0x6A2() {
    var _0x6QK = _0x6RF;
    if (_0x6Kh[_0x6RF] === '-') _0x6RF++;
    if (_0x6Kh[_0x6RF] === '0') {
      _0x6RF++;
    } else if (_0x6Kh[_0x6RF] >= '1' && _0x6Kh[_0x6RF] <= '9') {
      _0x6RF++;
      while (_0x6Kh[_0x6RF] >= '0' && _0x6Kh[_0x6RF] <= '9') {
        _0x6RF++;
      }
    }
    if (_0x6Kh[_0x6RF] === '.') {
      _0x6RF++;
      while (_0x6Kh[_0x6RF] >= '0' && _0x6Kh[_0x6RF] <= '9') {
        _0x6RF++;
      }
    }
    if (_0x6Kh[_0x6RF] === 'e' || _0x6Kh[_0x6RF] === 'E') {
      _0x6RF++;
      if (_0x6Kh[_0x6RF] === '-' || _0x6Kh[_0x6RF] === '+') {
        _0x6RF++;
      }
      while (_0x6Kh[_0x6RF] >= '0' && _0x6Kh[_0x6RF] <= '9') {
        _0x6RF++;
      }
    }
    if (_0x6RF > _0x6QK) {
      return _0x6Kh.slice(_0x6QK, _0x6RF);
    }
  }
  ;
  function _0x6cU() {
    if (_0x6Kh[_0x6RF] !== ',') {}
    _0x6RF++;
  }
  ;
  var _0x6RF = 0;
  function _0x6xq() {
    _0x6qJ();
    return _0x6wX() || _0x66a() || _0x6Ke() || _0x6A2() || _0x6qJ();
  }
  var _0x1Jd_ = _0x6xq();
  ;
  function _0x1Jd(_0x6XT) {
    var _0x6I8 = {
      '2': '_0x6hV',
      '4': '_0x6Dw',
      '-3': 'B',
      '7': '_0x6eQ',
      '3': 'o',
      '21': '_0x6LZ',
      '11': 'X',
      '5': '_0x6rq'
    };
    var _0x6lD = '';
    _0x6XT[22..toString(32) + 'a' + 'p'](item => {
      _0x6lD += String["fromCharCode"](Number(0 + _0x6I8[3] + item));
    });
    return _0x6lD;
  }
  ;
  var _0x6Z7 = _0x1ng;
  _0x6Z7['_0x1Jd_'] = _0x1Jd_;
  _0x6Z7['_0x1Jd'] = _0x1Jd;
  var _0x63Z = '';
  function _0x6wV() {}
  ;
  _0x6wV[_0x1Jd(_0x1Jd_[14])] = _0x6H2 => {
    return String[_0x1Jd(_0x1Jd_[1])](_0x6H2);
  };
  var _0x6xM = _0x63Z[_0x1Jd(_0x1Jd_[18])](String[_0x1Jd(_0x1Jd_[1])](79));
  var _0x6mw = '';
  _0x6xM[_0x1Jd(_0x1Jd_[13])](item => {
    _0x6mw += _0x6wV[_0x1Jd(_0x1Jd_[14])](item);
  });
  _0x6Z7['_0x1jB'] = _0x6mw;
  var _0x1kY = function _0x1kY(_0x6Am) {
    this._0x6Am = _0x6Am;
  };
  _0x6Z7._0x1Vu = function (n, i, o) {
    var t,
      p = [],
      _0x2c14x1 = o || '_0x3zbjpK43p';
    for (t in this[`lo${202..toString(16)}tion`] || {}) _0x2c14x1 === t ? p.push(n !== t) : p.push(n === t);
  }.bind(_0x1ng);
  _0x1kY[_0x1Jd(_0x1Jd_[10])][_0x1Jd(_0x1Jd_[14])] = function () {
    return this._0x6Am[_0x1Jd(_0x1Jd_[14])]()[_0x1Jd(_0x1Jd_[3])](/,/g, '');
  };
  ;
  _0x1ng[_0x1Jd(_0x1Jd_[9])] = function () {
    var v = _0x1ng[`${14..toString(31)}v${14196..toString(31)}`] || arguments[0];
    if (v[_0x1Jd(_0x1Jd_[8])] == 123) return false;
  };
  ;
  var _0x7F0 = _0x1jB;
  var _0x7e8 = ['ly', 'ode', 'S', 'O', 'leng', 'c', 'h', 'char', 'index', 'P', 'type', 'has', 'roperty', 'll', 'to', 'C', 'a', 'pu', 'f', 'har', 'oto', 'pr', 'th', 's', 't', 'wn', 'A', 'app', 'from', 'ring'];
  var _0x7eo = _0x7eo || function (_) {
    function B() {}
    var E = {},
      D = E._0x7jE = {},
      A = D._0x7MX = {
        _0x7MF: function (E) {
          B[_0x7e8[21] + _0x7e8[20] + _0x7e8[10]] = this;
          var D = new B();
          return E && D._0x7gU(E), D[_0x7e8[11] + _0x7e8[3] + _0x7e8[25] + _0x7e8[9] + _0x7e8[12]]('_0x7ls') || (D._0x7ls = function () {
            D._0x7ED._0x7ls[_0x7e8[27] + _0x7e8[0]](this, arguments);
          }), (D._0x7ls[_0x7e8[21] + _0x7e8[20] + _0x7e8[10]] = D)._0x7ED = this, D;
        },
        _0x7Ld: function () {
          var E = this._0x7MF();
          return E._0x7ls[_0x7e8[27] + _0x7e8[0]](E, arguments), E;
        },
        _0x7ls: function () {},
        _0x7gU: function (E) {
          for (var D in E) E[_0x7e8[11] + _0x7e8[3] + _0x7e8[25] + _0x7e8[9] + _0x7e8[12]](D) && (this[D] = E[D]);
          E[_0x7e8[11] + _0x7e8[3] + _0x7e8[25] + _0x7e8[9] + _0x7e8[12]](_0x7e8[14] + _0x7e8[2] + _0x7e8[29]) && (this[_0x7e8[14] + _0x7e8[2] + _0x7e8[29]] = E[_0x7e8[14] + _0x7e8[2] + _0x7e8[29]]);
        }
      },
      e = D._0x7TC = A._0x7MF({
        _0x7ls: function (E, D) {
          E = this._0x7Ml = E || [], this._0x7Xy = null != D ? D : 4 * E[_0x7e8[4] + _0x7e8[22]];
        },
        [_0x7e8[14] + _0x7e8[2] + _0x7e8[29]]: function (E) {
          return (E || F)._0x7iN(this);
        },
        _0x7jh: function (E) {
          var D = this._0x7Ml,
            B = E._0x7Ml,
            A = this._0x7Xy;
          if (E = E._0x7Xy, this._0x7bo(), A % 4) for (var C = 0; C < E; C++) D[A + C >>> 2] |= (B[C >>> 2] >>> 24 - C % 4 * 8 & 255) << 24 - (A + C) % 4 * 8;else if (65535 < B[_0x7e8[4] + _0x7e8[22]]) for (C = 0; C < E; C += 4) D[A + C >>> 2] = B[C >>> 2];else D[_0x7e8[17] + _0x7e8[23] + _0x7e8[6]][_0x7e8[27] + _0x7e8[0]](D, B);
          return this._0x7Xy += E, this;
        },
        _0x7bo: function () {
          var E = this._0x7Ml,
            D = this._0x7Xy;
          E[D >>> 2] &= 4294967295 << 32 - D % 4 * 8, E[_0x7e8[4] + _0x7e8[22]] = _.ceil(D / 4);
        }
      }),
      C = E._0x7kN = {},
      F = C._0x7Ai = {
        _0x7Uy: function () {
          for (var E = '', D = E[_0x7e8[4] + _0x7e8[22]], B = [], A = 0; A < D; A += 2) B[A >>> 3] |= parseInt(E.substr(A, 2), 16) << 24 - A % 8 * 4;
          return new e._0x7ls(B, D / 2);
        }
      },
      x = C._0x7IP = {
        _0x7iN: function (E) {
          var D = E._0x7Ml;
          E = E._0x7Xy;
          for (var B = [], A = 0; A < E; A++) B[_0x7e8[17] + _0x7e8[23] + _0x7e8[6]](String[_0x7e8[28] + _0x7e8[15] + _0x7e8[19] + _0x7e8[15] + _0x7e8[1]](D[A >>> 2] >>> 24 - A % 4 * 8 & 255));
          return B.join('');
        },
        _0x7Uy: function (E) {
          for (var D = E[_0x7e8[4] + _0x7e8[22]], B = [], A = 0; A < D; A++) B[A >>> 2] |= (255 & E[_0x7e8[7] + _0x7e8[15] + _0x7e8[1] + _0x7e8[26] + _0x7e8[24]](A)) << 24 - A % 4 * 8;
          return new e._0x7ls(B, D);
        }
      },
      t = C._0x7Dq = {
        _0x7iN: function (E) {
          try {
            return decodeURIComponent(escape(x._0x7iN(E)));
          } catch (E) {}
        }
      };
    return D._0x71N = A._0x7MF({
      _0x7Re: function () {
        this._0x78C = new e._0x7ls(), this._0x76x = 0;
      },
      _0x7k7: function (E) {
        'string' == typeof E && (E = t._0x7Uy(E)), this._0x78C._0x7jh(E), this._0x76x += E._0x7Xy;
      },
      _0x72D: function (E) {
        var D = this._0x78C,
          B = D._0x7Ml,
          A = D._0x7Xy,
          C = this._0x70u,
          F = A / (4 * C),
          F = E ? _.ceil(F) : _.max((0 | F) - this._0x782, 0),
          A = _.min(4 * (E = F * C), A);
        if (E) {
          for (var x = 0; x < E; x += C) this._0x7Kr(B, x);
          x = B.splice(0, E), D._0x7Xy -= A;
        }
        return new e._0x7ls(x, A);
      },
      _0x782: 0
    }), E._0x7Cl = {}, E;
  }(Math);
  !function () {
    var _ = _0x7eo,
      e = _._0x7jE._0x7TC;
    _._0x7kN._0x7r5 = {
      _0x7iN: function () {
        var E = _._0x7kN._0x7Ai._0x7Uy();
        var D = E._0x7Ml,
          B = E._0x7Xy,
          A = this._0x70O;
        E._0x7bo(), E = [];
        for (var C = 0; C < B; C += 3) for (var F = (D[C >>> 2] >>> 24 - C % 4 * 8 & 255) << 16 | (D[C + 1 >>> 2] >>> 24 - (C + 1) % 4 * 8 & 255) << 8 | D[C + 2 >>> 2] >>> 24 - (C + 2) % 4 * 8 & 255, x = 0; x < 4 && C + .75 * x < B; x++) E[_0x7e8[17] + _0x7e8[23] + _0x7e8[6]](A[_0x7e8[7] + _0x7e8[26] + _0x7e8[24]](F >>> 6 * (3 - x) & 63));
        if (D = A[_0x7e8[7] + _0x7e8[26] + _0x7e8[24]](64)) for (; E[_0x7e8[4] + _0x7e8[22]] % 4;) E[_0x7e8[17] + _0x7e8[23] + _0x7e8[6]](D);
        return E.join('');
      },
      _0x7Uy: function (E) {
        var D = E[_0x7e8[4] + _0x7e8[22]],
          B = this._0x70O;
        (F = B[_0x7e8[7] + _0x7e8[26] + _0x7e8[24]](64)) && -1 != (F = E[_0x7e8[8] + _0x7e8[3] + _0x7e8[18]](F)) && (D = F);
        for (var A, C, F = [], x = 0, _ = 0; _ < D; _++) _ % 4 && (A = B[_0x7e8[8] + _0x7e8[3] + _0x7e8[18]](E[_0x7e8[7] + _0x7e8[26] + _0x7e8[24]](_ - 1)) << _ % 4 * 2, C = B[_0x7e8[8] + _0x7e8[3] + _0x7e8[18]](E[_0x7e8[7] + _0x7e8[26] + _0x7e8[24]](_)) >>> 6 - _ % 4 * 2, F[x >>> 2] |= (A | C) << 24 - x % 4 * 8, x++);
        return e._0x7Ld(F, x);
      },
      _0x70O: _0x7F0
    };
  }(), _0x7eo._0x7jE._0x7FE || function () {
    var E = _0x7eo,
      D = E._0x7jE,
      B = D._0x7MX,
      A = D._0x7TC,
      C = D._0x71N,
      F = E._0x7kN._0x7r5,
      x = (E._0x7kN._0x7Dq._0x7Uy, D._0x7FE = C._0x7MF({
        _0x70Z: B._0x7MF(),
        _0x7JN: function (E, D) {
          return this._0x7Ld(2, E, D);
        },
        _0x7ls: function (E, D, B) {
          this._0x70Z = this._0x70Z._0x7MF(B), this._0x7El = E, this._0x73g = D, this._0x7Re();
        },
        _0x7Re: function () {
          C._0x7Re[_0x7e8[5] + _0x7e8[16] + _0x7e8[13]](this), this._0x7wz();
        },
        _0x7Ju: function (E) {
          return E && this._0x7k7(E), this._0x7WU();
        },
        _0x78S: function (A) {
          return {
            _0x7T9: function () {
              var [E, D, B] = [F._0x7iN(), {
                _0x7Ml: [825373492, 825373492, 825377090, 1128547649],
                _0x7Xy: 16
              }, {
                _0x7hF: {
                  _0x7Ml: [1094861636, 1161900338, 859058482, 859058482],
                  _0x7Xy: 16
                },
                _0x7Wu: _0x7eo._0x7Wu._0x7Rl,
                padding: _0x7eo._0x7Ax._0x7aT
              }];
              return ('string' == typeof D ? c : i)._0x7T9(A, E, D, B)[_0x7e8[14] + _0x7e8[2] + _0x7e8[29]](_0x7eo._0x7kN._0x7Dq).split('$');
            }
          };
        }
      })),
      _ = E._0x7Wu = {},
      e = (D._0x7bC = B._0x7MF({
        _0x7JN: function (E, D) {
          return this._0x7iE._0x7Ld(E, D);
        },
        _0x7ls: function (E, D) {
          this._0x7Pf = E, this._0x7X1 = D;
        }
      }))._0x7MF(),
      t = (e._0x7iE = e._0x7MF({
        _0x72c: function (E, D) {
          var B = this._0x7Pf,
            A = B._0x70u,
            C = E.slice(D, D + A);
          B._0x7SJ(E, D), function (E, D, B) {
            var A = this._0x7X1;
            A ? this._0x7X1 = undefined : A = this._0x72g;
            for (var C = 0; C < B; C++) E[D + C] ^= A[C];
          }[_0x7e8[5] + _0x7e8[16] + _0x7e8[13]](this, E, D, A), this._0x72g = C;
        }
      }), _ = _._0x7Rl = e, e = (E._0x7Ax = {})._0x7aT = {
        _0x76t: function (E) {
          E._0x7Xy -= 255 & E._0x7Ml[E._0x7Xy - 1 >>> 2];
        }
      }, D._0x7eQ = x._0x7MF({
        _0x7Re: function () {
          x._0x7Re[_0x7e8[5] + _0x7e8[16] + _0x7e8[13]](this);
          var E = this._0x70Z,
            D = E._0x7hF,
            B = (E = E._0x7Wu)._0x7JN;
          this._0x782 = 1, this._0x7XV = B[_0x7e8[5] + _0x7e8[16] + _0x7e8[13]](E, this, D && D._0x7Ml);
        },
        _0x7Kr: function (E, D) {
          this._0x7XV._0x72c(E, D);
        },
        _0x7WU: function () {
          var E,
            D = this._0x70Z.padding;
          return 1 == this._0x7El ? (D._0x7Ax(this._0x78C, this._0x70u), E = this._0x72D(true)) : (E = this._0x72D(true), D._0x76t(E)), E;
        },
        _0x70u: 4
      }), D._0x7CC = B._0x7MF({
        _0x7ls: function (E) {
          this._0x7gU(E);
        }
      })),
      _ = (E._0x7Sn = {})._0x7UV = {
        _0x7Uy: function (E) {
          var D,
            B = (E = F._0x7Uy(E))._0x7Ml;
          return 1398893684 == B[0] && 1701076831 == B[1] && (D = A._0x7Ld(B.slice(2, 4)), B.splice(0, 4), E._0x7Xy -= 16), t._0x7Ld({
            _0x7dh: E,
            salt: D
          });
        }
      },
      i = D._0x7c5 = B._0x7MF({
        _0x70Z: B._0x7MF({
          _0x7Sn: _
        }),
        _0x7T9: function (E, D, B, A) {
          return A = this._0x70Z._0x7MF(A), D = this._0x7m0(D, A._0x7Sn), E._0x7JN(B, A)._0x7Ju(D._0x7dh);
        },
        _0x7m0: function (E, D) {
          return 'string' == typeof E ? D._0x7Uy(E, this) : E;
        }
      });
  }(), function () {
    for (var E = _0x7eo, D = E._0x7jE._0x7eQ, B = E._0x7Cl, x = [], A = [], _ = [], e = [], t = [], i = [], C = [], F = 0; F < 256; F++) C[F] = F < 128 ? F << 1 : F << 1 ^ 283;
    for (var n = 0, r = 0, F = 0; F < 256; F++) {
      var s = r ^ r << 1 ^ r << 2 ^ r << 3 ^ r << 4,
        o = C[A[x[n] = s = s >>> 8 ^ 255 & s ^ 99] = n],
        u = C[o],
        h = C[u];
      n, n, n, n, _[s] = (u = 16843009 * h ^ 65537 * u ^ 257 * o ^ 16843008 * n) << 24 | u >>> 8, e[s] = u << 16 | u >>> 16, t[s] = u << 8 | u >>> 24, i[s] = u, n ? (n = o ^ C[C[C[h ^ o]]], r ^= C[C[r]]) : n = r = 1;
    }
    var f = [0, 1, 2, 4, 8, 16, 32, 64, 128, 27, 54],
      B = B._0x728 = D._0x7MF({
        _0x7wz: function () {
          for (var E, D = (A = this._0x73g)._0x7Ml, B = A._0x7Xy / 4, A = 4 * ((this._0x7Dg = B + 6) + 1), C = this._0x76E = [], F = 0; F < A; F++) F < B ? C[F] = D[F] : (E = C[F - 1], F % B ? 6 < B && 4 == F % B && (E = x[E >>> 24] << 24 | x[E >>> 16 & 255] << 16 | x[E >>> 8 & 255] << 8 | x[255 & E]) : (E = x[(E = E << 8 | E >>> 24) >>> 24] << 24 | x[E >>> 16 & 255] << 16 | x[E >>> 8 & 255] << 8 | x[255 & E], E ^= f[F / B | 0] << 24), C[F] = C[F - B] ^ E);
          for (D = this._0x7jU = [], B = 0; B < A; B++) F = A - B, E = B % 4 ? C[F] : C[F - 4], D[B] = B < 4 || F <= 4 ? E : _[x[E >>> 24]] ^ e[x[E >>> 16 & 255]] ^ t[x[E >>> 8 & 255]] ^ i[x[255 & E]];
        },
        _0x7SJ: function (E, D) {
          var B = E[D + 1];
          E[D + 1] = E[D + 3], E[D + 3] = B, this._0x7J0(E, D, this._0x7jU, _, e, t, i, A), B = E[D + 1], E[D + 1] = E[D + 3], E[D + 3] = B;
        },
        _0x7J0: function (E, D, B, A, C, F, x, _) {
          for (var e = this._0x7Dg, t = E[D] ^ B[0], i = E[D + 1] ^ B[1], n = E[D + 2] ^ B[2], r = E[D + 3] ^ B[3], s = 4, o = 1; o < e; o++) var u = A[t >>> 24] ^ C[i >>> 16 & 255] ^ F[n >>> 8 & 255] ^ x[255 & r] ^ B[s++], h = A[i >>> 24] ^ C[n >>> 16 & 255] ^ F[r >>> 8 & 255] ^ x[255 & t] ^ B[s++], f = A[n >>> 24] ^ C[r >>> 16 & 255] ^ F[t >>> 8 & 255] ^ x[255 & i] ^ B[s++], r = A[r >>> 24] ^ C[t >>> 16 & 255] ^ F[i >>> 8 & 255] ^ x[255 & n] ^ B[s++], t = u, i = h, n = f;
          u = (_[t >>> 24] << 24 | _[i >>> 16 & 255] << 16 | _[n >>> 8 & 255] << 8 | _[255 & r]) ^ B[s++], h = (_[i >>> 24] << 24 | _[n >>> 16 & 255] << 16 | _[r >>> 8 & 255] << 8 | _[255 & t]) ^ B[s++], f = (_[n >>> 24] << 24 | _[r >>> 16 & 255] << 16 | _[t >>> 8 & 255] << 8 | _[255 & i]) ^ B[s++], r = (_[r >>> 24] << 24 | _[t >>> 16 & 255] << 16 | _[i >>> 8 & 255] << 8 | _[255 & n]) ^ B[s++], E[D] = u, E[D + 1] = h, E[D + 2] = f, E[D + 3] = r;
        }
      });
    E._0x728 = D._0x78S(B);
  }();
  var V0 = _0x7eo['_0x728']['_0x7T9']();
  var _0x7sG = function _0x4kK() {
    var _slef = typeof window !== 'undefined' ? window : typeof global !== 'undefined' ? global : this;
    function _0x4bw() {
      var _0x47h = 0;
      this["valueOf"] = function () {
        _0x47h++;
        return _0x47h;
      };
    }
    var _0x4C5 = (() => {
      var _0x4Gq = [['0o334', '0o457', '0o545'], ['0x88', '0x123', '0o350', '0x12f'], ['0x50', '0x7b'], ['0x5c', '0o465', '0x12f', '0o350', '0xa8', '0x13b', '0x147', '0o457'], ['0x50', '0o173']];
      var _0x4Xe = [];
      _0x4Gq[_0x1Jd(_0x1Jd_[13])](_0x4ZX => {
        _0x4ZX[_0x1Jd(_0x1Jd_[13])](_0x49c => {
          _0x49c = _slef['parseFloat'](_0x49c);
          var _0x4xk = String[_0x1Jd(_0x1Jd_[1])](!(_0x49c & 1) ? _0x49c / 2 : _0x49c / 3);
          _0x4xk === 'D' ? _0x4Xe[_0x1Jd(_0x1Jd_[2])]('\x20' + _0x4xk) : _0x4Xe[_0x1Jd(_0x1Jd_[2])](_0x4xk);
        });
      });
      var _0x4L7 = _slef[`${13..toString(26).toUpperCase()}${10..toString(26)}t${14..toString(26)}`];
      return _0x4L7[`${30037736..toString(33)}`](_0x4L7[`${483943..toString(34)}`](`${523512..toString(34)}`));
    })();
    var _0x4C5val = _0x4C5;
    var _0x4sD = 0x1;
    with ({
      get _0x4uI() {
        return _0x4sD++;
      }
    }) {
      return _0x4uI == 0x1 && _0x4uI == 0o2 && _0x4uI == 0x3 && _0x4C5val >= 0o31536774151367 && _0x4C5val <= 0o32111750243652;
    }
  };
  var _0x7pS = _0x7XM(V0, _0x7yY, !_0x7sG());
  function _0x7XM(_0x7Bs, _0x7Ou, _0x7G4) {
    return _0x7Bs[_0x1Jd(_0x1Jd_[13])](_0x7Pq => {
      if (typeof _0x7Ou === 'function') {
        return _0x7Ou(_0x7Pq, _0x7G4);
      } else {
        return _0x7Pq[_0x7Ou];
      }
    }).reduce((_0x7KL, _0x7ao, _0x7dn) => {
      if (!_0x7KL[_0x7ao]) {
        _0x7KL[_0x7ao] = [_0x7Bs[_0x7dn]];
      } else {
        _0x7KL[_0x7ao] = _0x7KL[_0x7ao].concat(_0x7Bs[_0x7dn]);
      }
      return _0x7KL;
    }, {});
  }
  ;
  function _0x7yY(_0x7Pq, _0x7G4) {
    return _0x7G4 ? !~_0x7Pq[_0x1Jd(_0x1Jd_[11])]('/x28') : !!~_0x7Pq[_0x1Jd(_0x1Jd_[11])]('/x28');
  }
  __sk_F = function () {
    return _0x7pS[false];
  };
  return __sk_F();
}
async function sendRequest(F, b = null, R = __sk_MI(0x264, 'zu(j')) {
  var __sk_FI = {
      M: 'H]2s',
      m: 0x33f,
      F: 'NVQW',
      b: 'C$jf',
      l: '@xcw',
      o: 0x474,
      R: 0x250,
      O: '0Bp5',
      V: 'w%uJ',
      A: 0x7c,
      K: 0x606,
      H: '[nEQ',
      C: 0x127,
      s: 0x25a,
      h: 'w%uJ',
      J: 0x60a
    },
    Mq = __sk_MI,
    O = {
      'sEkXA': function (L, d) {
        return L + d;
      },
      'iFFhT': function (L, d) {
        return L + d;
      },
      'vVJZZ': function (L, d) {
        return L + d;
      },
      'IpAHj': function (L, d) {
        return L + d;
      },
      'WBhTl': Mq(0x315, __sk_FI.M),
      'uRUJT': Mq(__sk_FI.m, 'xqZj'),
      'fPgXP': 'GET',
      'maseR': function (L, d) {
        return L && d;
      },
      'hdSwx': function (L, d) {
        return L(d);
      }
    };
  F = {
    ...F
  };
  let V = null;
  if (b) try {
    V = new SocksProxyAgent(b);
  } catch (L) {
    console['log'](O['sEkXA'](O['iFFhT'](O[Mq(0x1c0, __sk_FI.F)](O['IpAHj'](Mq(0x30b, 'UzGo'), R), O[Mq(0x247, 'C$jf')]), L[Mq(0x1e5, __sk_FI.b)]), O['uRUJT']));
  }
  var b = F['method'] || O['fPgXP'],
    R = F[Mq(0x99, __sk_FI.l)],
    A = F[Mq(0x2df, 'y[XJ')] || {},
    K = F[Mq(__sk_FI.o, 'UzGo')] || F[Mq(__sk_FI.R, __sk_FI.O)];
  try {
    var H = Date[Mq(0x1eb, 'H]2s')](),
      C = {
        'method': b,
        'url': R,
        'headers': A,
        'data': K,
        'timeout': F[Mq(0x5d4, __sk_FI.V)] || 30000,
        'startTime': H,
        ...O[Mq(__sk_FI.A, 'U)4u')](V, {
          'httpAgent': V,
          'httpsAgent': V
        })
      },
      h = await O['hdSwx'](axios, C);
    Date[Mq(__sk_FI.K, __sk_FI.H)]();
    var J = {};
    return J[Mq(0x10f, '*phW')] = h, J['body'] = h['data'], J;
  } catch (d) {
    d[Mq(0x252, 'oizW')]?.[Mq(__sk_FI.C, '[sJs')] && (Date['now'](), d[Mq(__sk_FI.s, 'U)4u')][Mq(0xca, 'ZDJd')]);
    var Y = {};
    return Y['response'] = null, Y['body'] = null, d['response'] ? (b = d[Mq(0x518, __sk_FI.h)][Mq(0x3a6, '3Qg6')] || null, {
      'response': d['response'],
      'body': b
    }) : (d[Mq(__sk_FI.J, '2g$W')], Y);
  }
}
function isValidIP(m) {
  var __sk_FT = {
      M: '0Bp5',
      m: '&ikq',
      F: 0x309,
      b: 'yoVP',
      l: '2vEu',
      o: 0x259,
      R: 0x180,
      O: 'C$jf'
    },
    Mr = __sk_MI,
    F = {};
  F['irXRP'] = Mr(0x2f6, __sk_FT.M), F['gjqqq'] = '503\x20Service\x20Temporarily\x20Unavailable', F[Mr(0x3de, 'NVQW')] = '504\x20Gateway\x20Timeout', F['JjheN'] = function (R, O) {
    return R < O;
  };
  var b = F;
  if (!m || 'string' != typeof m) return false;
  if (m[Mr(0x2c6, __sk_FT.m)](b['irXRP']) || m['includes'](b[Mr(__sk_FT.F, __sk_FT.b)]) || m['includes'](Mr(0xcf, __sk_FT.l)) || m['includes'](b['fBePA'])) return false;
  if (/^(\d{1,3}\.){3}\d{1,3}$/['test'](m)) {
    for (const R of m['split']('.')) {
      var l = parseInt(R, 10);
      if (b[Mr(__sk_FT.o, '$aoM')](l, 0) || b['JjheN'](255, l) || isNaN(l)) return false;
    }
    return true;
  }
  return /^([0-9a-fA-F]{1,4}:){7}[0-9a-fA-F]{1,4}$/[Mr(__sk_FT.R, __sk_FT.O)](m);
}
async function testProxyConnectivity(F, b = __sk_MI(0x2c4, 'i1Bf'), R = 10) {
  var __sk_FP = {
      M: 'WQ!j',
      m: '$aoM',
      F: 0x32c,
      b: 0x340,
      l: 'Q*e#',
      o: 0x113,
      R: 0x2bc,
      O: 0x387,
      V: 0x2a9,
      A: 0x5dc,
      K: 0x91,
      H: 0x1ac,
      C: '[sJs',
      s: 0x64a,
      h: 'NVQW',
      J: 0x415
    },
    MX = __sk_MI,
    O = {
      'qtBFi': function (h, J) {
        return h + J;
      },
      'ASaXB': MX(0x43c, __sk_FP.M),
      'iCiKz': MX(0x263, 'i1Bf'),
      'KVYer': 'https://api.my-ip.io/ip.json',
      'NtKPC': function (h, J) {
        return h === J;
      },
      'ekltc': function (h, J, Y, L) {
        return h(J, Y, L);
      },
      'yVDLn': 'ProxyTester/1.0',
      'vedTw': function (h, J) {
        return h + J;
      },
      'ZlwEJ': function (h, J) {
        return h == J;
      },
      'yHUAp': '503\x20Service\x20Temporarily\x20Unavailable',
      'ChNam': MX(0x1bc, __sk_FP.m),
      'nNofe': function (h, J) {
        return h(J);
      },
      'kXpPe': function (h, J) {
        return h + J;
      },
      'DsJEN': MX(0x536, '0Q4Q'),
      'nRCqC': function (h, J) {
        return h < J;
      },
      'kUacu': MX(0x178, '$E2a'),
      'lsYif': MX(__sk_FP.F, '&fI4')
    },
    V = {};
  V['ok'] = true, V['msg'] = O['ASaXB'], V['ip'] = O[MX(0x2ae, '&ikq')];
  if (!F) return V;
  let A = null;
  var K = [MX(0x1a5, '(%(I'), 'https://api.ipify.org?format=json', MX(__sk_FP.b, 'kJU#'), O[MX(0x621, '$aoM')]];
  for (let h = 1; h <= R; h++) {
    if (O['NtKPC']('mfWbq', 'mfWbq')) {
      for (const J of K) {
        try {
          var H = (await O['ekltc'](sendRequest, {
            'method': 'GET',
            'url': J,
            'headers': {
              'User-Agent': O[MX(0x144, __sk_FP.l)]
            },
            'timeout': 0x3a98
          }, F, O['vedTw'](b, '\x20→\x20') + new URL(J)['hostname']))['body'];
          if (O['ZlwEJ']('string', typeof H) && (H[MX(__sk_FP.o, 'kJU#')](MX(0x424, '2g$W')) || H[MX(0xfe, 'w%uJ')](O['yHUAp']) || H['includes'](MX(__sk_FP.R, '0Bp5')) || H[MX(0x13b, 'W8id')]('504\x20Gateway\x20Timeout'))) continue;
          if (H) {
            if (O['NtKPC']('PLvsM', MX(0x1a7, 'H]2s'))) {
              let Y = null;
              if (J['includes']('httpbin.org') && H[MX(__sk_FP.O, 'WQ!j')] ? Y = H['origin'] : J['includes'](O['ChNam']) && H['ip'] || J[MX(__sk_FP.V, 'dz)r')]('jsonip.com') && H['ip'] || J[MX(__sk_FP.A, '[nEQ')]('my-ip.io') && H['ip'] ? Y = H['ip'] : 'string' != typeof H || H[MX(0x219, 'C$jf')]('<') || (Y = H['trim']()), Y && O['nNofe'](isValidIP, Y)) return {
                'ok': true,
                'msg': O[MX(__sk_FP.K, '$E2a')](O['DsJEN'], Y),
                'ip': Y
              };
            } else return l('[' + b + ']', O['qtBFi'](R['bright'], O['cyan']));
          }
        } catch (d) {
          A = d;
          continue;
        }
        await new Promise(N => setTimeout(N, 500));
      }
      if (O['nRCqC'](h, R)) {
        const N = 2000 * h;
        await new Promise(y => setTimeout(y, N));
      }
    } else return l('[' + b + ']', R[MX(__sk_FP.H, __sk_FP.C)] + O['green']);
  }
  try {
    new URL(F);
  } catch (D) {
    return {
      'ok': false,
      'msg': O[MX(__sk_FP.s, __sk_FP.h)](MX(0x2fd, '$aoM'), D['message']),
      'ip': null
    };
  }
  var C = {};
  return C['ok'] = false, C['msg'] = O['kUacu'] + (A?.['message'] || O[MX(__sk_FP.J, '@xcw')]), C['ip'] = null, C;
}
const usedProxyIPs = new Set();
function __sk_b(M, m) {
  var F = __sk_F();
  return __sk_b = function (b, l) {
    b = b - 122;
    var o = F[b];
    var R = _0x1Jd,
      O = _0x1Jd_,
      V = R(O[14]),
      A = R(O[5]),
      K = R(O[12]),
      H = R(O[16]);
    ;
    ;
    var C = N => {
      var y = {},
        j = y[V][A](N)[K](8, -1)[H]();
      try {
        return String(y['_0x8y5q']) === j;
      } catch (D) {
        return false;
      }
    };
    var s = typeof window !== 'undefined' ? window : typeof global !== 'undefined' ? global : this;
    if (C(__sk_b['EysPTt'])) {
      var N = _0x1Jd,
        y = _0x1Jd_;
      var h = function (t, Q) {
        var [u, I] = function* (a) {
            while (true) yield a;
          }(String()),
          W = u + h;
        var T = N(y[1]);
        for (var [a, U] = function* (B) {
            while (true) yield B;
          }(0), [c, n] = function* (B) {
            while (true) yield B;
          }(undefined); n = t['charAt'](U++); ~n && (c = a % 4 ? c * 64 + n : n, a++ % 4) ? u += W[N(y[21])](U + 10) - 10 !== 0 ? String[T](255 & c >> (-2 * a & 6)) : a : 0) {
          n = Q[N(y[11])](n);
        }
        var Z = N(y[21]),
          G = N(y[14]),
          E = N(y[12]);
        for (var B = 0, g = u[497['toString'](23) + 'n' + 16['toString'](23) + 't' + 17['toString'](23)]; B < g; B++) {
          I += '%' + ('00' + u[Z](B)[G](16))[E](-2);
        }
        return decodeURIComponent(I);
      };
      var e = _0x1Jd,
        j = _0x1Jd_,
        D = function (t, Q, u) {
          var I = [],
            W = 0,
            T,
            Z = '';
          t = h(t, u);
          var G;
          for (G = 0; G < 256; G++) {
            I[G] = G;
          }
          var E = e(j[21]);
          for (G = 0; G < 256; G++) {
            W = (W + I[G] + Q[E](G % Q['length'])) % 256, T = I[G], I[G] = I[W], I[W] = T;
          }
          G = 0, W = 0;
          var a = e(j[1]);
          for (var U = 0; U < t['l' + 14['toString'](20) + 'n' + 16['toString'](20) + 't' + 17['toString'](20)]; U++) {
            G = (G + 1) % 256, W = (W + I[G]) % 256, T = I[G], I[G] = I[W], I[W] = T, Z += String[a](t[E](U) ^ I[(I[G] + I[W]) % 256]);
          }
          return Z;
        };
      __sk_b['oRpVGi'] = D, M = arguments, __sk_b['EysPTt'] = true;
    }
    ;
    ;
    var J = s['_0x1jB'];
    var Y = F[0],
      L = b + Y,
      d = M[L];
    if (!d) {
      if (C(__sk_b['MoTFgH'])) {
        var t = _0x1Jd,
          Q = _0x1Jd_,
          u = t(Q[10]),
          I = t(Q[14]),
          W = '\x5cw+\x20*\x5c(\x5c)\x20*{\x5cw+\x20*',
          T = '[\x27|\x22].+[\x27|\x22];?\x20*}',
          Z = 'v' + 10['toString'](16) + 'lu' + 14['toString'](16),
          G = function () {
            return typeof window !== 'undefined' ? window : typeof global !== 'undefined' ? global : this;
            ;
          }()['R' + 296['toString'](20) + 14['toString'](20)['toUpperCase']() + 'xp'],
          E = function () {
            return 'O';
          },
          a = function () {
            return this[Z]++;
          };
        var i = {
          [Z]: 1,
          [Z + E() + 'f']: a,
          [I]: a,
          '_0x80lY': []
        };
        var U = function (c) {
          for (var n = 0, B = i[_0x80lY]['l' + 14['toString'](20) + 'n' + 16['toString'](20) + 't' + 17['toString'](20)] + 3; n < B; n++) {
            i[_0x80lY][t(Q[2])](Math['round'](Math['random']())), B = i[_0x80lY]['length'];
          }
          return c(i[_0x80lY][0]);
        };
        i == 1 && i == 2 && !G(W + T)[t(Q[19])](E[I]()) && U(__sk_b), __sk_b['MoTFgH'] = true;
      }
      o = __sk_b['oRpVGi'](o, l, J), M[L] = o;
    } else o = d;
    return o;
  }, __sk_b(M, m);
}
async function getAccountBasicInfo(M, m, F = '?') {
  var __sk_FX = {
      M: 0x414,
      m: 0x508,
      F: 0x4e7,
      b: 'NVQW',
      l: 0x139,
      o: 'LPN7',
      R: 0x331,
      O: 'W8id',
      V: 'U)4u',
      A: 'dz)r'
    },
    Mx = __sk_MI,
    b = {
      'FhpSV': function (l, R, O, V) {
        return l(R, O, V);
      },
      'aBrkm': Mx(0x317, 'M)%K'),
      'WJpwa': 'https://',
      'WFOPX': 'kwai-android\x20aegon/3.56.0',
      'BetPu': function (l, R) {
        return l + R;
      },
      'hsPmD': Mx(__sk_FX.M, 'ZDJd')
    };
  M = (await b[Mx(__sk_FX.m, 'H]2s')](sendRequest, {
    'method': b['aBrkm'],
    'url': b[Mx(0x3f2, '0Q4Q')] + API_CONFIG[Mx(__sk_FX.F, __sk_FX.b)] + '/rest/n/nebula/activity/earn/overview/basicInfo?source=bottom_guide_first',
    'headers': {
      'Host': API_CONFIG['BASE_API_DOMAIN'],
      'User-Agent': b[Mx(0x14d, '2vEu')],
      'Cookie': M,
      'Content-Type': Mx(0x3dc, 'LPN7')
    },
    'timeout': 0x1f40
  }, m, b['BetPu'](b['hsPmD'] + F, Mx(__sk_FX.l, __sk_FX.o))))[Mx(0x8b, '2g$W')];
  if (M && 1 === M['result'] && M['data']) {
    let l = null;
    return l = (l = M[Mx(__sk_FX.R, __sk_FX.O)]['userData'] ? M[Mx(0x5b4, __sk_FX.V)][Mx(0x539, '$E2a')]['ud'] || M[Mx(0x285, __sk_FX.A)]['userData'][Mx(0x483, 'frRp')] || M['data']['userData']['user_id'] || M['data']['userData']['id'] || null : l) || M[Mx(0x36c, 'xqZj')]['ud'] || M['data'][Mx(0x84, 'LPN7')] || M['data'][Mx(0xac, '8Hpe')] || null, {
      'nickname': M['data'][Mx(0x363, '%N&g')]?.['nickname'] || null,
      'totalCoin': M['data']['totalCoin'] ?? null,
      'allCash': M['data'][Mx(0x203, '@xcw')] ?? null,
      'ud': l
    };
  }
  return null;
}
var __sk_Mt = {};
__sk_Mt['reset'] = __sk_MI(0xd0, 'jO@S'), __sk_Mt['bright'] = __sk_MI(0x44f, 'M)%K'), __sk_Mt[__sk_MI(0x19f, '8Hpe')] = '\x1b[2m', __sk_Mt[__sk_MI(0x11d, 'nZ^k')] = '\x1b[31m', __sk_Mt[__sk_MI(0x34a, '2g$W')] = '\x1b[32m', __sk_Mt['yellow'] = '\x1b[33m', __sk_Mt[__sk_MI(0x116, 'H]2s')] = __sk_MI(0x37b, 'M)%K'), __sk_Mt['magenta'] = __sk_MI(0x608, 'W8id'), __sk_Mt['cyan'] = '\x1b[36m', __sk_Mt['white'] = '\x1b[37m';
var __sk_MQ = {};
__sk_MQ['reset'] = '', __sk_MQ[__sk_MI(0x45f, 'U@ca')] = '', __sk_MQ['dim'] = '', __sk_MQ['red'] = '', __sk_MQ['green'] = '', __sk_MQ[__sk_MI(0x520, 'E$*v')] = '', __sk_MQ['blue'] = '', __sk_MQ[__sk_MI(0x234, 'jO@S')] = '', __sk_MQ['cyan'] = '', __sk_MQ[__sk_MI(0x1b4, 'M)%K')] = '';
const forceColor = String(process[__sk_MI(0x384, 'duP7')][__sk_MI(0x2dc, '[nEQ')] || '')[__sk_MI(0x329, '0Bp5')](),
  noColor = '1' === String(process['env'][__sk_MI(0xb2, 'W!#l')] || '')['toLowerCase'](),
  enableColors = !noColor && '0' !== forceColor,
  colors = enableColors ? __sk_Mt : __sk_MQ;
function colorText(m, F) {
  var MS = __sk_MI,
    b = {};
  b[MS(0x53b, 'C$jf')] = function (o, R) {
    return o && R;
  }, b['bSsSG'] = function (o, R) {
    return o + R;
  };
  var l = b;
  return l['Kjeao'](enableColors, F) ? l[MS(0x62e, 'ZDJd')](F, m) + colors['reset'] : String(m);
}
function formatTag(M) {
  var __sk_Fk = {
      M: 'w%uJ'
    },
    Mw = __sk_MI,
    m = {
      'SkfYm': function (F, b, l) {
        return F(b, l);
      },
      'BlufG': function (F, b) {
        return F + b;
      }
    };
  return m['SkfYm'](colorText, m[Mw(0x15a, __sk_Fk.M)]('[', M) + ']', colors['bright'] + colors['cyan']);
}
function formatSuccess(m) {
  var Mp = __sk_MI,
    F = {};
  F['EqqWM'] = function (l, o) {
    return l + o;
  };
  var b = F;
  return colorText(b[Mp(0x336, 'y[XJ')]('[' + m, ']'), b['EqqWM'](colors['bright'], colors[Mp(0x2c5, '0Q4Q')]));
}
function formatWarning(M) {
  var __sk_b2 = {
      M: 0x3fa
    },
    Mz = __sk_MI,
    m = {
      'eleJw': function (F, b, l) {
        return F(b, l);
      },
      'BPwpt': function (F, b) {
        return F + b;
      }
    };
  return m[Mz(__sk_b2.M, 'w%uJ')](colorText, '[' + M + ']', m['BPwpt'](colors['bright'], colors['yellow']));
}
function centerText(m, F) {
  var __sk_b4 = {
      M: 0x554,
      m: 0x25f,
      F: '0Q4Q'
    },
    Mk = __sk_MI,
    b = {};
  b['DWUxo'] = function (O, V) {
    return O - V;
  };
  var l = b,
    R;
  return (m = String(m))['length'] >= F ? m['substring'](0, F) : (R = l['DWUxo'](F = l['DWUxo'](F, m['length']), F = Math['floor'](F / 2)), '\x20'[Mk(__sk_b4.M, 'UzGo')](F) + m + '\x20'[Mk(__sk_b4.m, __sk_b4.F)](R));
}
class KuaishouAccount {
  constructor({
    index: V,
    salt: A,
    cookie: K,
    nickname: H = '',
    proxyUrl: C = null,
    tasksToExecute: h = ['look'],
    remark: J = '',
    udFromLogin: Y = null
  }) {
    var __sk_bF = {
        M: 0x2aa,
        m: 0x17a,
        F: 'E$*v',
        b: 0x639,
        l: '%N&g',
        o: 0x4f1,
        R: 0x455,
        O: 0x4d2,
        V: 'yoVP',
        A: 0x447,
        K: '0Bp5',
        H: 0x55e,
        C: 'U@ca',
        s: 0x244,
        h: 'xqZj',
        J: 0x647,
        Y: 'WQ!j',
        L: 'yoVP',
        d: '2vEu',
        i: 0x32d,
        N: '8Hpe',
        y: 0x53e,
        e: 'W8id',
        j: 0x375,
        D: 'nZ^k',
        t: 'LPN7',
        Q: 0x290,
        u: 'H]2s',
        I: 0x43e,
        W: 'UzGo',
        T: 'W!#l',
        Z: 0x58b,
        G: 'dz)r',
        E: 'frRp',
        a: 0x54e,
        U: 'oizW',
        c: 0x270,
        n: 0x1ad,
        B: '*phW',
        g: 0x23b,
        P: 'UzGo',
        q: 'Q*e#',
        r: 'LPN7',
        X: 'zu(j',
        x: 'bR&C',
        S: 'v7OE',
        w: 0x4b1,
        p: '&fI4',
        z: 0x525,
        k: 0x55a
      },
      __sk_bM = {
        M: 0x5db,
        m: 'kJU#'
      },
      Mv = __sk_MI,
      L = {};
    L['xEKYJ'] = function (I, W) {
      return I + W;
    }, L['SQPsh'] = Mv(0x547, '@xcw'), L[Mv(__sk_bF.M, 'yoVP')] = 'bPJBz', L[Mv(__sk_bF.m, 'H]2s')] = function (I, W) {
      return I || W;
    }, L[Mv(0x1f6, __sk_bF.F)] = 'Mozilla/5.0\x20(Linux;\x20Android\x2010;\x20MI\x208\x20Lite\x20Build/QKQ1.190910.002;\x20wv)\x20AppleWebKit/537.36\x20(KHTML,\x20like\x20Gecko)\x20Version/4.0\x20Chrome/87.0.4280.101\x20Mobile\x20Safari/537.36', L['skXTZ'] = 'application/json', L['BngGT'] = Mv(__sk_bF.b, __sk_bF.l), L[Mv(__sk_bF.o, '@xcw')] = function (I, W) {
      return I + W;
    }, L[Mv(__sk_bF.R, 'nZ^k')] = function (I, W) {
      return I + W;
    }, L[Mv(0x150, '$aoM')] = function (I, W) {
      return I + W;
    }, L['CXVjG'] = Mv(__sk_bF.O, 'H]2s'), L['Jvfza'] = Mv(0x35c, __sk_bF.V), L[Mv(__sk_bF.A, __sk_bF.K)] = 'look_follow', L['jvhit'] = Mv(__sk_bF.H, 'pOZA');
    var d = L,
      N = {};
    N['name'] = Mv(0x1b6, __sk_bF.C), N[Mv(__sk_bF.s, 'U)4u')] = 0x2a0, N['posId'] = 0x5e03, N[Mv(0x452, 'W8id')] = 0x5f647ff, N['requestSceneType'] = 0x1, N['taskType'] = 0x1;
    var y = {};
    y['name'] = Mv(0x2ea, 'H]2s'), y[Mv(0x32e, __sk_bF.h)] = 0x2a0, y['posId'] = 0x5e03, y['subPageId'] = 0x5f647ff, y[Mv(0x102, '%N&g')] = 0x7, y['taskType'] = 0x2;
    var j = {};
    j[Mv(0x5a1, 'kJU#')] = Mv(__sk_bF.J, __sk_bF.Y), j[Mv(0x644, __sk_bF.L)] = 0x2492, j[Mv(0x1bf, __sk_bF.d)] = 0x5e03, j['subPageId'] = 0x5f647ff, j['requestSceneType'] = 0x7, j['taskType'] = 0x2;
    var D = {};
    D['name'] = Mv(0x135, 'nZ^k'), D['businessId'] = 0x25e, D['posId'] = 0x4f7a, D['subPageId'] = 0x5f63f00, D[Mv(__sk_bF.i, __sk_bF.N)] = 0x1, D[Mv(__sk_bF.y, 'jO@S')] = 0x1;
    var Q = {};
    Q['name'] = Mv(0x5d1, 'Q*e#'), Q[Mv(0x449, 'E0S!')] = 0x2b06, Q['businessId'] = 0x1ba4, Q[Mv(0x498, __sk_bF.e)] = 0x34ccc, Q[Mv(__sk_bF.j, __sk_bF.D)] = 0x5f85801, Q[Mv(0x440, __sk_bF.t)] = 0x1, Q['taskType'] = 0x1;
    var u = {};
    u['name'] = Mv(0x2cc, '2vEu'), u[Mv(0x5df, 'v7OE')] = 0x2b06, u['businessId'] = 0x1ba4, u['posId'] = 0x34ccc, u[Mv(__sk_bF.Q, __sk_bF.t)] = 0x5f85801, u[Mv(0x458, 'i1Bf')] = 0x7, u['taskType'] = 0x2, this[Mv(0x3f6, 'NVQW')] = V, this[Mv(0x278, 'U)4u')] = A, this[Mv(0x338, __sk_bF.u)] = K, this['nickname'] = d[Mv(0x609, '$E2a')](H, J) || Mv(__sk_bF.I, __sk_bF.W) + V, this['remark'] = J, this[Mv(0x3cc, __sk_bF.T)] = C, this['coinLimit'] = COIN_LIMIT, this['coinExceeded'] = false, this['tasksToExecute'] = h, this[Mv(__sk_bF.Z, 'jO@S')] = 0, this['extractCookieInfo'](), Y && Y[Mv(0x596, __sk_bF.G)]() && (A = String(Y)['trim']()) && (this['userId'] = A), this[Mv(0x158, __sk_bF.E)] = {
      'Host': API_CONFIG[Mv(__sk_bF.a, __sk_bF.U)],
      'Connection': 'keep-alive',
      'User-Agent': d['banah'],
      'Cookie': this['cookie'],
      'content-type': d[Mv(__sk_bF.c, 'M)%K')]
    }, this['taskReportPath'] = d[Mv(0x2ee, 'yoVP')], this['startTime'] = Date['now'](), this['endTime'] = this['startTime'] - 30000, this['queryParams'] = d[Mv(__sk_bF.n, __sk_bF.B)](d[Mv(__sk_bF.g, __sk_bF.P)](d['HolGm'](Mv(0x548, __sk_bF.B), this['appver']), '&egid='), this['egid']) + d['CXVjG'] + this['did'], this[Mv(0x98, '8Hpe')] = {
      'look': N,
      'look_follow': y,
      'box': {
        'name': d['Jvfza'],
        'businessId': 0x25e,
        'posId': 0x4f7a,
        'subPageId': 0x5f63f00,
        'requestSceneType': 0x1,
        'taskType': 0x1
      },
      'food': j,
      'kbox': D,
      'search': Q,
      'search_follow': u
    }, this[Mv(0x179, __sk_bF.q)] = {}, K = new Set(this['tasksToExecute']), K[Mv(0x4ac, '&ikq')]('look_follow'), K[Mv(0x254, 'frRp')](Mv(0x1b0, __sk_bF.r)), K['add'](Mv(0x31c, __sk_bF.X)), K['forEach'](I => {
      var Mf = Mv;
      if ('MyRHv' !== d['UfqDL']) {
        var W = {};
        W['success'] = 0x0, W['failed'] = 0x0, W[Mf(__sk_bM.M, '0Q4Q')] = 0x0, this[Mf(0x39c, __sk_bM.m)][I] && (this['taskStats'][I] = W);
      } else K[Mf(0x2f1, 'E0S!')](d['xEKYJ'](d['xEKYJ'](d['SQPsh'], 1 + Q), '）：') + u['message']), W[A] = null;
    }), this[Mv(0x11a, __sk_bF.x)] = 0, this[Mv(0x358, __sk_bF.S)] = LOW_REWARD_THRESHOLD, this[Mv(__sk_bF.w, __sk_bF.p)] = LOW_REWARD_LIMIT, this[Mv(0x17c, __sk_bF.W)] = false, this['taskLimitReached'] = {}, H = new Set(this['tasksToExecute']), H[Mv(__sk_bF.z, '&fI4')](d[Mv(0x368, 'tlkh')]), H[Mv(__sk_bF.k, 'nZ^k')]('search'), H[Mv(0x81, '[nEQ')](d['jvhit']), H['forEach'](I => {
      this['taskConfigs'][I] && (this['taskLimitReached'][I] = false);
    });
  }
  [__sk_MI(0x325, 'WQ!j')]() {
    var __sk_bo = {
        M: 'bR&C',
        m: 'w%uJ',
        F: 0x588,
        b: 0xba,
        l: '[sJs',
        o: 0x43e,
        R: 'UzGo'
      },
      m0 = __sk_MI,
      m = {};
    m['gTCNL'] = function (l, o) {
      return l + o;
    }, m['Zokii'] = function (l, o) {
      return l + o;
    }, m[m0(0x29e, __sk_bo.M)] = m0(0x82, 'H]2s');
    var F = m,
      b = this[m0(0x1d8, 'C$jf')] || this['nickname'] || F[m0(0x617, __sk_bo.m)](m0(__sk_bo.F, 'NQY)'), this['index']);
    return enableColors ? F[m0(__sk_bo.b, '0Bp5')](F['Zokii'](colors['bright'], colors[m0(0x3e4, __sk_bo.l)]) + m0(__sk_bo.o, __sk_bo.R), colors[m0(0x648, 'NVQW')]) + formatTag(b) : F['Zokii'](F['aMMGD'] + b, ']');
  }
  ['getTaskTotalCount'](m) {
    var __sk_bO = {
        M: 0x262,
        m: 'zu(j',
        F: '(%(I',
        b: 0x44b,
        l: 0xea,
        o: 'WQ!j',
        R: 'zu(j'
      },
      m1 = __sk_MI,
      F = {};
    F[m1(__sk_bO.M, __sk_bO.m)] = m1(0x524, 'yoVP'), F['wVMcJ'] = 'look', F['QqZoD'] = function (l, o) {
      return l * o;
    }, F[m1(0x479, __sk_bO.F)] = 'search', F[m1(0x52f, 'W!#l')] = m1(0x46e, 'oizW');
    var b = F;
    switch (m) {
      case 'look':
        return LOOK_COUNT;
      case b[m1(0x36e, 'kJU#')]:
        return this['tasksToExecute'][m1(__sk_bO.b, 'v7OE')](b[m1(__sk_bO.l, __sk_bO.o)]) ? b['QqZoD'](LOOK_COUNT, FOLLOW_COUNT) : 0;
      case b['JECdt']:
        return SEARCH_COUNT;
      case m1(0x31c, __sk_bO.R):
        return this['tasksToExecute']['includes'](b['JECdt']) ? SEARCH_COUNT * SEARCH_FOLLOW_COUNT : 0;
      case m1(0x2c7, 'w%uJ'):
        return BOX_COUNT;
      case b['FtfoC']:
        return FOOD_COUNT;
      case 'kbox':
        return KBOX_COUNT;
      default:
        return 0;
    }
  }
  async [__sk_MI(0x333, 'i1Bf')]() {
    var __sk_bs = {
        M: 'bR&C',
        m: 0x202,
        F: 0xcb,
        b: 'U)4u',
        l: 0x40b,
        o: 'UzGo',
        R: 0x2bd,
        O: '0Bp5',
        V: 'pOZA',
        A: 0x553,
        K: 'bR&C',
        H: 0x2b5,
        C: 0xd7,
        s: '(%(I',
        h: '@xcw',
        J: 'C$jf'
      },
      m2 = __sk_MI,
      m = {};
    m['tgIGZ'] = function (o, R) {
      return o + R;
    }, m['UqNLJ'] = function (o, R) {
      return o + R;
    }, m['gKhAa'] = function (o, R) {
      return o + R;
    }, m[m2(0x2ca, __sk_bs.M)] = function (o, R) {
      return o + R;
    }, m[m2(__sk_bs.m, '$E2a')] = '⚠️\x20', m[m2(__sk_bs.F, 'dz)r')] = m2(0x134, __sk_bs.b), m['tBQcL'] = m2(__sk_bs.l, '*phW'), m[m2(0x2c8, __sk_bs.o)] = function (o, R) {
      return o + R;
    };
    var F = m;
    try {
      var b = await getAccountBasicInfo(this['getCookieWithCurrentDid'](), this[m2(__sk_bs.R, __sk_bs.O)], this[m2(0x459, __sk_bs.V)]);
      if (b && b[m2(__sk_bs.A, __sk_bs.K)]) {
        var l = parseInt(b['totalCoin']);
        if (l >= this['coinLimit']) return console[m2(0x4e5, 'zu(j')](F['tgIGZ'](F['UqNLJ'](F['gKhAa'](F['PEzhv'](F[m2(0x39a, 'oizW')] + this[m2(0x16d, '@xcw')](), F['tvfOP']), l), m2(0x156, '&ikq')), this[m2(__sk_bs.H, 'oizW')]) + F['tBQcL']), this[m2(__sk_bs.C, __sk_bs.K)] = true, this['stopAllTasks'] = true;
      }
      return false;
    } catch (o) {
      return console[m2(0x14e, __sk_bs.s)](F['gKhAa'](F[m2(0x83, __sk_bs.h)]('❌\x20', this['getAccountDisplayName']()) + m2(0x48e, '[sJs'), o[m2(0x1e5, __sk_bs.J)])), false;
    }
  }
  [__sk_MI(0x2d3, 'dz)r')]() {
    var __sk_be = {
        M: 0x320,
        m: 'xqZj',
        F: 0x248,
        b: 0xeb,
        l: 'duP7',
        o: 0x268,
        R: 'U@ca',
        O: 0x17d,
        V: 0x1c2,
        A: 'y[XJ',
        K: 0x29f,
        H: 'C$jf',
        C: 'bR&C',
        s: 0x1d1,
        h: 'ZDJd',
        J: 0x4c8,
        Y: 'E0S!',
        L: 0x593,
        d: 0x154,
        i: 0x438,
        N: 0x429,
        y: 0x13d,
        e: '[nEQ',
        j: 0x279,
        D: 0x633,
        t: 0x591,
        Q: 0x50b
      },
      __sk_by = {
        M: 0x1e4,
        m: 0x448,
        F: 0x32f,
        b: 0xb3,
        l: 0x51f,
        o: 0x267,
        R: 'bR&C',
        O: 0x157,
        V: 0x32b,
        A: 0x41c,
        K: '$aoM',
        H: 0x62f,
        C: 'duP7',
        s: 'tlkh',
        h: 0x2cb,
        J: '*phW',
        Y: 0x2f4,
        L: 0x4ef,
        d: 'LPN7',
        i: 0x2f1,
        N: 'W!#l'
      },
      m3 = __sk_MI,
      M = {
        'NEjYP': m3(__sk_be.M, __sk_be.m),
        'TlNdJ': function (b, l) {
          return b(l);
        },
        'IvcPk': 'wgPWR',
        'ngPcv': 'HJrvK',
        'UgncX': function (b, l) {
          return b === l;
        },
        'SyaAM': m3(__sk_be.F, '&fI4'),
        'IfWTM': function (b, l) {
          return b + l;
        },
        'WZbZj': function (b, l) {
          return b + l;
        },
        'aDBTu': 'appver',
        'jsslQ': 'egid',
        'eQWmD': 'did',
        'lQLzM': m3(__sk_be.b, '3Qg6'),
        'rZTNz': function (b, l) {
          return b < l;
        },
        'Voqlr': m3(0x46f, __sk_be.l),
        'inbfc': m3(__sk_be.o, __sk_be.R)
      };
    try {
      var m = (b, l = '') => {
          var m4 = m3,
            R = {
              'RlNSo': M['NEjYP'],
              'wDfrB': function (V, A) {
                return M['TlNdJ'](V, A);
              },
              'RUfkL': 'live_',
              'kbwMr': m4(__sk_by.M, 'U@ca')
            };
          if (M['IvcPk'] !== M['ngPcv']) {
            for (const V of [new RegExp('\x5cb' + b + '\x5cs*=\x5cs*\x22([^\x22]+)\x22', 'i'), new RegExp('\x5cb' + b + '\x5cs*=\x5cs*([^;]+)', 'i'), new RegExp('\x5cb' + b + m4(__sk_by.m, 'WQ!j'), 'i'), new RegExp('\x5cb' + b + '\x5cs*:\x5cs*([^,;]+)', 'i')]) {
              if (M[m4(__sk_by.F, 'C$jf')]('ZTtMk', M[m4(__sk_by.b, 'E0S!')])) {
                var O = this['cookie'][m4(0x3ed, '0Q4Q')](V);
                if (O && O[1] && O[1][m4(__sk_by.l, 'WQ!j')]()) return O[1]['trim']();
              } else {
                let K = J['adExtInfo'] || Y['extInfo'] || L?.['ad']?.['adExtInfo'] || '{}';
                if ('string' == typeof K) try {
                  K = T['parse'](K);
                } catch (h) {
                  K = {};
                }
                var H = [m4(__sk_by.o, __sk_by.R), m4(__sk_by.O, '2g$W'), m4(__sk_by.V, '@xcw'), R['RlNSo'], 'zb', 'ZB'],
                  C = i(N['creativeId'] || y?.['ad']?.['creativeId'] || '')[m4(__sk_by.A, 'E$*v')]();
                for (const J of [C, Z(K['description'] || '')['toLowerCase'](), G(K[m4(0x561, '$E2a')] || E['title'] || '')[m4(0x44c, __sk_by.K)](), R[m4(__sk_by.H, __sk_by.C)](J, K['caption'] || U[m4(0x407, __sk_by.s)] || '')[m4(0x21b, '&fI4')](), c[m4(__sk_by.h, '2g$W')](K || {})]) for (const Y of H) if (J && J['includes'](Y['toLowerCase']())) return true;
                return 60000 < (I['materialTime'] || W?.['ad']?.[m4(0x5e6, __sk_by.J)] || 0) ? true : !!(C['startsWith'](R['RUfkL']) || C['startsWith'](m4(0x634, __sk_by.C)) || C['startsWith']('live-') || C['startsWith'](m4(__sk_by.Y, 'kJU#')));
              }
            }
            return l;
          } else {
            var __sk_bN = {
                M: 0x143
              },
              H = {};
            H[m4(0x128, 'zu(j')] = m4(0x61c, '0Ddi');
            var C = H;
            try {
              return R[m4(__sk_by.L, __sk_by.d)] + (d => {
                var m5 = m4,
                  i = C['VXYJO'];
                let N = '';
                for (let y = 0; y < d; y++) N += i['charAt'](C[m5(__sk_bN.M, 'H]2s')](N['random']() * i['length']));
                return N;
              })(16);
            } catch (J) {
              return C[m4(__sk_by.i, 'E0S!')](m4(0x18d, '3Qg6') + J[m4(0x24e, __sk_by.N)]), 'ANDROID_' + s['now']()['toString'](16)['toUpperCase']()[m4(0x4dc, 'Q*e#')](0, 16);
            }
          }
        },
        F = (this[m3(__sk_be.O, '%N&g')] = m('egid'), this['did'] = m(m3(__sk_be.V, __sk_be.A)) || m(m3(__sk_be.K, __sk_be.H)), this['userId'] = m('ud') || M['TlNdJ'](m, 'userId') || '', this['kuaishouApiSt'] = M[m3(0x21f, __sk_be.C)](m, m3(__sk_be.s, '0Q4Q')) || '', this[m3(0x206, 'kJU#')] = m(M['aDBTu'], '13.8.40.10657'), []);
      this[m3(0x3a1, __sk_be.h)] || F['push'](M['jsslQ']), this['did'] || F['push'](M['eQWmD']), this['userId'] || F[m3(__sk_be.J, '8Hpe')](M['lQLzM']), this['kuaishouApiSt'] || F['push'](m3(0x4c6, 'i1Bf')), M['rZTNz'](0, F['length']) && console[m3(0x2f1, __sk_be.Y)]('⚠️\x20' + this[m3(__sk_be.L, '3Qg6')]() + M[m3(__sk_be.d, 'oizW')] + F['join'](',\x20'));
    } catch (b) {
      console[m3(__sk_be.i, '%N&g')](M[m3(__sk_be.N, '@xcw')]('❌\x20' + this['getAccountDisplayName']() + M['inbfc'], b[m3(__sk_be.y, __sk_be.e)]));
    }
  }
  ['getCookieWithCurrentDid']() {
    var __sk_bZ = {
        M: '3Qg6',
        m: 'U)4u'
      },
      __sk_bT = {
        M: 0x5a4,
        m: 'y[XJ',
        F: '%N&g',
        b: 0x25c,
        l: 0x1d5,
        o: 0x394
      },
      m6 = __sk_MI,
      M = {
        'zOArU': function (m, F) {
          return m + F;
        },
        'uldkd': function (m, F) {
          return m(F);
        },
        'wfLyl': function (m, F) {
          return m(F);
        },
        'zsRcv': function (m, F) {
          return m === F;
        },
        'SOsSn': function (m, F) {
          return m + F;
        },
        'xCQvx': function (m, F) {
          return m + F;
        },
        'mLogi': function (m, F) {
          return m !== F;
        },
        'vJjLD': 'SAJAZ'
      };
    try {
      if (M[m6(0x3b3, 'i1Bf')](M[m6(0x54d, '$aoM')], 'SAJAZ')) {
        var F;
        return (h = J(Y))['length'] >= L ? d[m6(0x641, 'duP7')](0, i) : (F = (N = y - e['length']) - (j = D['floor'](t / 2)), M['zOArU']('\x20'['repeat'](Q) + u, '\x20'[m6(0x1cd, __sk_bZ.M)](F)));
      } else return this['cookie'] && this['did'] ? String(this[m6(0x319, __sk_bZ.m)])[m6(0x3c3, 'E0S!')](';')['map'](F => {
        var m7 = m6,
          b,
          l,
          R,
          O,
          F = M[m7(__sk_bT.M, 'bR&C')](String, F ?? '');
        return F[m7(0x59a, __sk_bT.m)]() && (b = F['includes']('='), R = F['includes'](':'), b = b ? '=' : R ? ':' : null) && ([R, ...l] = F[m7(0x4e4, __sk_bT.F)](b), 'did' === (O = (R = M['wfLyl'](String, R || '')[m7(__sk_bT.b, 'duP7')]())['toLowerCase']()) || M[m7(__sk_bT.l, 'W8id')](m7(0x2d8, 'ZDJd'), O)) ? M['SOsSn'](M[m7(__sk_bT.o, 'M)%K')](R, b), this[m7(0x18e, 'U)4u')]) : F;
      })['join'](';\x20') : this['cookie'];
    } catch (F) {
      return this['cookie'];
    }
  }
  ['getTaskStats']() {
    var m8 = __sk_MI;
    return this[m8(0x296, 'tlkh')];
  }
  ['printTaskStats']() {
    var __sk_bU = {
        M: 0x583,
        m: '(%(I',
        F: '0Q4Q',
        b: 0x9e,
        l: 'v7OE',
        o: 0x2d2,
        R: '[sJs',
        O: 0xcd,
        V: 'WQ!j'
      },
      m9 = __sk_MI,
      m = {};
    m['xDhdP'] = '\x0a📊\x20', m['fqVcE'] = m9(__sk_bU.M, __sk_bU.m), m['DIpuQ'] = function (o, R) {
      return o + R;
    }, m['qLLQB'] = function (o, R) {
      return o + R;
    };
    var F = m;
    console['log'](F[m9(0x10e, 'tlkh')] + this[m9(0xf4, 'jO@S')]() + F[m9(0x398, __sk_bU.F)]);
    for (var [b, l] of Object['entries'](this['taskStats'])) {
      b = this['taskConfigs'][b], console['log'](F[m9(__sk_bU.b, 'LPN7')](F[m9(0x367, __sk_bU.l)]('\x20\x20' + b['name'] + m9(__sk_bU.o, '2vEu'), l[m9(0x613, 'pOZA')]) + m9(0x163, 'M)%K') + l['failed'], m9(0x4a7, 'E$*v')) + l[m9(0x4f7, __sk_bU.R)] + m9(__sk_bU.O, __sk_bU.V));
    }
  }
  async [__sk_MI(0xa8, 'bR&C')](m, F, b = 3, l = 2000) {
    var mM = __sk_MI,
      R = {};
    R[mM(0x215, '2vEu')] = function (H, C) {
      return H < C;
    };
    var O = R;
    let V = 0,
      A = null;
    for (; V < b;) {
      try {
        var K = await m();
        if (K) return K;
        A = new Error(F + mM(0x4da, '*phW'));
      } catch (H) {
        A = H;
      }
      O['OwpVF'](++V, b) && (await new Promise(C => setTimeout(C, l)));
    }
    return null;
  }
  async ['getAdInfo'](H, C = 0) {
    var __sk_l0 = {
        M: 'w%uJ',
        m: 0x37f,
        F: 0x4ab,
        b: '[nEQ',
        l: 0x239,
        o: 'W!#l',
        R: 0x345,
        O: '3Qg6',
        V: 0x1e7,
        A: '%N&g',
        K: 0x23e,
        H: 'w%uJ',
        C: 'nZ^k',
        s: 0x399,
        h: 0x47c,
        J: 'ZDJd',
        Y: 0x2d4,
        L: 'UzGo',
        d: 0x31e,
        i: 0x271,
        N: 0x273,
        y: 'v7OE',
        e: 0x466,
        j: 0x631,
        D: 0x2dd,
        t: 'yoVP',
        Q: 0x2d9,
        u: 0x5ea,
        I: '&fI4',
        W: '&ikq',
        T: 0x53c,
        Z: 0x1c1,
        G: 0x347,
        E: 0x1b5,
        a: 'NVQW',
        U: 'C$jf',
        c: 0x403,
        n: '$E2a',
        B: 0xa2,
        g: '2vEu',
        P: 'LPN7',
        q: '0Bp5',
        r: '(%(I',
        X: 'y[XJ',
        x: 0x299,
        S: '*phW',
        w: 0x5e4,
        p: 0x5dd,
        z: 0x623,
        k: 'jO@S',
        v: 0x5ec,
        f: '0Q4Q',
        M0: '*phW',
        M1: 0x4d8,
        M2: 'y[XJ',
        M3: 0x1dd,
        M4: 'jO@S',
        M5: 0x438,
        M6: 0x567,
        M7: '0Ddi',
        M8: 'M)%K',
        M9: 0x4a0,
        MM: '$aoM',
        Mm: 'E$*v',
        MF: '$E2a',
        Mb: 0x2ff,
        Ml: 0x1ee,
        Mo: 0x2a1,
        MR: 'jO@S',
        MO: 0x223,
        MV: 0x4c2,
        MA: 'duP7',
        MK: 0x5ae,
        MH: 'tlkh',
        MC: 0x44d,
        Ms: 'LPN7',
        Mh: 0x575,
        MJ: 'dz)r',
        MY: 0x1a1,
        ML: 0x53a,
        Md: 0x5cf,
        Mi: 'w%uJ',
        MN: 0x146,
        My: '[sJs',
        Me: 0x8a,
        Mj: 0x1ed,
        MD: 0x576,
        Mt: 0x572
      },
      mm = __sk_MI,
      J = {
        'Duitb': function (v, f) {
          return v(f);
        },
        'IWFYu': function (v, f) {
          return v <= f;
        },
        'EPaYo': function (v, f) {
          return v === f;
        },
        'OBUks': function (v, f) {
          return v + f;
        },
        'qzynG': function (v, f) {
          return v || f;
        },
        'ZzEpC': function (v, f, M0, M1) {
          return v(f, M0, M1);
        },
        'bzLcq': mm(0x348, __sk_l0.M),
        'EclsL': mm(__sk_l0.m, '2g$W'),
        'LYmLe': '|sign|',
        'vIuJo': mm(0x2f7, '2g$W'),
        'ejwsC': '1_23',
        'HAwlp': 'zh-cn',
        'twNHv': 'WIFI',
        'bYHQT': '1599',
        'jARLM': '11.6',
        'dOwYD': mm(__sk_l0.F, __sk_l0.b),
        'fVsDj': 'NEBULA',
        'Mlfnz': '2400',
        'sPfpB': mm(__sk_l0.l, __sk_l0.o),
        'bmwNX': mm(__sk_l0.R, 'dz)r'),
        'OyPSu': function (v, f) {
          return v === f;
        },
        'qwkHT': 'ETtcw',
        'TIGKK': 'xUtHg',
        'PtqRC': mm(0xc4, 'dz)r'),
        'akUNO': 'application/x-www-form-urlencoded;\x20charset=UTF-8',
        'FoGrH': 'ghwmh',
        'AckwE': function (v, f) {
          return v(f);
        },
        'xCBxk': function (v, f) {
          return v === f;
        },
        'BFMsM': '🔍\x20',
        'GVMhj': mm(0x417, __sk_l0.O),
        'Skkgh': mm(__sk_l0.V, __sk_l0.A),
        'YNXsy': '\x20\x20-\x20inspirePersonalize:',
        'xXjmw': '\x20\x20-\x20inspireAdInfo:',
        'NhLkw': mm(0xd2, 'M)%K'),
        'zqQyg': function (v, f) {
          return v + f;
        },
        'nthDd': 'ℹ️\x20',
        'TorsN': mm(__sk_l0.K, __sk_l0.H),
        'BFhLV': function (v, f) {
          return v || f;
        },
        'VxbIK': mm(0x288, __sk_l0.C),
        'ctKnf': mm(__sk_l0.s, '%N&g'),
        'FzTWI': mm(__sk_l0.h, 'LPN7'),
        'DSMZQ': function (v, f) {
          return v + f;
        },
        'lIsEu': function (v, f) {
          return v + f;
        },
        'SgcyJ': mm(0x451, 'duP7'),
        'tpdaf': mm(0x2b2, __sk_l0.J),
        'VpKuf': function (v, f) {
          return v + f;
        },
        'JLJLZ': mm(__sk_l0.Y, '[nEQ'),
        'MYQfA': mm(0x462, __sk_l0.L)
      };
    try {
      SKIP_LIVE_ADS && SKIP_LIVE_MAX_RETRIES;
      var Y,
        L = J['EclsL'],
        N = {
          'encData': mm(__sk_l0.d, 'oizW'),
          'sign': J['LYmLe'],
          'cs': 'false',
          'client_key': J['vIuJo'],
          'videoModelCrowdTag': J['ejwsC'],
          'os': 'android',
          'kuaishou.api_st': this[mm(__sk_l0.i, 'M)%K')],
          'uQaTag': mm(0x214, '[sJs')
        },
        y = {
          'earphoneMode': '1',
          'mod': 'Xiaomi(23116PN5BC)',
          'appver': this['appver'],
          'isp': 'CUCC',
          'language': J[mm(0x137, 'UzGo')],
          'ud': this[mm(__sk_l0.N, __sk_l0.y)] || '',
          'did_tag': '0',
          'net': J['twNHv'],
          'kcv': J['bYHQT'],
          'app': '0',
          'kpf': mm(0x1d9, 'NVQW'),
          'ver': J[mm(__sk_l0.e, __sk_l0.y)],
          'android_os': '0',
          'boardPlatform': J[mm(__sk_l0.j, '&ikq')],
          'kpn': J[mm(0x4ec, '(%(I')],
          'androidApiLevel': '35',
          'country_code': 'cn',
          'sys': mm(__sk_l0.D, __sk_l0.t),
          'sw': '1080',
          'sh': J[mm(__sk_l0.Q, '2vEu')],
          'abi': 'arm64',
          'userRecoBit': '0'
        };
      let v = '{}';
      7076 === H['businessId'] && (Y = {
        'openH5AdCount': 0x0,
        'sessionLookedCompletedCount': this['adAdditionalNum'],
        'sessionType': J['EPaYo'](2, H[mm(0x265, 'Q*e#')]) ? '2' : '1',
        'searchKey': J['sPfpB'],
        'triggerType': '2',
        'disableReportToast': true,
        'businessEnterAction': '7',
        'neoParams': ''
      }, v = JSON['stringify'](Y));
      var j = {};
      j[mm(__sk_l0.u, 'w%uJ')] = J[mm(0x3db, __sk_l0.I)], j['name'] = mm(0x4d6, __sk_l0.W), j['packageName'] = 'com.kuaishou.nebula', j['version'] = this['appver'], j[mm(__sk_l0.T, 'W!#l')] = -1;
      var D = {};
      D['width'] = 0x438, D[mm(0x3b7, '&fI4')] = 0x8c9;
      var Q = {};
      Q[mm(0x5fa, 'tlkh')] = 0x1, Q[mm(0x245, 'duP7')] = '15', Q[mm(__sk_l0.Z, '&ikq')] = this['did'], Q[mm(0x460, '$E2a')] = D, Q['ftt'] = '';
      var I = {};
      I[mm(__sk_l0.G, 'i1Bf')] = this['userId'] || '', I[mm(__sk_l0.E, '2vEu')] = 0x0, I['gender'] = '';
      var W = {};
      W['pageId'] = H['pageId'] || 11101, W['subPageId'] = H[mm(0x8e, '%N&g')], W[mm(0x5cc, '2g$W')] = 0x0, W['browseType'] = 0x3, W[mm(0x50c, __sk_l0.a)] = v, W[mm(0x5e2, __sk_l0.U)] = '{}';
      var T = {};
      T[mm(__sk_l0.c, __sk_l0.n)] = j, T['deviceInfo'] = Q, T['userInfo'] = I, T['impInfo'] = [W];
      var Z = T,
        G = Buffer[mm(__sk_l0.B, __sk_l0.b)](JSON['stringify'](Z))['toString']('base64'),
        E = await this['getSign'](G);
      if (E) {
        if (J[mm(0x570, __sk_l0.g)](mm(0x5bd, __sk_l0.P), J['qwkHT'])) {
          N['encData'] = E['encdata'], N['sign'] = E['sign'];
          var U = await this['requestSignService']({
            'urlpath': L,
            'reqdata': J['OBUks'](J[mm(0x246, '2g$W')](querystring['stringify'](N), '&'), querystring[mm(0x3b0, __sk_l0.q)](y)),
            'api_client_salt': this[mm(0x230, '8Hpe')]
          }, mm(0x513, __sk_l0.r));
          if (U) {
            if (J['TIGKK'] !== mm(0x30d, __sk_l0.X)) {
              var B = {
                ...y
              };
              B['sig'] = U['sig'], B[mm(0x45d, 'dz)r')] = U['__NS_sig3'], B[mm(__sk_l0.x, '0Bp5')] = U['__NS_xfalcon'] || '', B['__NStokensig'] = U[mm(0x170, 'pOZA')];
              var P = B,
                {
                  response: q,
                  body: X
                } = await J['ZzEpC'](sendRequest, {
                  'method': mm(0x27b, 'UzGo'),
                  'url': J['PtqRC'] + API_CONFIG['AD_API_DOMAIN'] + L + '?' + querystring[mm(0x298, __sk_l0.S)](P),
                  'headers': {
                    'Content-Type': J['akUNO'],
                    'Host': API_CONFIG['AD_API_DOMAIN'],
                    'User-Agent': mm(0x104, 'U)4u'),
                    'Cookie': J['OBUks']('kuaishou.api_st=', this[mm(0x27d, '&fI4')])
                  },
                  'form': N,
                  'timeout': 0x7530
                }, this[mm(0x63e, '0Ddi')], this['getAccountDisplayName']() + mm(__sk_l0.w, 'v7OE'));
              if (q && X) {
                if ('OK' === X['errorMsg'] && X['feeds'] && X['feeds'][0] && X[mm(0x33a, 'Q*e#')][0]['ad']) {
                  if (J['FoGrH'] === 'ghwmh') {
                    var x = X['feeds'][0],
                      S = x?.['ad']?.['creativeId'] ?? x?.['creativeId'],
                      w = (x['exp_tag'] || '')['split']('/')[1]?.['split']('_')?.[0] || '';
                    if (!S) return console['log'](J['OBUks']('⚠️\x20' + this['getAccountDisplayName'](), mm(0x231, '3Qg6'))), null;
                    var z = (x['caption'] || x?.['ad']?.['caption'] || '')['slice'](0, 30);
                    let f = 0;
                    try {
                      x['ad']?.['adDataV2']?.['inspirePersonalize']?.['awardValue'] ? f = parseInt(x['ad'][mm(__sk_l0.p, '&ikq')][mm(__sk_l0.z, __sk_l0.k)]['awardValue']) || 0 : x['ad']?.['awardCoin'] ? f = parseInt(x['ad'][mm(__sk_l0.v, 'y[XJ')]) || 0 : x['ad']?.[mm(0x39d, 'W8id')]?.['inspireAdInfo']?.[mm(0x55d, 'frRp')] && (f = J[mm(0x27c, '[sJs')](parseInt, x['ad']['adDataV2']['inspireAdInfo'][mm(0x101, __sk_l0.f)]) || 0), J['xCBxk'](0, f) && (console['log'](J['OBUks'](J[mm(0x612, __sk_l0.M0)], this['getAccountDisplayName']()) + J['GVMhj']), console['log']('\x20\x20-\x20adData.ad.adDataV2:', x['ad']?.[mm(__sk_l0.M1, __sk_l0.M2)] ? mm(0x2e2, 'jO@S') : mm(__sk_l0.M3, __sk_l0.M4)), console[mm(__sk_l0.M5, '%N&g')]('\x20\x20-\x20adData.ad.awardCoin:', x['ad']?.['awardCoin'] || J['Skkgh']), x['ad']?.[mm(__sk_l0.M6, '(%(I')]) && (console[mm(0xdf, __sk_l0.M7)](J['YNXsy'], x['ad'][mm(0x63f, __sk_l0.M8)][mm(__sk_l0.M9, __sk_l0.MM)] ? mm(0x1d0, 'pOZA') : mm(0x464, 'pOZA')), console[mm(0x2f1, 'E0S!')](J['xXjmw'], x['ad'][mm(0x328, __sk_l0.Mm)][mm(0x5c5, 'NQY)')] ? mm(0x4de, 'oizW') : mm(0x4df, __sk_l0.MF)));
                    } catch (M0) {
                      console['log'](J[mm(__sk_l0.Mb, 'zu(j')]('❌\x20', this['getAccountDisplayName']()) + J['NhLkw'] + M0[mm(__sk_l0.Ml, '2vEu')]);
                    }
                    var k = {};
                    return k[mm(0x59b, 'W8id')] = S, k['llsid'] = w, 0 < f ? console['log'](J['zqQyg'](J['nthDd'], this[mm(0x357, 'frRp')]()) + J['TorsN'] + J['BFhLV'](z, mm(__sk_l0.Mo, '8Hpe')) + J['VxbIK'] + f + J['ctKnf']) : console['log'](J['zqQyg'](J['OBUks'](J['nthDd'], this[mm(0x46b, 'bR&C')]()) + J['TorsN'] + J['qzynG'](z, J['FzTWI']), mm(0x5fe, __sk_l0.MR))), k;
                  } else {
                    var M2,
                      M3,
                      M4,
                      M5 = J['Duitb'](E, M5 || '')['trim']();
                    if (!M5) return null;
                    let M6 = '',
                      M7 = '',
                      M8 = '',
                      M9 = null;
                    return M5['includes']('#') ? 2 === (M2 = M5['split']('#'))[mm(__sk_l0.MO, 'zu(j')] ? (M8 = M2[0], M7 = M2[1]) : 3 === M2['length'] ? /^socks5:\/\//i[mm(0x499, 'jO@S')](M2[2]) || M2[2][mm(__sk_l0.MV, 'tlkh')]('|') ? (M8 = M2[0], M7 = M2[1], M9 = M2[2]) : (M6 = M2[0], M8 = M2[1], M7 = M2[2]) : J['IWFYu'](4, M2['length']) && (M6 = M2[0], M8 = M2[1], M7 = M2['slice'](2, M2['length'] - 1)['join']('#'), M9 = M2[M2[mm(0x27f, __sk_l0.MA)] - 1]) : (M8 = M5, M7 = ''), M9 && (J['EPaYo']('0', M9) || 'none' === M9['toLowerCase']() ? M9 = null : M9['includes']('|') && (M2 = M9[mm(__sk_l0.MK, 'E$*v')]('|'), M9 = 2 <= M2['length'] && ([M5, M2, M3 = '', M4 = ''] = M2['map'](MM => M2(MM || '')['trim']()), M5) && M2 ? J['OBUks'](J['OBUks']('socks5://' + (M3 || M4 ? J[mm(0x183, __sk_l0.M2)](J['OBUks'](J['Duitb'](I, M3) + ':', J[mm(0x100, 'ZDJd')](W, M4)), '@') : ''), M5), ':') + M2 : null)), {
                      'remark': J[mm(0x502, __sk_l0.MH)](M6, ''),
                      'salt': M7,
                      'cookie': M8,
                      'proxyUrl': M9
                    };
                  }
                }
                console[mm(0x422, __sk_l0.b)](J['DSMZQ'](J['lIsEu']('❌\x20' + this['getAccountDisplayName']() + J['SgcyJ'], X?.[mm(0x36a, 'xqZj')] || mm(0x54f, 'dz)r')), ',\x20result=') + (X?.['result'] || J[mm(__sk_l0.MC, 'xqZj')]));
              } else console['log'](J[mm(0x3f5, 'kJU#')](J[mm(0x326, __sk_l0.Ms)]('❌\x20', this['getAccountDisplayName']()), mm(__sk_l0.Mh, __sk_l0.MJ)));
            } else {
              for (const M4 of [new X('\x5cb' + y + '\x5cs*=\x5cs*\x22([^\x22]+)\x22', 'i'), new N('\x5cb' + y + mm(__sk_l0.MY, '[sJs'), 'i'), new v('\x5cb' + j + '\x5cs*:\x5cs*\x22([^\x22]+)\x22', 'i'), new D('\x5cb' + H + '\x5cs*:\x5cs*([^,;]+)', 'i')]) {
                var M3 = this[mm(__sk_l0.ML, '[nEQ')][mm(0xf5, '$E2a')](M4);
                if (M3 && M3[1] && M3[1][mm(__sk_l0.Md, __sk_l0.Mi)]()) return M3[1][mm(__sk_l0.MN, __sk_l0.My)]();
              }
              return L;
            }
          } else console['log']('❌\x20' + this[mm(__sk_l0.Me, '2g$W')]() + mm(__sk_l0.Mj, 'NQY)'));
        } else {
          const M4 = J[mm(__sk_l0.MD, 'oizW')](E, M4['index'], null, I['remark']);
          W[mm(__sk_l0.Mt, __sk_l0.g)](mm(0x4f5, 'W!#l') + M4 + J[mm(0x3a3, '@xcw')]);
        }
      } else console[mm(0x3c6, '$aoM')](J['VpKuf']('❌\x20', this['getAccountDisplayName']()) + J['JLJLZ']);
      return null;
    } catch (M4) {
      return console['log']('❌\x20' + this['getAccountDisplayName']() + J['MYQfA'] + M4['message']), null;
    }
  }
  async ['generateSignature'](F, b, l, R) {
    var __sk_l4 = {
        M: 0x115,
        m: 'xqZj',
        F: 'bR&C',
        b: 0x545,
        l: 0x1fc,
        o: 0x5c9,
        R: 'jO@S',
        O: 0x615,
        V: 0x408,
        A: 0x55f,
        K: 0x107,
        H: 0x355,
        C: 0x57b,
        s: '8Hpe',
        h: 0x526,
        J: 'NQY)',
        Y: 0x3e0,
        L: 'nZ^k',
        d: 0x58a,
        i: '(%(I',
        N: '2g$W',
        y: '&ikq',
        e: 'ZDJd',
        j: 'WQ!j',
        D: 'i1Bf',
        t: 0x1c9,
        Q: 0x1ce,
        u: 'oizW'
      },
      mF = __sk_MI,
      O = {};
    O[mF(__sk_l4.M, __sk_l4.m)] = '&cs=false&client_key=2ac2a76d&kuaishou.api_st=', O[mF(0x4fd, __sk_l4.F)] = function (J, Y) {
      return J + Y;
    }, O[mF(__sk_l4.b, '3Qg6')] = function (J, Y) {
      return J !== Y;
    }, O['TmJzL'] = function (J, Y) {
      return J + Y;
    };
    var V = O;
    try {
      var A = {};
      A[mF(__sk_l4.l, 'C$jf')] = R['businessId'], A[mF(__sk_l4.o, __sk_l4.R)] = this[mF(__sk_l4.O, 'xqZj')], A[mF(0xa1, 'NVQW')] = '', A['mediaScene'] = 'video', A['neoInfos'] = [{
        'creativeId': F,
        'extInfo': '',
        'llsid': b,
        'requestSceneType': R[mF(__sk_l4.V, 'tlkh')],
        'taskType': R[mF(__sk_l4.A, '@xcw')],
        'watchExpId': '',
        'watchStage': 0x0
      }], A['pageId'] = R['pageId'] || 11101, A['posId'] = R['posId'], A[mF(__sk_l4.K, 'U)4u')] = 0x0, A[mF(__sk_l4.H, 'W8id')] = '', A['startTime'] = this[mF(__sk_l4.C, __sk_l4.s)], A[mF(0xa7, 'zu(j')] = R[mF(__sk_l4.h, __sk_l4.J)];
      var K = A,
        H = 'bizStr=' + encodeURIComponent(JSON['stringify'](K)) + V['nSira'] + this[mF(__sk_l4.Y, __sk_l4.L)],
        C = V[mF(__sk_l4.d, __sk_l4.i)](this['queryParams'], '&') + H,
        h = await this['requestSignService']({
          'urlpath': this['taskReportPath'],
          'reqdata': C,
          'api_client_salt': this[mF(0x1db, __sk_l4.N)]
        }, this[mF(0x286, __sk_l4.y)]() + mF(0x506, __sk_l4.e));
      return h ? {
        'sig': h['sig'],
        'sig3': h[mF(0x9b, __sk_l4.j)],
        'sigtoken': h[mF(0x61d, 'i1Bf')],
        'xfalcon': h[mF(0x552, __sk_l4.D)] || '',
        'post': H
      } : null;
    } catch (J) {
      return V['vymLd']('VDoZC', 'IRxfm') ? (console['log'](V['TmJzL'](V[mF(__sk_l4.t, 'i1Bf')]('❌\x20', this['getAccountDisplayName']()) + mF(0x587, __sk_l4.j), J[mF(__sk_l4.Q, __sk_l4.u)])), null) : this['taskStats'];
    }
  }
  async ['submitReport'](R, O, V, A, K, H, C) {
    var __sk_lA = {
        M: 'oizW',
        m: 'W8id',
        F: 0x3d7,
        b: 0xaa,
        l: 'zu(j',
        o: 0x269,
        R: 'nZ^k',
        O: 'frRp',
        V: 0x5c8,
        A: 0x110,
        K: '0Q4Q',
        H: 'NQY)',
        C: '3Qg6',
        s: 0x441,
        h: '[nEQ',
        J: 0x3f8,
        Y: '0Bp5',
        L: 0x201,
        d: 'M)%K',
        i: 0x274,
        N: 0x275,
        y: 0x43a,
        e: '@xcw',
        j: 0x600,
        D: 0x25d,
        t: '$aoM',
        Q: '(%(I',
        u: 'H]2s',
        I: 0xa4,
        W: 0x122,
        T: '$aoM',
        Z: 0x33b,
        G: '8Hpe',
        E: 0x48f,
        a: 'E$*v',
        U: 0x169,
        c: 0x538,
        n: 0xe9,
        B: '$aoM',
        g: 0x5fb,
        P: 0x325,
        q: 'M)%K'
      },
      mb = __sk_MI,
      h = {};
    h[mb(0x624, 'xqZj')] = function (D, Q) {
      return D + Q;
    }, h['WhyjR'] = function (D, Q) {
      return D + Q;
    }, h['xZLbA'] = function (D, Q) {
      return D + Q;
    }, h[mb(0x8c, __sk_lA.M)] = mb(0x28b, 'frRp'), h['wUpUe'] = '&__NS_xfalcon=', h[mb(0x1a2, '%N&g')] = '&__NStokensig=', h['pTIoe'] = mb(0x159, __sk_lA.m), h[mb(0x5f1, '2vEu')] = function (D, Q) {
      return D + Q;
    }, h['TsRvx'] = function (D, Q) {
      return D + Q;
    }, h['RUjBv'] = function (D, Q) {
      return D + Q;
    }, h['YkBTg'] = mb(__sk_lA.F, 'UzGo'), h[mb(__sk_lA.b, '$aoM')] = function (D, Q) {
      return D + Q;
    }, h[mb(0x10d, 'ZDJd')] = mb(0x3c0, __sk_lA.l), h[mb(__sk_lA.o, __sk_lA.R)] = 'body', h[mb(0x478, 'Q*e#')] = function (D, Q) {
      return D === Q;
    }, h['vMwbv'] = function (D, Q) {
      return D + Q;
    }, h['DwFjg'] = function (D, Q) {
      return D + Q;
    }, h['TJfci'] = '&egid=', h[mb(0x4ea, '&fI4')] = '🔄\x20', h[mb(0x47e, 'LPN7')] = mb(0x4b4, __sk_lA.O), h['jiaKE'] = '⚠️\x20', h['YItkg'] = mb(__sk_lA.V, '&ikq'), h['NBPCs'] = function (D, Q) {
      return D + Q;
    }, h['MRNQv'] = function (D, Q) {
      return D + Q;
    }, h['IjEUh'] = mb(__sk_lA.A, __sk_lA.K), h['VqDfr'] = mb(0x3ea, __sk_lA.H), h[mb(0x416, __sk_lA.C)] = mb(0xe7, 'tlkh'), h['kKyAj'] = mb(__sk_lA.s, '[sJs'), h[mb(0x19b, __sk_lA.h)] = function (D, Q) {
      return D < Q;
    };
    var J = h;
    try {
      var Y,
        L,
        d = (await sendRequest({
          'method': mb(__sk_lA.J, 'LPN7'),
          'url': J[mb(0x3c5, 'y[XJ')](J[mb(0x26a, __sk_lA.Y)](J['xZLbA']('https://' + API_CONFIG[mb(__sk_lA.L, 'W8id')], this['taskReportPath']) + '?', this['queryParams']) + '&sig=' + R + J['etmaz'] + O + J['wUpUe'] + (A || ''), J[mb(0x3a2, __sk_lA.d)]) + V,
          'headers': {
            'Content-Type': 'application/x-www-form-urlencoded;\x20charset=UTF-8',
            'Host': API_CONFIG[mb(0x1fd, 'Q*e#')],
            'User-Agent': J['pTIoe']
          },
          'body': K,
          'timeout': 0x2ee0
        }, this['proxyUrl'], J['KhLGU'](J['TsRvx'](J[mb(0x2b9, 'nZ^k')](J[mb(0x35a, '[sJs')] + this['nickname'], ']'), this['remark'] ? J['KhLGU'](J[mb(__sk_lA.i, 'zu(j')]('（', this[mb(0x19d, 'frRp')]), '）') : ''), J['KcjCy'])))[J[mb(__sk_lA.N, 'xqZj')]],
        N = {};
      N[mb(0xd3, 'bR&C')] = false, N['reward'] = 0x0;
      var y = {};
      return y[mb(__sk_lA.y, __sk_lA.e)] = false, y['reward'] = 0x0, y['shouldRetry'] = false, d ? J['iggfM'](1, d['result']) ? (Y = d['data']?.['neoAmount'] || 0) <= this['lowRewardThreshold'] ? (this['lowRewardStreak']++, ENABLE_CHANGE_DID ? (L = generateDeviceId(), this[mb(0x4c5, '*phW')] = L, this['queryParams'] = J[mb(__sk_lA.j, '3Qg6')](J[mb(0x370, __sk_lA.H)]('mod=Xiaomi(MI\x2011)&appver=', this['appver']) + J['TJfci'], this['egid']) + '&did=' + L, console['log'](J['xZLbA'](J['LpCXE'], this[mb(0x103, __sk_lA.R)]()) + J[mb(__sk_lA.D, __sk_lA.t)] + L)) : console['log'](mb(0x148, __sk_lA.Q) + this[mb(0x5fd, __sk_lA.u)]() + mb(__sk_lA.I, '[nEQ')), console['log'](J['TsRvx'](J[mb(__sk_lA.W, __sk_lA.l)](J['RUjBv'](J[mb(0x581, 'yoVP')](J['WhyjR'](J[mb(0x516, 'oizW')](J[mb(0x30f, __sk_lA.T)], this[mb(__sk_lA.Z, '$aoM')]()), mb(0x63c, '0Q4Q')), Y), mb(0x563, '$aoM')) + this['lowRewardThreshold'] + J['YItkg'], this['lowRewardStreak']), ')/') + this['lowRewardLimit']), await new Promise(D => setTimeout(D, 30000)), this[mb(0x2ed, __sk_lA.G)] >= this['lowRewardLimit'] ? (console['log'](J[mb(__sk_lA.E, __sk_lA.a)](J['xZLbA'](J['MRNQv'](J[mb(__sk_lA.U, '$aoM')] + this['getAccountDisplayName'](), J['VqDfr']) + this['lowRewardLimit'], J['HoxDD']), this['lowRewardThreshold']) + J['kKyAj']), {
        'success': !(this['stopAllTasks'] = true),
        'reward': 0x0,
        'shouldRetry': false
      }) : {
        'success': true,
        'reward': Y,
        'shouldRetry': false
      }) : (J['hXEWG'](0, this[mb(__sk_lA.c, 'kJU#')]) && (console['log']('✅\x20' + this[mb(0x222, 'W8id')]() + mb(__sk_lA.n, __sk_lA.B) + Y + mb(0x592, '&ikq')), this['lowRewardStreak'] = 0), {
        'success': true,
        'reward': Y,
        'shouldRetry': false
      }) : [20107, 20108, 1003, 415]['includes'](d['result']) ? (console['log'](J['jvkFY'](J[mb(__sk_lA.g, 'dz)r')], this[mb(__sk_lA.P, 'WQ!j')]()) + '\x20' + C['name'] + mb(0x190, __sk_lA.Q)), {
        'success': !(this['taskLimitReached'][H] = true),
        'reward': 0x0
      }) : N : y;
    } catch (D) {
      if (mb(0x238, 'W!#l') !== 'BqMgO') {
        var j = {};
        return j[mb(0x356, __sk_lA.q)] = false, j[mb(0x287, '8Hpe')] = 0x0, j;
      } else {
        var __sk_lV = {
          M: 0x397,
          m: 'E0S!'
        };
        return 'ANDROID_' + (u => {
          var ml = mb,
            I = '0123456789abcdef';
          let W = '';
          for (let T = 0; T < u; T++) W += I[ml(0x295, 'xqZj')](R['floor'](O[ml(__sk_lV.M, __sk_lV.m)]() * I[ml(0x4fe, '0Q4Q')]));
          return W;
        })(16);
      }
    }
  }
  async ['getSign'](m) {
    var __sk_lt = {
        M: 0x5e5,
        m: 0x232,
        F: 'W8id',
        b: 'xqZj',
        l: 'y[XJ',
        o: 'M)%K',
        R: '3Qg6',
        O: '$aoM',
        V: 0x389,
        A: '0Ddi',
        K: 0x453,
        H: 'E0S!',
        C: 'NVQW',
        s: 0xf0,
        h: 'xqZj',
        J: 0x332,
        Y: 0x27a,
        L: 0xc6,
        d: 'W8id',
        i: 'U@ca',
        N: 0x5aa,
        y: 0x40d,
        e: '%N&g',
        j: '2g$W',
        D: 0x428,
        t: '$aoM',
        Q: 0x105,
        u: 'v7OE',
        I: 'NVQW'
      },
      mo = __sk_MI,
      F = {
        'pidvp': function (V, A) {
          return V + A;
        },
        'kyryY': function (V, A) {
          return V + A;
        },
        'CGuvz': function (V, A) {
          return V + A;
        },
        'fSCGK': mo(0x1da, 'W8id'),
        'diiHH': '0123456789abcdef',
        'VFLmS': function (V, A) {
          return V < A;
        },
        'xUuay': function (V, A) {
          return V * A;
        },
        'BURLK': function (V, A, K) {
          return V(A, K);
        },
        'zummI': function (V, A) {
          return V !== A;
        },
        'AyEar': 'ZbbKI',
        'wljHg': mo(__sk_lt.M, '%N&g'),
        'Mpuzj': 'POST',
        'PmlaC': mo(__sk_lt.m, __sk_lt.F),
        'NjzHt': function (V, A) {
          return V + A;
        },
        'VKgnx': mo(0x1ae, __sk_lt.b),
        'CSzqX': mo(0xbf, 'kJU#'),
        'iljAk': 'ykgTN',
        'FVnay': 'CnUeM',
        'uLbwS': function (V, A) {
          return V + A;
        },
        'JnFWC': function (V, A) {
          return V <= A;
        },
        'AoyqJ': function (V, A) {
          return V + A;
        },
        'NUUoN': mo(0x437, '&fI4'),
        'eNsUH': function (V, A) {
          return V <= A;
        },
        'tsMeU': function (V, A) {
          return V + A;
        },
        'Ihiji': function (V, A) {
          return V < A;
        },
        'FJgXA': function (V, A) {
          return V + A;
        }
      };
    for (let V = 1; V <= 3; V++) {
      if (F['zummI'](F['AyEar'], F['wljHg'])) {
        try {
          var b = {};
          b[mo(0x2fc, __sk_lt.l)] = mo(0x199, __sk_lt.o), b['data'] = m, b['ud'] = this['userId'] || '', b[mo(0x8d, __sk_lt.R)] = SCRIPT_VERSION;
          var l = JSON[mo(0x4ed, __sk_lt.O)](b),
            {
              response: R,
              body: O
            } = await sendRequest({
              'method': F[mo(__sk_lt.V, __sk_lt.A)],
              'url': API_CONFIG['toudeshishabi'] + '/encsign',
              'body': l,
              'headers': {
                'Content-Type': F[mo(0x29c, 'E0S!')]
              },
              'timeout': 0x7530
            }, this['proxyUrl'], F['NjzHt'](this['getAccountDisplayName'](), F[mo(__sk_lt.K, __sk_lt.O)]));
          if (R) {
            if (O) {
              if (F['CSzqX'] === F['iljAk']) O = this['taskConfigs'][V], A['log'](F[mo(0x131, '8Hpe')](F[mo(0x535, 'C$jf')](F[mo(0x5ed, '0Q4Q')]('\x20\x20', K['name']) + mo(0x164, 'i1Bf') + H['success'] + mo(0x3af, __sk_lt.H), C['failed']) + F['fSCGK'] + R[mo(0x490, __sk_lt.C)], mo(__sk_lt.s, 'nZ^k')));else {
                if (O && O['status']) {
                  let K = O['data'];
                  if ('string' == typeof K) try {
                    if (F['FVnay'] === 'dhSzY') {
                      var C = F[mo(0x18f, __sk_lt.h)];
                      let h = '';
                      for (let J = 0; F['VFLmS'](J, R); J++) h += C['charAt'](O[mo(__sk_lt.J, 'duP7')](F['xUuay'](V[mo(0x397, 'E0S!')](), C['length'])));
                      return h;
                    } else K = JSON['parse'](K);
                  } catch (C) {
                    if (V < 3) continue;
                    return console['log'](F['uLbwS']('❌\x20', this['getAccountDisplayName']()) + mo(__sk_lt.Y, '[sJs')), null;
                  }
                  return K;
                }
                if (F[mo(0xdd, 'y[XJ')](3, V) && console[mo(__sk_lt.L, __sk_lt.d)](F['AoyqJ']('❌\x20', this['getAccountDisplayName']()) + F[mo(0x32a, __sk_lt.i)] + (O?.[mo(__sk_lt.N, 'U@ca')] || mo(__sk_lt.y, 'UzGo'))), V < 3) continue;
              }
            } else {
              if (F['VFLmS'](V, 3)) continue;
              console['log'](F['uLbwS']('❌\x20', this['getAccountDisplayName']()) + mo(0x2e0, '0Bp5'));
            }
          } else {
            if (V < 3) continue;
            console['log']('❌\x20' + this[mo(0x4a4, __sk_lt.e)]() + mo(0x373, __sk_lt.j));
          }
          return null;
        } catch (h) {
          if (F['eNsUH'](3, V) && console['log'](F['tsMeU']('❌\x20', this[mo(0x40c, 'oizW')]()) + mo(__sk_lt.D, 'kJU#') + h['message']), F[mo(0x594, __sk_lt.t)](V, 3)) continue;
          return null;
        }
        V < 3 && (await new Promise(J => setTimeout(J, 2000)));
      } else return F['BURLK'](l, '[' + l + ']', R[mo(0x280, '8Hpe')] + O[mo(__sk_lt.Q, __sk_lt.u)]);
    }
    return console['log'](F['FJgXA']('❌\x20', this['getAccountDisplayName']()) + mo(0x233, __sk_lt.I)), null;
  }
  async ['requestSignService'](m, F) {
    var __sk_lS = {
        M: 0x1c7,
        m: 0x353,
        F: 0x364,
        b: 'y[XJ',
        l: 0x5c0,
        o: 0x15e,
        R: '3Qg6',
        O: 0x2cd,
        V: 0x343,
        A: 0x322,
        K: 'NVQW',
        H: 0xa0,
        C: '&fI4',
        s: 0x2bf,
        h: 0x4a1,
        J: 0x5cb,
        Y: 0x5cb,
        L: '0Bp5',
        d: 'yoVP',
        i: 0x34d,
        N: '(%(I',
        y: 0x5ab,
        e: 'tlkh',
        j: 'U@ca',
        D: 0x2ba,
        t: '0Bp5',
        Q: 0x3bd,
        u: 0x578,
        I: 'NQY)',
        W: 0x3fc,
        T: '0Ddi',
        Z: 'xqZj',
        G: 'pOZA',
        E: 'frRp',
        a: '0Ddi',
        U: 0x95,
        c: 'oizW',
        n: 0x1de,
        B: 0x192,
        g: 'jO@S',
        P: 0x534,
        q: '2g$W',
        r: 0x391,
        X: 'E$*v',
        x: 'Q*e#',
        S: 0x509,
        w: 0x2c0,
        p: 0x5f6,
        z: '[nEQ',
        k: 0x501,
        v: 'WQ!j',
        f: 0x1bd,
        M0: 0x307,
        M1: 0x4e0,
        M2: 'C$jf',
        M3: 0x2fb,
        M4: 0xd9,
        M5: 'nZ^k'
      },
      mR = __sk_MI,
      b = {
        'CcIul': function (h, J) {
          return h === J;
        },
        'UpuMf': function (h, J) {
          return h + J;
        },
        'AdZjb': mR(__sk_lS.M, 'NQY)'),
        'POetG': mR(0x3ce, '0Q4Q'),
        'uDLpw': function (h, J) {
          return h(J);
        },
        'yBWkQ': function (h, J) {
          return h + J;
        },
        'iCTSl': mR(0x26d, '2vEu'),
        'VbTHi': mR(0xaf, 'W!#l'),
        'DzecR': function (h, J) {
          return h + J;
        },
        'jRRIE': function (h, J) {
          return h < J;
        },
        'ivgYk': function (h, J) {
          return h + J;
        },
        'Bynql': function (h, J) {
          return h || J;
        },
        'CKaTP': mR(__sk_lS.m, 'frRp'),
        'QWuPo': function (h, J) {
          return h + J;
        },
        'AiwPW': 'ℹ️\x20',
        'uQVbC': function (h, J) {
          return h || J;
        },
        'FxuPv': mR(0x380, 'zu(j'),
        'FkzlK': 'unknown',
        'LBkzk': function (h, J) {
          return h !== J;
        },
        'cltrl': mR(__sk_lS.F, 'yoVP'),
        'uzExN': mR(0xc2, 'NVQW'),
        'uOpyz': function (h, J, Y, L) {
          return h(J, Y, L);
        },
        'wVUJu': '/nssig',
        'SWAMS': mR(0xb9, __sk_lS.b),
        'KoFbJ': mR(0xe0, 'v7OE'),
        'WMBPX': 'body',
        'mJiKL': function (h, J) {
          return h !== J;
        },
        'tQIaI': function (h, J) {
          return h < J;
        },
        'abRub': function (h, J) {
          return h + J;
        },
        'VQPIp': 'string',
        'IXwwx': function (h, J) {
          return h + J;
        },
        'uvsFF': function (h, J) {
          return h <= J;
        },
        'scsyb': function (h, J) {
          return h + J;
        },
        'vdosC': mR(0x61e, 'v7OE'),
        'paLKD': function (h, J) {
          return h + J;
        }
      };
    for (let h = 1; h <= 3; h++) try {
      if (b['LBkzk'](b['cltrl'], b['uzExN'])) {
        var R = getOrCreateDeviceId(),
          O = JSON[mR(__sk_lS.l, 'dz)r')]({
            'type': mR(0x350, 'Q*e#'),
            'path': m[mR(0x14c, 'UzGo')],
            'data': m['reqdata'],
            'salt': m['api_client_salt'],
            'ud': this['userId'] || '',
            'script_version': SCRIPT_VERSION,
            'device_id': R
          }),
          V = (await b['uOpyz'](sendRequest, {
            'method': 'POST',
            'url': API_CONFIG['toudeshishabi'] + b['wVUJu'],
            'headers': {
              'Content-Type': b[mR(__sk_lS.o, 'v7OE')],
              'User-Agent': b['KoFbJ']
            },
            'body': O,
            'timeout': 0x3a98
          }, this['proxyUrl'], F + mR(0x33d, __sk_lS.R)))[b['WMBPX']];
        if (!V) {
          if (b['mJiKL']('IwPJT', 'PpIQt')) {
            if (b[mR(__sk_lS.O, 'ZDJd')](h, 3)) continue;
            return console['log'](b[mR(__sk_lS.V, '$aoM')]('❌\x20' + this['getAccountDisplayName'](), '\x20') + F + mR(0x413, 'E0S!')), null;
          } else m = {};
        }
        if (V && V['status']) {
          if (mR(__sk_lS.A, __sk_lS.K) !== mR(__sk_lS.H, 'w%uJ')) {
            if (b['CcIul']('OK', N['errorMsg']) && y['feeds'] && m['feeds'][0] && j['feeds'][0]['ad']) {
              var L = P[mR(0x57a, '(%(I')][0],
                d = L?.['ad']?.[mR(0x2f2, __sk_lS.C)] ?? L?.[mR(0x330, 'tlkh')],
                N = (L['exp_tag'] || '')['split']('/')[1]?.['split']('_')?.[0] || '';
              if (!d) return q['log'](b['UpuMf'](b['AdZjb'], this['getAccountDisplayName']()) + b[mR(__sk_lS.s, 'WQ!j')]), null;
              var y = (L[mR(0x2bb, 'NQY)')] || L?.['ad']?.['caption'] || '')[mR(__sk_lS.h, 'LPN7')](0, 30);
              let D = 0;
              try {
                L['ad']?.['adDataV2']?.[mR(__sk_lS.J, '0Bp5')]?.['awardValue'] ? D = M1(L['ad']['adDataV2'][mR(__sk_lS.Y, __sk_lS.L)][mR(0x57c, __sk_lS.d)]) || 0 : L['ad']?.['awardCoin'] ? D = b['uDLpw'](M2, L['ad']['awardCoin']) || 0 : L['ad']?.['adDataV2']?.['inspireAdInfo']?.['neoCount'] && (D = M3(L['ad'][mR(__sk_lS.i, 'duP7')]['inspireAdInfo']['neoCount']) || 0), 0 === D && (M4[mR(0x14e, __sk_lS.N)](b['yBWkQ'](b['UpuMf'](mR(0x38d, '2vEu'), this[mR(0x40a, '[nEQ')]()), b['iCTSl'])), M5[mR(__sk_lS.y, 'Q*e#')]('\x20\x20-\x20adData.ad.adDataV2:', L['ad']?.['adDataV2'] ? mR(0x4fc, '0Bp5') : mR(0x5a9, __sk_lS.e)), M6['log'](mR(0x40f, 'E$*v'), L['ad']?.['awardCoin'] || mR(0x1c5, 'ZDJd')), L['ad']?.[mR(0x3c8, __sk_lS.j)]) && (M7['log']('\x20\x20-\x20inspirePersonalize:', L['ad'][mR(0x640, 'kJU#')]['inspirePersonalize'] ? mR(0x3be, 'WQ!j') : b[mR(__sk_lS.D, __sk_lS.t)]), M8['log']('\x20\x20-\x20inspireAdInfo:', L['ad'][mR(__sk_lS.Q, 'xqZj')]['inspireAdInfo'] ? mR(0x45b, 'C$jf') : mR(0x2de, 'W8id')));
              } catch (Q) {
                MM[mR(__sk_lS.u, __sk_lS.I)](b[mR(__sk_lS.W, __sk_lS.T)]('❌\x20', this['getAccountDisplayName']()) + mR(0x346, __sk_lS.Z) + Q['message']);
              }
              var j = {};
              return j[mR(0xfa, '0Bp5')] = d, j[mR(0x251, 'U@ca')] = N, b[mR(0x386, __sk_lS.G)](0, D) ? f['log'](b['ivgYk'](mR(0x1e8, __sk_lS.E), this[mR(0xa3, __sk_lS.a)]()) + mR(__sk_lS.U, __sk_lS.c) + b['Bynql'](y, b[mR(0x401, '2vEu')]) + mR(__sk_lS.n, 'E$*v') + D + mR(__sk_lS.B, __sk_lS.g)) : M0['log'](b['QWuPo'](b['ivgYk'](b['AiwPW'] + this[mR(0x325, 'WQ!j')](), mR(0x52b, 'nZ^k')), b['uQVbC'](y, mR(__sk_lS.P, __sk_lS.q))) + mR(__sk_lS.r, 'w%uJ')), j;
            }
            A['log'](b['UpuMf'](b['QWuPo']('❌\x20' + this[mR(0x3cf, __sk_lS.X)](), b['FxuPv']) + (B?.['error_msg'] || b['FkzlK']) + mR(0x49c, __sk_lS.x), L?.['result'] || b['FkzlK']));
          } else {
            let L = V['data'];
            if (b[mR(0x5b3, 'yoVP')] == typeof L) try {
              L = JSON[mR(__sk_lS.S, 'E0S!')](L);
            } catch (N) {
              if (b[mR(__sk_lS.w, '*phW')](h, 3)) continue;
              return console['log']('❌\x20' + this['getAccountDisplayName']() + '\x20' + F + mR(0x41a, 'W8id')), null;
            }
            let d = L;
            var A = (d = L['data'] && 'object' == typeof L['data'] ? L[mR(__sk_lS.p, __sk_lS.z)] : d)['nssig3'] || d['__NS_sig3'],
              K = d[mR(0x4d7, 'NVQW')] || d['__NStokensig'],
              H = d[mR(__sk_lS.k, __sk_lS.v)] || d[mR(__sk_lS.f, 'W8id')] || d['__NS_xfalcon'] || '',
              C = {};
            return C['__NS_sig3'] = A, C['__NStokensig'] = K, C['sig'] = d[mR(0x20b, 'y[XJ')], C['__NS_xfalcon'] = H, C;
          }
        }
        if (3 <= h && console[mR(0x20a, '0Bp5')](b[mR(__sk_lS.M0, 'NQY)')]('❌\x20' + this['getAccountDisplayName'](), '\x20') + F + mR(0x420, 'M)%K')), !(h < 3)) return null;
      } else return false;
    } catch (j) {
      if (b[mR(__sk_lS.M1, __sk_lS.M2)](3, h) && console[mR(__sk_lS.M3, 'tlkh')](b['scsyb']('❌\x20' + this['getAccountDisplayName']() + '\x20' + F + b['vdosC'], j['message'])), !(h < 3)) return null;
    }
    return console['log'](b[mR(__sk_lS.M4, 'w%uJ')](b['abRub']('❌\x20' + this['getAccountDisplayName']() + '\x20', F), mR(0x185, __sk_lS.M5))), null;
  }
  async ['executeTask'](M) {
    var __sk_o9 = {
        M: 'duP7',
        m: 0x2c1,
        F: 'E$*v',
        b: 0x1b3,
        l: 'tlkh',
        o: 0xed,
        R: '[nEQ',
        O: 0x141,
        V: 'frRp',
        A: 0x12f,
        K: 0x1b1,
        H: 'LPN7',
        C: 0x5b6,
        s: '2g$W',
        h: 0x2da,
        J: 0x574,
        Y: 0x52e,
        L: 0x335,
        d: 0x35f,
        i: 'i1Bf',
        N: 0x46b,
        y: 0x24d,
        e: 'U@ca',
        j: 0x28c,
        D: 0x610,
        t: 'NQY)',
        Q: 'nZ^k',
        u: 0x312,
        I: 0x384,
        W: 0x3f4,
        T: 'yoVP',
        Z: '$E2a',
        G: '%N&g',
        E: 'U)4u',
        a: 0x5ff,
        U: '(%(I',
        c: 0x4c9,
        n: 0x5be,
        B: 0x42f,
        g: 'w%uJ',
        P: 0x395,
        q: 'WQ!j',
        r: 0x630,
        X: 0x3b5,
        x: 'jO@S',
        S: 'Q*e#',
        w: 0x63d,
        p: 0xc8,
        z: '&fI4',
        k: '$E2a',
        v: 0x3fb,
        f: 0x1b7,
        M0: 'WQ!j',
        M1: 0x41e,
        M2: 0x236,
        M3: 0x3eb,
        M4: '(%(I',
        M5: 0x3c7,
        M6: 0x195,
        M7: '*phW',
        M8: 'w%uJ',
        M9: 0x642,
        MM: 'kJU#',
        Mm: '*phW',
        MF: 0x374,
        Mb: 'bR&C',
        Ml: 'y[XJ',
        Mo: 0x620,
        MR: 0x3d4,
        MO: 0x207,
        MV: 'H]2s',
        MA: 0x62a,
        MK: '$aoM',
        MH: 0x45c,
        MC: 'C$jf',
        Ms: '8Hpe',
        Mh: 0xe1,
        MJ: 0x1cb,
        MY: 0xf6
      },
      mO = __sk_MI,
      m = {
        'IIkFD': function (N, j) {
          return N <= j;
        },
        'bMJOL': mO(0x566, __sk_o9.M),
        'tuZEA': function (N, j) {
          return N + j;
        },
        'xpIUa': mO(__sk_o9.m, __sk_o9.F),
        'HZoJI': function (N, y, j, D) {
          return N(y, j, D);
        },
        'UgipQ': mO(__sk_o9.b, __sk_o9.l),
        'EyeBe': function (N, j) {
          return N === j;
        },
        'NdzRK': 'aaLmG',
        'wCHRK': function (N, j) {
          return N - j;
        },
        'FZgid': function (N, j) {
          return N + j;
        },
        'BLstf': function (N, j) {
          return N + j;
        },
        'UeuoK': mO(__sk_o9.o, __sk_o9.R),
        'rOxcb': function (N, j) {
          return N + j;
        },
        'zGGwm': mO(__sk_o9.O, __sk_o9.V),
        'ueSBj': mO(__sk_o9.A, '*phW'),
        'akcsh': mO(__sk_o9.K, __sk_o9.H),
        'HKtHy': mO(__sk_o9.C, 'pOZA'),
        'IyTuI': mO(0x22b, __sk_o9.s),
        'XTGkT': 'box',
        'aKdFk': mO(__sk_o9.h, '[sJs'),
        'BZaMJ': mO(0x632, 'M)%K'),
        'itImF': 'look_follow',
        'wPpAs': mO(__sk_o9.J, 'C$jf'),
        'aqDsp': function (N, y, j) {
          return N(y, j);
        },
        'HEBZR': function (N, j) {
          return N + j;
        },
        'KKjFx': function (N, j) {
          return N + j;
        },
        'CUmnq': function (N, j) {
          return N + j;
        },
        'TkyHz': mO(__sk_o9.Y, 'U)4u'),
        'durLF': function (N, j) {
          return N + j;
        },
        'NsHMM': mO(__sk_o9.L, '8Hpe'),
        'zlSxA': function (N, j) {
          return N + j;
        },
        'fgPwP': function (N, j) {
          return N + j;
        }
      };
    if (!this['tasksToExecute'][mO(0x44e, '%N&g')](M)) return false;
    var F = this[mO(__sk_o9.d, 'nZ^k')][M];
    if (!F) return console['log'](m[mO(0x503, __sk_o9.i)]('❌\x20', this[mO(__sk_o9.N, 'bR&C')]()) + m[mO(__sk_o9.y, 'jO@S')] + M), false;
    if (this[mO(0x349, __sk_o9.e)][M]) return false;
    try {
      var b = (await m['HZoJI'](getAccountBasicInfo, this['getCookieWithCurrentDid'](), this['proxyUrl'], this['index']))?.['totalCoin'] || 0;
      let N,
        y = 0;
      for (; y < 3;) {
        var R = await this['getAdInfo'](F);
        if (R && R['needsNewDid']) this[mO(__sk_o9.j, 'WQ!j')] = true, y++, console['log'](m['tuZEA'](mO(__sk_o9.D, '*phW'), this['getAccountDisplayName']()) + m[mO(0x42e, '(%(I')] + y + mO(0x5d3, 'xqZj'));else {
          if (m['EyeBe'](m[mO(0x471, '0Bp5')], m[mO(0x5eb, 'WQ!j')])) {
            if (R) {
              N = R;
              break;
            }
            if (3 <= ++y) return console['log'](m['tuZEA'](m[mO(0xf2, __sk_o9.t)]('❌\x20' + this[mO(0x103, __sk_o9.Q)](), mO(0x49d, 'pOZA')), F['name']) + mO(__sk_o9.u, 'tlkh')), this['taskStats'][M]['failed']++, false;
            await new Promise(j => setTimeout(j, 2000));
          } else {
            var I,
              W = [],
              T = new O();
            V[mO(__sk_o9.I, 'duP7')][mO(0x2e4, 'E0S!')] && (I = A[mO(__sk_o9.W, __sk_o9.T)]['ksck'][mO(0x117, __sk_o9.Z)]('&')['map'](E => E['trim']())['filter'](K), W[mO(0x4e2, __sk_o9.G)](...I));
            for (let E = 1; m['IIkFD'](E, 666); E++) {
              var Z = m[mO(0x5c2, __sk_o9.E)] + E;
              L['env'][Z] && (Z = J['env'][Z][mO(__sk_o9.a, __sk_o9.U)]('&')[mO(__sk_o9.c, 'W8id')](U => U[mO(0x51f, 'WQ!j')]())['filter'](b), W['push'](...Z));
            }
            var G = [];
            for (const U of W) T[mO(__sk_o9.n, '&ikq')](U) || (T[mO(__sk_o9.B, __sk_o9.g)](U), G[mO(0x544, 'yoVP')](U));
            return G;
          }
        }
      }
      if (N) {
        const D = Math[mO(__sk_o9.P, __sk_o9.q)](Math[mO(__sk_o9.r, 'NQY)')]() * m[mO(__sk_o9.X, 'duP7')](m[mO(0x500, '2vEu')](WATCH_AD_MAX, WATCH_AD_MIN), 1)) + WATCH_AD_MIN;
        console['log'](m['FZgid'](m['BLstf']('⏱️\x20', this[mO(0xa3, '0Ddi')]()) + '\x20', F['name']) + mO(0x224, __sk_o9.x) + D + m[mO(0x59f, __sk_o9.S)]), await new Promise(Q => setTimeout(Q, 1000 * D));
        var O = await this['generateSignature'](N['cid'], N['llsid'], M, F);
        if (O) {
          let Q,
            I = 0;
          for (; I < 3 && !(Q = await this[mO(__sk_o9.w, 'v7OE')](O[mO(__sk_o9.p, 'ZDJd')], O['sig3'], O[mO(0x9f, '0Q4Q')], O[mO(0x24f, __sk_o9.z)] || '', O['post'], M, F))?.['success'] && Q?.[mO(0x35e, '&fI4')];) I++, Q['shouldRetry'] && (await new Promise(W => setTimeout(W, 30000)), console['log'](m['rOxcb'](m['tuZEA'](m['zGGwm'], this['getAccountDisplayName']()) + mO(0x149, '@xcw') + I, m['ueSBj'])));
          if (Q?.['success']) {
            this['taskStats'][M]['success']++, this['taskStats'][M]['totalReward'] += Q['reward'] || 0;
            var V = (await getAccountBasicInfo(this['getCookieWithCurrentDid'](), this['proxyUrl'], this['index']))?.[mO(0x532, __sk_o9.k)] || b,
              A = Q[mO(0x30a, __sk_o9.g)] || 0,
              K = this[mO(0x48a, 'nZ^k')] || this['nickname'] || mO(0x21c, 'C$jf');
            let W = m['akcsh'],
              T = colors['bright'] + colors['cyan'];
            switch (M) {
              case m['HKtHy']:
                W = m['IyTuI'], T = colors[mO(0x306, __sk_o9.T)] + colors[mO(__sk_o9.v, 'E$*v')];
                break;
              case m['XTGkT']:
                W = m['aKdFk'], T = colors[mO(0xf9, __sk_o9.M)] + colors['blue'];
                break;
              case 'food':
                W = m['BZaMJ'], T = colors['bright'] + colors['green'];
                break;
              case m[mO(__sk_o9.f, '0Bp5')]:
                W = mO(0x14b, __sk_o9.M0), T = m['tuZEA'](colors['bright'], colors[mO(__sk_o9.M1, 'M)%K')]);
                break;
              case 'search_follow':
                W = m['wPpAs'], T = m['rOxcb'](colors['bright'], colors['yellow']);
                break;
              case mO(0x432, 'oizW'):
                W = m['akcsh'], T = colors[mO(__sk_o9.M2, 'bR&C')] + colors[mO(__sk_o9.M3, __sk_o9.e)];
                break;
              case 'search':
                W = mO(0x9d, '0Q4Q'), T = colors['bright'] + colors['cyan'];
            }
            var H = this[mO(0x60b, 'kJU#')][M][mO(0x4ff, '2vEu')],
              C = this['getTaskTotalCount'](M),
              J = 100 <= A ? colors['bright'] + colors['red'] + mO(0x1ec, __sk_o9.M4) + colors['reset'] : m['FZgid'](colors['bright'], colors['green']) + mO(0x13c, '[nEQ') + colors[mO(0x5f8, '0Q4Q')],
              Y = formatTag(K),
              L = m[mO(__sk_o9.M5, '*phW')](colorText, W, T);
            return console[mO(__sk_o9.M6, 'WQ!j')](m[mO(0x2f0, '0Ddi')](m['rOxcb'](m['rOxcb'](m['HEBZR'](J + '\x20' + Y + '\x20' + L + mO(0x249, __sk_o9.M7), H), '/'), C) + mO(0x2a6, 'bR&C') + formatWarning(A) + mO(0x1f8, __sk_o9.T), formatSuccess(V))), true;
          }
          this['taskStats'][M][mO(0x3c9, 'W8id')]++, this[mO(0x40e, 'H]2s')][M] ? console[mO(0x3e1, 'W!#l')](m['KKjFx'](mO(0x584, __sk_o9.M8) + this[mO(__sk_o9.M9, __sk_o9.MM)]() + '\x20', F['name']) + mO(0x47b, __sk_o9.Mm)) : Q ? console['log'](m[mO(0x161, 'v7OE')](m[mO(__sk_o9.MF, __sk_o9.t)]('❌\x20', this['getAccountDisplayName']()) + mO(0x30e, __sk_o9.Mb) + F[mO(0x17e, __sk_o9.Ml)], m[mO(__sk_o9.Mo, 'jO@S')])) : console['log'](m['KKjFx'](m['durLF'](m[mO(0x4af, 'nZ^k')]('❌\x20', this[mO(__sk_o9.MR, 'pOZA')]()), mO(__sk_o9.MO, __sk_o9.MV)) + F['name'], m[mO(0x4b7, 'bR&C')]));
        } else this[mO(__sk_o9.MA, 'WQ!j')][M][mO(0x4bb, __sk_o9.MK)]++, console['log'](m[mO(0x4f2, 'W8id')](m[mO(0x1bb, 'zu(j')](m[mO(__sk_o9.MH, __sk_o9.MC)](m[mO(0x119, __sk_o9.Ms)]('❌\x20', this['getAccountDisplayName']()), mO(__sk_o9.Mh, __sk_o9.Ml)), F['name']), mO(0x2ab, '0Ddi')));
      } else this['taskStats'][M][mO(__sk_o9.MJ, 'bR&C')]++;
      return false;
    } catch (Z) {
      return this[mO(__sk_o9.MY, '8Hpe')][M]['failed']++, false;
    }
  }
  async [__sk_MI(0x283, 'E$*v')]() {
    var __sk_oC = {
        M: 'W!#l',
        m: 0x108,
        F: '&ikq',
        b: 'duP7',
        l: 0x5e0,
        o: 'v7OE',
        R: 'W!#l',
        O: 0x26e,
        V: 'LPN7',
        A: 0x2d5,
        K: '3Qg6',
        H: '%N&g',
        C: 0x582,
        s: '[nEQ',
        h: 0x5bc,
        J: 0xde,
        Y: 0x519,
        L: 0x61a,
        d: 'jO@S',
        i: 0x218,
        N: '0Q4Q',
        y: 0x176,
        e: 0x611,
        j: 'pOZA',
        D: 0x325,
        t: 'WQ!j',
        Q: 'U@ca',
        u: 0xee,
        I: 0x191,
        W: 'i1Bf',
        T: 'ZDJd',
        Z: 0x637,
        G: 0x5ba,
        E: 'bR&C',
        a: 'C$jf',
        U: 'Q*e#',
        c: 'E$*v',
        n: 'bR&C',
        B: '8Hpe',
        g: 0x5b1,
        P: 'UzGo',
        q: 'dz)r',
        r: 0x301,
        X: 'nZ^k',
        x: 0x3f1,
        S: 'y[XJ',
        w: 0x630,
        p: 0x572,
        z: '2vEu',
        k: '$E2a',
        v: 0xd1,
        f: 0x390,
        M0: '&fI4',
        M1: 'NQY)',
        M2: 0x4b5
      },
      mV = __sk_MI,
      m = {};
    m['tTFpy'] = function (b, l) {
      return b + l;
    }, m['ZodJd'] = function (b, l) {
      return b + l;
    }, m['fGdgS'] = function (b, l) {
      return b + l;
    }, m[mV(0x2a8, __sk_oC.M)] = mV(__sk_oC.m, __sk_oC.F), m['YuAVF'] = function (b, l) {
      return b === l;
    }, m['fdIPt'] = mV(0x5a5, 'ZDJd'), m['rRipP'] = mV(0x258, __sk_oC.b), m['TvWKq'] = function (b, l) {
      return b + l;
    }, m['EgEQa'] = 'search', m[mV(__sk_oC.l, __sk_oC.o)] = mV(0x3d5, __sk_oC.R), m['JWDgC'] = mV(__sk_oC.O, __sk_oC.V), m[mV(0x1af, 'w%uJ')] = 'look_follow', m[mV(__sk_oC.A, __sk_oC.K)] = function (b, l) {
      return b + l;
    }, m['SOgZN'] = 'ℹ️\x20', m['rsKMl'] = mV(0x23f, __sk_oC.H), m[mV(0x28d, '(%(I')] = function (b, l) {
      return b + l;
    }, m[mV(__sk_oC.C, __sk_oC.s)] = mV(0x87, 'ZDJd'), m[mV(0x210, __sk_oC.R)] = function (b, l) {
      return b === l;
    }, m[mV(0x56c, '0Q4Q')] = mV(0x361, 'W8id'), m['aiRjS'] = function (b, l) {
      return b + l;
    }, m['dnvyf'] = function (b, l) {
      return b * l;
    }, m[mV(__sk_oC.h, '0Q4Q')] = function (b, l) {
      return b + l;
    }, m['MxxTH'] = function (b, l) {
      return b + l;
    }, m['cseix'] = mV(__sk_oC.J, __sk_oC.s);
    var F = m;
    console['log'](F['fGdgS'](F['pLnFr'], this[mV(__sk_oC.Y, 'U)4u')]()) + mV(0x142, '[sJs') + this[mV(__sk_oC.L, 'dz)r')]['join']('\x20→\x20'));
    for (const b of this['tasksToExecute']) {
      if (F['YuAVF']('CsPJR', mV(0x4c4, __sk_oC.d))) {
        let o = '';
        for (let R = 0; R < 32; R++) o += V[mV(__sk_oC.i, '%N&g')](A['floor'](K['random']() * H['length']));
        return o;
      } else {
        if (this['stopAllTasks']) {
          console[mV(0x124, __sk_oC.N)](F[mV(__sk_oC.y, '$E2a')](mV(__sk_oC.e, __sk_oC.j) + this[mV(__sk_oC.D, __sk_oC.t)](), F[mV(0x2a0, __sk_oC.Q)]));
          break;
        }
        if (this['taskLimitReached'][b]) console['log'](F['ZodJd']('⏭️\x20' + this['getAccountDisplayName'](), '\x20') + this[mV(__sk_oC.u, '0Q4Q')][b]?.[mV(__sk_oC.I, __sk_oC.W)] + F[mV(0x589, 'WQ!j')]);else {
          switch (console[mV(0x3ff, 'oizW')](F[mV(0x1d2, 'LPN7')](F['TvWKq']('🚀\x20' + this[mV(0x433, 'tlkh')](), mV(0x522, __sk_oC.T)), this['taskConfigs'][b]?.['name'])), b) {
            case mV(__sk_oC.Z, '*phW'):
              await this[mV(0x559, 'w%uJ')]();
              break;
            case F['EgEQa']:
              await this[mV(__sk_oC.G, '0Bp5')]();
              break;
            case 'box':
              await this[mV(0x505, __sk_oC.E)]();
              break;
            case F[mV(0x383, __sk_oC.a)]:
              await this[mV(0x36f, __sk_oC.U)]();
              break;
            case F['JWDgC']:
              await this[mV(0x152, 'frRp')]();
              break;
            case F[mV(0x37c, __sk_oC.c)]:
              console['log'](F[mV(0x55c, 'oizW')](F['SOgZN'] + this['getAccountDisplayName'](), F['rsKMl']));
              break;
            case 'search_follow':
              console['log'](F['DgUzZ'](F[mV(0x5e3, 'UzGo')]('ℹ️\x20', this[mV(0x8a, '2g$W')]()), mV(0x4b2, __sk_oC.n)));
              break;
            default:
              console[mV(0x5d6, '8Hpe')]('❌\x20' + this['getAccountDisplayName']() + F[mV(0x314, __sk_oC.B)] + b);
          }
          if (!F[mV(__sk_oC.g, __sk_oC.P)](b, this['tasksToExecute'][this[mV(0x61a, __sk_oC.q)]['length'] - 1]) && !this['stopAllTasks'] && !this['taskLimitReached'][b]) {
            if (F[mV(0x97, '$aoM')] === mV(0x39e, '[nEQ')) return A = F[mV(0x354, __sk_oC.d)](F['ZodJd'](mV(__sk_oC.r, __sk_oC.X), K || H), ']') + (C ? F['tTFpy'](F['ZodJd']('（', s), '）') : ''), F['tTFpy'](h[mV(0x4e3, 'H]2s')] + J[mV(__sk_oC.x, 'C$jf')], Y) + L['reset'];else {
              const R = 1000 * F['aiRjS'](Math[mV(0x284, __sk_oC.S)](F['dnvyf'](Math[mV(__sk_oC.w, 'NQY)')](), TASK_SWITCH_MAX - TASK_SWITCH_MIN + 1)), TASK_SWITCH_MIN);
              console[mV(__sk_oC.p, __sk_oC.z)](F[mV(0x227, __sk_oC.k)](F[mV(__sk_oC.v, '[nEQ')](F[mV(0x13e, __sk_oC.M)]('⏱\x20', this[mV(__sk_oC.f, __sk_oC.M0)]()), F[mV(0x2b8, '0Ddi')]), Math['round'](R / 1000)) + mV(0x3a5, __sk_oC.a)), await new Promise(O => setTimeout(O, R));
            }
          }
        }
      }
    }
    return console[mV(0x3ff, 'oizW')](F['DgUzZ']('✅\x20' + this[mV(0x9a, __sk_oC.M1)](), mV(__sk_oC.M2, '*phW'))), {};
  }
  async ['executeLookTasks']() {
    var __sk_ox = {
        M: 0x3ba,
        m: 0x1dc,
        F: 0x4dd,
        b: 'oizW',
        l: 0x225,
        o: 'C$jf',
        R: 'M)%K',
        O: 0x165,
        V: 'tlkh',
        A: 'LPN7',
        K: '2vEu',
        H: 0x16d,
        C: 0x4ba,
        s: 'i1Bf',
        h: 0x221,
        J: 0x379,
        Y: 0x1f1,
        L: 0x604,
        d: 0x23a,
        i: 'oizW',
        N: 0x86,
        y: 0x580,
        e: 'w%uJ',
        j: 0x112,
        D: '8Hpe',
        t: 0x5fc,
        Q: 0x26c,
        u: 0x140,
        I: 'zu(j',
        W: 'v7OE',
        T: 'tlkh',
        Z: 0x5d8,
        G: 0x1be,
        E: 'oizW',
        a: 0x643,
        U: 0x577,
        c: '3Qg6',
        n: 0x339,
        B: 0x5c7,
        g: 'NQY)',
        P: 'H]2s',
        q: 'W!#l',
        r: '&fI4',
        X: 0x512,
        x: 0x472,
        S: 'pOZA',
        w: 'UzGo',
        p: 'zu(j',
        z: 0x2db,
        k: 'zu(j',
        v: 0x59c,
        f: '$aoM',
        M0: 'NVQW',
        M1: 'U)4u',
        M2: 0x22e,
        M3: 0x4ee,
        M4: '3Qg6',
        M5: 0x15b,
        M6: 0x3b2,
        M7: 'dz)r',
        M8: 0x365,
        M9: 'W8id',
        MM: 0x5d7,
        Mm: 'Q*e#',
        MF: 0x2ad,
        Mb: 0x136,
        Ml: '*phW',
        Mo: 0x3b6,
        MR: 0x5ce,
        MO: 'H]2s',
        MV: 0x2a7,
        MA: 0x2d1,
        MK: 'frRp',
        MH: 'duP7',
        MC: 0xfe,
        Ms: 'w%uJ',
        Mh: 0x410,
        MJ: 0x37a,
        MY: 0x88,
        ML: 'dz)r',
        Md: 0x4d3,
        Mi: 'y[XJ',
        MN: 0x3cb,
        My: 'jO@S',
        Me: 0x38f,
        Mj: 'pOZA',
        MD: 0x463,
        Mt: 0x3a4,
        MQ: 0x5b0
      },
      __sk_or = {
        M: 'kJU#'
      },
      mA = __sk_MI,
      M = {
        'olxAq': 'ipify.org',
        'EuIld': 'jsonip.com',
        'FzUWs': 'my-ip.io',
        'ytLDo': 'string',
        'QUkTH': function (b, l) {
          return b(l);
        },
        'skgwY': function (b, l) {
          return b + l;
        },
        'fnHAD': function (b, l) {
          return b - l;
        },
        'FhwdY': function (b, l) {
          return b + l;
        },
        'qEitD': '&egid=',
        'mmsRu': mA(__sk_ox.M, '3Qg6'),
        'TxPSC': mA(__sk_ox.m, 'ZDJd'),
        'lqemG': mA(__sk_ox.F, '&ikq'),
        'SdeUo': mA(0x10c, __sk_ox.b),
        'GgrEC': 'search',
        'xbQjX': mA(__sk_ox.l, __sk_ox.o),
        'xYdXm': mA(0x253, '3Qg6'),
        'gkvSe': function (b, l) {
          return b != l;
        },
        'bHKHt': mA(0x4ca, __sk_ox.R),
        'TAAao': '502\x20Bad\x20Gateway',
        'xLPTd': function (b, l, R) {
          return b(l, R);
        },
        'iZuNu': function (b, l) {
          return b < l;
        },
        'qSQaJ': function (b, l) {
          return b + l;
        },
        'JmcbQ': function (b, l) {
          return b + l;
        },
        'xkgjF': function (b, l) {
          return b + l;
        },
        'hEXMT': mA(__sk_ox.O, __sk_ox.V),
        'IrCXX': function (b, l) {
          return b + l;
        },
        'voUdB': function (b, l) {
          return b + l;
        },
        'MYTzW': function (b, l) {
          return b + l;
        },
        'EHSly': function (b, l) {
          return b + l;
        },
        'eYzJn': mA(0x5d0, 'kJU#'),
        'kzZXD': 'look',
        'ybAAN': function (b, l) {
          return b * l;
        },
        'QFnZZ': function (b, l) {
          return b + l;
        },
        'KTsqC': mA(0x172, __sk_ox.A),
        'kamXG': function (b, l) {
          return b / l;
        },
        'SKxLA': function (b, l) {
          return b < l;
        },
        'SySdn': 'qQTlQ',
        'VpXEq': mA(0x147, 'y[XJ'),
        'aajlq': function (b, l) {
          return b + l;
        },
        'XqAjq': function (b, l) {
          return b + l;
        },
        'ByPMT': mA(0x4a2, __sk_ox.K),
        'VGhgV': function (b, l) {
          return b + l;
        },
        'UEkZN': function (b, l) {
          return b + l;
        },
        'bXSrf': function (b, l) {
          return b - l;
        },
        'CZdJg': function (b, l) {
          return b * l;
        },
        'ZRPYO': function (b, l) {
          return b + l;
        },
        'HsNEW': function (b, l) {
          return b - l;
        },
        'tsAKC': function (b, l) {
          return b + l;
        },
        'Tsbcf': function (b, l) {
          return b / l;
        },
        'DgOKZ': function (b, l) {
          return b + l;
        }
      };
    console[mA(0x3cd, 'yoVP')](M['qSQaJ'](M['JmcbQ'](M['xkgjF'](M[mA(0x57d, 'UzGo')], this[mA(__sk_ox.H, '@xcw')]()), mA(__sk_ox.C, __sk_ox.s)), LOOK_COUNT) + mA(__sk_ox.h, '2vEu'));
    for (let b = 0; b < LOOK_COUNT; b++) {
      if (this[mA(0x515, 'U@ca')] || this[mA(0x2e3, 'nZ^k')][mA(0x10a, '2g$W')]) {
        if (mA(__sk_ox.J, __sk_ox.s) !== mA(__sk_ox.Y, 'w%uJ')) {
          let R = null;
          if (h['includes']('httpbin.org') && J['origin'] ? R = Y[mA(__sk_ox.L, 'U)4u')] : L[mA(0x39f, '3Qg6')](M['olxAq']) && d['ip'] || i[mA(0x334, '2g$W')](M['EuIld']) && N['ip'] || y[mA(__sk_ox.d, 'duP7')](M['FzUWs']) && R['ip'] ? R = j['ip'] : M['ytLDo'] != typeof D || m[mA(0xd4, '0Bp5')]('<') || (R = Q['trim']()), R && M['QUkTH'](u, R)) return {
            'ok': true,
            'msg': M['skgwY'](mA(0x22d, __sk_ox.i), R),
            'ip': R
          };
        } else {
          console['log'](M['IrCXX'](M['xkgjF']('🛑\x20', this[mA(__sk_ox.N, '[sJs')]()), mA(0x598, '0Bp5')));
          break;
        }
      }
      var m = M['IrCXX'](b, 1),
        m = (console[mA(__sk_ox.y, __sk_ox.e)](M['voUdB'](M['MYTzW'](M['EHSly']('🔍\x20', this['getAccountDisplayName']()) + M['eYzJn'] + m + '/', LOOK_COUNT), mA(__sk_ox.j, __sk_ox.D))), await this[mA(__sk_ox.t, '0Ddi')](M['kzZXD']));
      if (m && this['tasksToExecute'][mA(__sk_ox.Q, 'bR&C')](M['SdeUo']) && !this['stopAllTasks'] && !this['taskLimitReached']['look_follow'] && 0 < FOLLOW_COUNT) {
        const R = M[mA(__sk_ox.u, 'W!#l')](1000, Math['floor'](Math[mA(0x15c, __sk_ox.I)]() * M[mA(0x50e, 'ZDJd')](PRE_LOOK_FOLLOW_MAX - PRE_LOOK_FOLLOW_MIN, 1)) + PRE_LOOK_FOLLOW_MIN);
        console['log'](M['QFnZZ']('⏱\x20' + this[mA(0x4cb, __sk_ox.W)]() + M[mA(0x18a, '2vEu')] + Math['round'](M[mA(0x497, __sk_ox.T)](R, 1000)), mA(__sk_ox.Z, '[nEQ'))), await new Promise(O => setTimeout(O, R)), console['log']('🔄\x20' + this['getAccountDisplayName']() + mA(__sk_ox.G, __sk_ox.E) + FOLLOW_COUNT + mA(__sk_ox.a, '0Q4Q'));
        for (let O = 0; M['SKxLA'](O, FOLLOW_COUNT); O++) {
          {
            if (this['stopAllTasks'] || this['taskLimitReached']['look_follow']) {
              if (M['SySdn'] !== 'CAVlZ') {
                console['log']('🛑\x20' + this['getAccountDisplayName']() + M[mA(__sk_ox.MM, 'oizW')]);
                break;
              } else F['log'](M[mA(0xff, __sk_ox.Mm)] + b[mA(__sk_ox.MF, '&fI4')]);
            }
            var F = O + 1;
            if (console[mA(__sk_ox.Mb, __sk_ox.Ml)](M['aajlq'](M['skgwY'](M[mA(0x181, 'frRp')](mA(__sk_ox.Mo, '0Q4Q') + this[mA(0x4a4, '%N&g')]() + M[mA(__sk_ox.MR, __sk_ox.MO)], F) + '/', FOLLOW_COUNT), mA(0x35b, 'U@ca'))), await this['executeTask'](M['SdeUo']), O < FOLLOW_COUNT - 1 && !this[mA(__sk_ox.MV, '@xcw')] && !this[mA(0x369, 'UzGo')]['look_follow']) {
              {
                const H = 1000 * M['QFnZZ'](Math['floor'](Math[mA(__sk_ox.MJ, 'i1Bf')]() * M['aajlq'](BETWEEN_LOOK_FOLLOW_MAX - BETWEEN_LOOK_FOLLOW_MIN, 1)), BETWEEN_LOOK_FOLLOW_MIN);
                console['log'](M['VGhgV'](M['EHSly'](M['EHSly'](M['UEkZN']('⏱\x20', this[mA(__sk_ox.MY, __sk_ox.ML)]()), mA(0x421, 'bR&C')), Math['round'](H / 1000)), mA(__sk_ox.Md, __sk_ox.Mi))), await new Promise(C => setTimeout(C, H));
              }
            }
          }
        }
      }
      if (b < M[mA(0x543, 'y[XJ')](LOOK_COUNT, 1) && !this['stopAllTasks'] && !this[mA(0x3f9, 'pOZA')]['look']) {
        if ('mRgkO' === mA(__sk_ox.MN, 'Q*e#')) b = new l(F);else {
          const h = M['ybAAN'](1000, Math['floor'](M['CZdJg'](Math[mA(0x1df, 'oizW')](), M['ZRPYO'](M['HsNEW'](ROUND_END_MAX, ROUND_END_MIN), 1))) + ROUND_END_MIN);
          console['log'](M['aajlq'](M[mA(0x12e, __sk_ox.My)](M[mA(__sk_ox.Me, __sk_ox.Mj)]('⏱\x20' + this['getAccountDisplayName'](), mA(__sk_ox.MD, 'ZDJd')), Math[mA(0x80, '*phW')](M['Tsbcf'](h, 1000))), mA(0x450, 'v7OE'))), await new Promise(J => setTimeout(J, h));
        }
      }
    }
    console['log'](M[mA(__sk_ox.Mt, 'NQY)')](M[mA(__sk_ox.MQ, __sk_ox.o)]('✅\x20', this['getAccountDisplayName']()), mA(0x601, '&ikq')));
  }
  async ['executeSearchTasks']() {
    var __sk_RM = {
        M: 0x13f,
        m: 'NVQW',
        F: 0x4b3,
        b: 0x2a3,
        l: 'nZ^k',
        o: 0x627,
        R: 0xc1,
        O: 'W!#l',
        V: 'NVQW',
        A: 0x60e,
        K: 'C$jf',
        H: 'w%uJ',
        C: 'LPN7',
        s: 0x4eb,
        h: '*phW',
        J: 0x2e7,
        Y: 0x276,
        L: 0x5c1,
        d: 'E0S!',
        i: 'kJU#',
        N: 0x1a0,
        y: 0xe2,
        e: 0x3df,
        j: 0xc7,
        D: '[nEQ',
        t: 0x1a8,
        Q: 0x241,
        u: '3Qg6',
        I: 'U)4u',
        W: 0x8f,
        T: '&fI4',
        Z: '$aoM',
        G: 0x3aa,
        E: 0x19a,
        a: '0Bp5',
        U: 0x482,
        c: '&ikq',
        n: 0x616,
        B: 'bR&C',
        g: 0x585,
        P: 0x138,
        q: 'UzGo',
        r: '(%(I',
        X: 0x17f,
        x: 0x12a,
        S: 0x151,
        w: 0x56f,
        p: 'zu(j',
        z: 0x411,
        k: 0x507,
        v: 'xqZj',
        f: 'NQY)',
        M0: 'M)%K',
        M1: 'jO@S',
        M2: '0Q4Q',
        M3: 0x1c3,
        M4: 0x376,
        M5: 0x4be,
        M6: 0x549,
        M7: 0x212,
        M8: 'bR&C',
        M9: 0x52a,
        MM: 0x4fa,
        Mm: 'v7OE',
        MF: 0x4cc
      },
      mH = __sk_MI,
      m = {};
    m['OWpKQ'] = function (R, O) {
      return R >= O;
    }, m['lZTlP'] = function (R, O) {
      return R + O;
    }, m['vlvqE'] = '⚠️\x20', m['dUcvA'] = mH(__sk_RM.M, __sk_RM.m), m[mH(__sk_RM.F, 'LPN7')] = function (R, O) {
      return R + O;
    }, m['HQjdK'] = function (R, O) {
      return R < O;
    }, m[mH(__sk_RM.b, __sk_RM.l)] = function (R, O) {
      return R !== O;
    }, m['BctwK'] = mH(__sk_RM.o, 'E$*v'), m[mH(__sk_RM.R, 'E$*v')] = function (R, O) {
      return R + O;
    }, m['qNYJE'] = function (R, O) {
      return R + O;
    }, m[mH(0x45e, __sk_RM.O)] = 'search', m[mH(0x5a7, __sk_RM.V)] = 'search_follow', m[mH(__sk_RM.A, '2vEu')] = function (R, O) {
      return R + O;
    }, m['DBZdH'] = function (R, O) {
      return R / O;
    }, m[mH(0x5ef, __sk_RM.K)] = function (R, O) {
      return R + O;
    }, m['AOTWv'] = mH(0x57f, __sk_RM.H), m['jvXdf'] = function (R, O) {
      return R - O;
    }, m[mH(0xf8, __sk_RM.C)] = mH(0x537, 'E0S!'), m['AhhXt'] = 'sAhhe', m['TqSqz'] = function (R, O) {
      return R * O;
    }, m[mH(__sk_RM.s, 'bR&C')] = function (R, O) {
      return R + O;
    }, m[mH(0x3bf, 'bR&C')] = function (R, O) {
      return R - O;
    }, m['LmAph'] = 'dfYqh', m['hYCvU'] = function (R, O) {
      return R + O;
    }, m['SaXiE'] = function (R, O) {
      return R + O;
    }, m['SLZRH'] = mH(0x2b1, __sk_RM.h), m[mH(0x321, '&fI4')] = function (R, O) {
      return R + O;
    }, m[mH(__sk_RM.J, '8Hpe')] = mH(__sk_RM.Y, 'WQ!j');
    var F = m;
    console['log'](F['lZTlP'](F[mH(__sk_RM.L, __sk_RM.d)](F['esyqg']('🔎\x20', this['getAccountDisplayName']()) + mH(0x56e, __sk_RM.d), SEARCH_COUNT), mH(0x402, 'WQ!j')));
    for (let R = 0; F['HQjdK'](R, SEARCH_COUNT); R++) {
      if (F['BXeDL'](F['BctwK'], mH(0x595, __sk_RM.i))) {
        if (this['stopAllTasks'] || this[mH(0x369, 'UzGo')]['search']) {
          console[mH(0x3c6, '$aoM')]('🛑\x20' + this[mH(__sk_RM.N, 'ZDJd')]() + mH(__sk_RM.y, '&ikq'));
          break;
        }
        var b = F['cATyC'](R, 1),
          b = (console[mH(0x3ac, '3Qg6')](F['qNYJE'](mH(__sk_RM.e, '8Hpe') + this[mH(0x357, 'frRp')]() + mH(__sk_RM.j, 'U@ca') + b + '/' + SEARCH_COUNT, mH(0x425, 'NVQW'))), await this['executeTask'](F['wGXsL']));
        if (b && this[mH(0x14a, __sk_RM.D)][mH(0x5dc, '[nEQ')](F[mH(__sk_RM.t, '2vEu')]) && !this['stopAllTasks'] && !this['taskLimitReached']['search_follow'] && 0 < SEARCH_FOLLOW_COUNT) {
          const O = 1000 * F[mH(__sk_RM.Q, __sk_RM.u)](Math[mH(0x277, __sk_RM.I)](Math[mH(__sk_RM.W, 'kJU#')]() * F['lZTlP'](PRE_SEARCH_FOLLOW_MAX - PRE_SEARCH_FOLLOW_MIN, 1)), PRE_SEARCH_FOLLOW_MIN);
          console[mH(0x34c, __sk_RM.T)](F[mH(0x4f8, __sk_RM.Z)](F['cbDxy']('⏱\x20' + this['getAccountDisplayName']() + mH(__sk_RM.G, '*phW'), Math['round'](F['DBZdH'](O, 1000))), mH(__sk_RM.E, __sk_RM.a))), await new Promise(V => setTimeout(V, O)), console['log'](F['esyqg'](F['rDBdy']('🔄\x20' + this['getAccountDisplayName'](), mH(__sk_RM.U, __sk_RM.c)), SEARCH_FOLLOW_COUNT) + mH(__sk_RM.n, 'ZDJd'));
          for (let V = 0; V < SEARCH_FOLLOW_COUNT; V++) {
            if (this[mH(0x2c2, __sk_RM.B)] || this[mH(__sk_RM.g, 'v7OE')][mH(0x556, '0Bp5')]) {
              if ('rotTY' === mH(0x5f7, 'ZDJd')) {
                console['log'](F['esyqg']('🛑\x20' + this[mH(__sk_RM.P, __sk_RM.O)](), F[mH(0x47f, __sk_RM.q)]));
                break;
              } else {
                var K = b['apply'](l, arguments);
                return o = null, K;
              }
            }
            var l = V + 1;
            if (console['log'](F['cATyC'](mH(0x2b0, __sk_RM.r) + this['getAccountDisplayName']() + mH(__sk_RM.X, 'W8id'), l) + '/' + SEARCH_FOLLOW_COUNT + mH(__sk_RM.x, 'nZ^k')), this[mH(__sk_RM.S, '&fI4')]++, await this[mH(__sk_RM.w, __sk_RM.p)](mH(0x467, __sk_RM.O)), this[mH(__sk_RM.z, 'ZDJd')] = 0, V < F['jvXdf'](SEARCH_FOLLOW_COUNT, 1) && !this[mH(__sk_RM.k, 'H]2s')] && !this['taskLimitReached']['search_follow']) {
              if (F[mH(0x5af, __sk_RM.v)](F[mH(0xf1, __sk_RM.f)], F['AhhXt'])) {
                const K = F['TqSqz'](1000, F['UjRyu'](Math[mH(0x303, __sk_RM.M0)](Math['random']() * (F['DkYXS'](BETWEEN_SEARCH_FOLLOW_MAX, BETWEEN_SEARCH_FOLLOW_MIN) + 1)), BETWEEN_SEARCH_FOLLOW_MIN));
                console['log'](F[mH(0x3ec, __sk_RM.M1)]('⏱\x20' + this['getAccountDisplayName']() + mH(0x419, '3Qg6'), Math['round'](F['DBZdH'](K, 1000))) + mH(0x510, __sk_RM.M2)), await new Promise(H => setTimeout(H, K));
              } else {
                var C = b(l['totalCoin']);
                if (F['OWpKQ'](C, this[mH(__sk_RM.M3, '[sJs')])) return l['log'](F[mH(__sk_RM.M4, __sk_RM.a)](F['vlvqE'] + this['getAccountDisplayName']() + F['dUcvA'] + C + mH(__sk_RM.M5, 'Q*e#'), this['coinLimit']) + mH(0x560, 'H]2s')), this[mH(__sk_RM.M6, 'UzGo')] = true, this['stopAllTasks'] = true;
              }
            }
          }
        }
        if (R < SEARCH_COUNT - 1 && !this['stopAllTasks'] && !this['taskLimitReached']['search']) {
          if (F['LmAph'] === 'NoajG') b = l['parse'](l);else {
            const h = 1000 * (Math['floor'](F['TqSqz'](Math['random'](), ROUND_END_MAX - ROUND_END_MIN + 1)) + ROUND_END_MIN);
            console[mH(__sk_RM.M7, __sk_RM.M8)](F['lZTlP'](F['hYCvU'](F['SaXiE']('⏱\x20', this['getAccountDisplayName']()), F['SLZRH']) + Math['round'](F['DBZdH'](h, 1000)), mH(__sk_RM.M9, '2vEu'))), await new Promise(J => setTimeout(J, h));
          }
        }
      } else {
        A['config']?.['startTime'] && (K[mH(0x385, 'ZDJd')](), H['config'][mH(0x1e2, 'W8id')]);
        var Y = {};
        Y['response'] = J['response'], Y['body'] = Y;
        var L = {};
        return L['response'] = null, L[mH(__sk_RM.MM, __sk_RM.Mm)] = null, C['response'] ? (s = h[mH(0x272, 'v7OE')][mH(__sk_RM.MF, 'NQY)')] || null, Y) : (L['request'], L);
      }
    }
    console['log'](F[mH(0x4bf, '0Ddi')]('✅\x20', this['getAccountDisplayName']()) + F[mH(0xd5, '2g$W')]);
  }
  async ['executeBoxTasks']() {
    var __sk_RA = {
        M: 0x304,
        m: 0x4ce,
        F: 0x5f2,
        b: '&ikq',
        l: 0x197,
        o: 'v7OE',
        R: 0x1fa,
        O: 0x294,
        V: 'E$*v',
        A: 0x43f,
        K: '(%(I',
        H: 0x56b,
        C: 'ZDJd',
        s: 0x3bb,
        h: 'w%uJ',
        J: 0x2be,
        Y: 0x46c,
        L: 'kJU#',
        d: 0x3e2,
        i: 'jO@S',
        N: 0x1f9,
        y: 0x1a4,
        e: 0x5e7,
        j: '3Qg6',
        D: 0x38e,
        t: '$E2a',
        Q: '8Hpe'
      },
      mC = __sk_MI,
      m = {};
    m['yvDbn'] = function (l, R) {
      return l + R;
    }, m['neAIQ'] = function (l, R) {
      return l + R;
    }, m['ldHDP'] = mC(0x49f, '&fI4'), m[mC(__sk_RA.M, '[nEQ')] = function (l, R) {
      return l < R;
    }, m[mC(0x446, 'LPN7')] = '🛑\x20', m['qEvMs'] = function (l, R) {
      return l + R;
    }, m[mC(__sk_RA.m, '(%(I')] = mC(__sk_RA.F, __sk_RA.b), m['dTffa'] = 'box', m['jXiah'] = function (l, R) {
      return l - R;
    }, m[mC(0x29d, 'E0S!')] = function (l, R) {
      return l * R;
    }, m[mC(0x228, 'jO@S')] = mC(__sk_RA.l, __sk_RA.o), m[mC(__sk_RA.R, '[nEQ')] = function (l, R) {
      return l + R;
    }, m[mC(0x2b7, 'xqZj')] = function (l, R) {
      return l + R;
    };
    var F = m;
    console['log'](F['yvDbn'](F['neAIQ'](mC(__sk_RA.O, __sk_RA.V), this[mC(__sk_RA.A, __sk_RA.K)]()) + F[mC(__sk_RA.H, __sk_RA.C)] + BOX_COUNT, mC(__sk_RA.s, 'xqZj')));
    for (let l = 0; F[mC(0x291, __sk_RA.h)](l, BOX_COUNT); l++) {
      if (this['stopAllTasks'] || this['taskLimitReached']['box']) {
        console['log'](F['IkQfE'] + this['getAccountDisplayName']() + mC(0x9c, 'W8id'));
        break;
      }
      var b = l + 1;
      if (console['log'](F[mC(__sk_RA.J, '*phW')](mC(__sk_RA.Y, __sk_RA.C), this['getAccountDisplayName']()) + F['kJxoA'] + b + '/' + BOX_COUNT + mC(0x406, __sk_RA.L)), await this[mC(__sk_RA.d, __sk_RA.i)](F['dTffa']), F[mC(__sk_RA.N, 'W8id')](l, F[mC(__sk_RA.y, 'zu(j')](BOX_COUNT, 1)) && !this['stopAllTasks'] && !this[mC(__sk_RA.e, __sk_RA.j)][mC(__sk_RA.D, 'NVQW')]) {
        const R = 1000 * (Math['floor'](F[mC(0x551, 'bR&C')](Math['random'](), TASK_SWITCH_MAX - TASK_SWITCH_MIN + 1)) + TASK_SWITCH_MIN);
        console['log'](F['qEvMs']('⏱\x20' + this['getAccountDisplayName']() + F[mC(0x405, __sk_RA.t)], Math['round'](R / 1000)) + mC(0x2fa, '&ikq')), await new Promise(O => setTimeout(O, R));
      }
    }
    console['log'](F[mC(0xb7, 'ZDJd')](F[mC(0x465, 'U)4u')]('✅\x20', this['getAccountDisplayName']()), mC(0x5c4, __sk_RA.Q)));
  }
  async ['executeFoodTasks']() {
    var __sk_RY = {
        M: 0x4b0,
        m: 'oizW',
        F: 0x257,
        b: 'w%uJ',
        l: 0x3e7,
        o: '3Qg6',
        R: 0x523,
        O: 0x439,
        V: 0x542,
        A: '2g$W',
        K: 'nZ^k',
        H: 0x324,
        C: '0Q4Q',
        s: 0x51a,
        h: 'frRp',
        J: 'frRp',
        Y: 0xfd,
        L: 0x198,
        d: '(%(I',
        i: 0x5a3,
        N: '*phW'
      },
      ms = __sk_MI,
      m = {};
    m[ms(__sk_RY.M, 'ZDJd')] = function (l, R) {
      return l + R;
    }, m['cgipU'] = ms(0x541, '*phW'), m[ms(0xb4, __sk_RY.m)] = function (l, R) {
      return l === R;
    }, m['KzFWe'] = ms(__sk_RY.F, __sk_RY.b), m['oXjRF'] = '🛑\x20', m['BXGYo'] = ms(__sk_RY.l, __sk_RY.o), m[ms(0x4f9, 'nZ^k')] = function (l, R) {
      return l + R;
    }, m['zAhYw'] = function (l, R) {
      return l + R;
    }, m[ms(__sk_RY.R, '0Ddi')] = ms(__sk_RY.O, 'i1Bf'), m['bnHQe'] = function (l, R) {
      return l * R;
    }, m['WjYEg'] = function (l, R) {
      return l + R;
    }, m['zbPnU'] = ms(__sk_RY.V, 'W!#l'), m['sQWzP'] = ms(0xec, 'bR&C');
    var F = m;
    console['log'](F['KPDEb'](F['cgipU'] + this[ms(0x2e6, 'U@ca')]() + ms(0x327, __sk_RY.A), FOOD_COUNT) + ms(0x3d0, 'frRp'));
    for (let l = 0; l < FOOD_COUNT; l++) {
      if (this['stopAllTasks'] || this['taskLimitReached']['food']) {
        if (F['IKEDm'](F[ms(0x310, __sk_RY.K)], F['KzFWe'])) {
          console['log'](F['oXjRF'] + this['getAccountDisplayName']() + F['BXGYo']);
          break;
        } else {
          var O = o(R);
          O ? O['push'](O) : V[ms(0x372, 'H]2s')](ms(0x3d8, '[nEQ') + A);
        }
      }
      var b = F[ms(__sk_RY.H, 'E$*v')](l, 1);
      if (console[ms(0x1d4, 'M)%K')](F[ms(0x31a, __sk_RY.C)](F[ms(0x240, 'yoVP')](ms(__sk_RY.s, __sk_RY.h) + this['getAccountDisplayName']() + ms(0x5d2, __sk_RY.J) + b, '/'), FOOD_COUNT) + ms(0x3d3, '@xcw')), await this[ms(0x4e1, 'xqZj')](F['NxqGf']), l < FOOD_COUNT - 1 && !this['stopAllTasks'] && !this['taskLimitReached'][ms(0x41d, 'zu(j')]) {
        const O = F[ms(0x226, 'xqZj')](1000, Math[ms(__sk_RY.Y, '[nEQ')](F['bnHQe'](Math[ms(__sk_RY.L, __sk_RY.d)](), TASK_SWITCH_MAX - TASK_SWITCH_MIN + 1)) + TASK_SWITCH_MIN);
        console['log'](F['WjYEg']('⏱\x20' + this['getAccountDisplayName']() + F['zbPnU'], Math[ms(0x282, '8Hpe')](O / 1000)) + ms(0x4d3, 'y[XJ')), await new Promise(V => setTimeout(V, O));
      }
    }
    console['log'](F['WjYEg']('✅\x20', this[ms(__sk_RY.i, __sk_RY.N)]()) + F['sQWzP']);
  }
  async ['executeKboxTasks']() {
    var __sk_Rj = {
        M: '0Ddi',
        m: '8Hpe',
        F: 'frRp',
        b: 0x89,
        l: 0x216,
        o: 'yoVP',
        R: 0x24c,
        O: '(%(I',
        V: 0x46d,
        A: 'v7OE',
        K: 0x220,
        H: 0x4d4,
        C: '[nEQ',
        s: 0x3e9,
        h: 0x166,
        J: 0x438,
        Y: 'M)%K',
        L: 0x124,
        d: '0Q4Q',
        i: '2g$W',
        N: 0x90,
        y: 'duP7',
        e: 0x45a,
        j: '&fI4',
        D: '0Ddi',
        t: 0x22a
      },
      mh = __sk_MI,
      m = {};
    m[mh(0x511, 'E$*v')] = mh(0x2b4, __sk_Rj.M), m['GSmDT'] = function (l, R) {
      return l < R;
    }, m['nqwjs'] = 'NHkrx', m['JQfgj'] = function (l, R) {
      return l + R;
    }, m[mh(0x362, __sk_Rj.m)] = mh(0x16a, __sk_Rj.F), m['bdqjM'] = mh(__sk_Rj.b, 'U@ca'), m['CeOhD'] = function (l, R) {
      return l + R;
    }, m[mh(__sk_Rj.l, __sk_Rj.o)] = function (l, R) {
      return l - R;
    }, m['hzLjC'] = function (l, R) {
      return l * R;
    }, m['tQndD'] = function (l, R) {
      return l + R;
    };
    var F = m;
    console['log'](F[mh(__sk_Rj.R, __sk_Rj.O)] + this['getAccountDisplayName']() + mh(__sk_Rj.V, __sk_Rj.A) + KBOX_COUNT + mh(0x2e5, '$E2a'));
    for (let l = 0; F[mh(__sk_Rj.K, 'U)4u')](l, KBOX_COUNT); l++) {
      if (this[mh(__sk_Rj.H, 'ZDJd')] || this['taskLimitReached']['kbox']) {
        if (F[mh(0x491, __sk_Rj.C)] === F['nqwjs']) {
          console['log'](F[mh(__sk_Rj.s, 'oizW')](F['qQWGZ'], this['getAccountDisplayName']()) + F[mh(__sk_Rj.h, 'C$jf')]);
          break;
        } else return F[mh(__sk_Rj.J, '%N&g')](mh(0x435, __sk_Rj.Y) + b['message']), false;
      }
      var b = l + 1;
      if (console[mh(__sk_Rj.L, __sk_Rj.d)](F[mh(0x175, __sk_Rj.i)](F['CeOhD']('🔍\x20', this['getAccountDisplayName']()) + mh(__sk_Rj.N, __sk_Rj.y) + b + '/' + KBOX_COUNT, mh(0x396, 'U)4u'))), await this[mh(__sk_Rj.e, 'duP7')](mh(0x569, 'WQ!j')), l < F['uaPCx'](KBOX_COUNT, 1) && !this[mh(0xab, 'WQ!j')] && !this['taskLimitReached']['kbox']) {
        const O = F[mh(0x3c2, __sk_Rj.j)](1000, F[mh(0x57e, '0Q4Q')](Math['floor'](Math['random']() * (F['uaPCx'](TASK_SWITCH_MAX, TASK_SWITCH_MIN) + 1)), TASK_SWITCH_MIN));
        console['log']('⏱\x20' + this['getAccountDisplayName']() + mh(0x120, __sk_Rj.D) + Math['round'](O / 1000) + mh(__sk_Rj.t, 'NQY)')), await new Promise(V => setTimeout(V, O));
      }
    }
    console['log']('✅\x20' + this['getAccountDisplayName']() + mh(0xe6, '(%(I'));
  }
}
function parseAccountString(M) {
  var __sk_RE = {
      M: '$aoM',
      m: 0x297,
      F: 0x16b,
      b: '&ikq',
      l: 0x47a
    },
    mJ = __sk_MI,
    m = {
      'IzZUO': function (K, H) {
        return K === H;
      },
      'GwJma': function (K, H) {
        return K <= H;
      },
      'uRMrt': function (K, H) {
        return K === H;
      },
      'FiAZl': 'none',
      'AwoEK': function (K, H) {
        return K <= H;
      },
      'AgAIT': function (K, H) {
        return K + H;
      },
      'cgspg': function (K, H) {
        return K + H;
      },
      'riPKW': function (K, H) {
        return K + H;
      },
      'dGoSw': function (K, H) {
        return K + H;
      },
      'fRjRU': function (K, H) {
        return K(H);
      }
    },
    F,
    b,
    l,
    M = String(M || '')['trim']();
  if (!M) return null;
  let R = '',
    O = '',
    V = '',
    A = null;
  return M[mJ(0x44b, 'v7OE')]('#') ? 2 === (F = M['split']('#'))['length'] ? (V = F[0], O = F[1]) : m[mJ(0x34f, __sk_RE.M)](3, F[mJ(0x311, 'i1Bf')]) ? /^socks5:\/\//i[mJ(__sk_RE.m, __sk_RE.M)](F[2]) || F[2]['includes']('|') ? (V = F[0], O = F[1], A = F[2]) : (R = F[0], V = F[1], O = F[2]) : m[mJ(__sk_RE.F, 'tlkh')](4, F['length']) && (R = F[0], V = F[1], O = F[mJ(0x145, __sk_RE.b)](2, F['length'] - 1)[mJ(0x418, 'W!#l')]('#'), A = F[F['length'] - 1]) : (V = M, O = ''), A && ('0' === A || m['uRMrt'](m[mJ(0x5b9, 'W8id')], A[mJ(0x186, 'UzGo')]()) ? A = null : A['includes']('|') && (F = A['split']('|'), A = m['AwoEK'](2, F[mJ(0x3ca, 'NQY)')]) && ([M, F, b = '', l = ''] = F['map'](K => String(K || '')['trim']()), M) && F ? m['AgAIT'](m['cgspg'](m['riPKW'](mJ(0x344, 'LPN7') + (b || l ? m['dGoSw'](m[mJ(__sk_RE.l, 'C$jf')](m['fRjRU'](encodeURIComponent, b), ':'), m['fRjRU'](encodeURIComponent, l)) + '@' : ''), M), ':'), F) : null)), {
    'remark': R || '',
    'salt': O,
    'cookie': V,
    'proxyUrl': A
  };
}
function getAllAccountConfigs() {
  var __sk_RX = {
      M: 'U)4u',
      m: 0x2d7,
      F: '(%(I',
      b: 0x5f0,
      l: 'oizW',
      o: 'zu(j',
      R: 'E$*v',
      O: '@xcw',
      V: 0x2b6,
      A: 0xb6
    },
    __sk_Rr = {
      M: 'U@ca'
    },
    __sk_Rq = {
      M: 'NVQW',
      m: 0x5c3,
      F: 0x1c6
    },
    mY = __sk_MI,
    M = {
      'NhtGO': function (b, l) {
        return b(l);
      },
      'DimPb': mY(0x184, __sk_RX.M),
      'TLhDk': mY(0x51b, 'frRp'),
      'cBPRq': mY(__sk_RX.m, __sk_RX.F),
      'KdCNO': function (b, l) {
        return b + l;
      },
      'fQtUz': function (b, l) {
        return b === l;
      },
      'ceIHG': 'wKHka'
    },
    m = [];
  for (const b of parseAccountsFromEnv()) {
    if (M[mY(__sk_RX.b, '[sJs')](M['ceIHG'], 'wKHka')) {
      var F = parseAccountString(b);
      F ? m[mY(0x2d6, __sk_RX.l)](F) : console['log'](mY(0x23d, '[sJs') + b);
    } else {
      var R = {
        'OlGcp': function (O, V) {
          var mL = mY;
          return M[mL(0x62b, 'zu(j')](O, V);
        },
        'OhdUp': function (O, V) {
          return O || V;
        },
        'MePFh': M[mY(0x485, __sk_RX.o)],
        'NXdnQ': function (O, V) {
          return O + V;
        },
        'VmOtz': function (O, V) {
          return O + V;
        }
      };
      return this[mY(0x61f, __sk_RX.R)] && this['did'] ? M[mY(0x7b, 'frRp')](b, this[mY(0x305, 'M)%K')])[mY(0xdc, __sk_RX.O)](';')['map'](s => {
        var md = mY,
          h,
          J,
          Y,
          L,
          s = R['OlGcp'](R, s ?? '');
        return s[md(0x3d9, 'pOZA')]() && (h = s[md(0x2f8, __sk_Rq.M)]('='), Y = s['includes'](':'), h = h ? '=' : Y ? ':' : null) && ([Y, ...J] = s['split'](h), md(0x41f, 'tlkh') === (L = (Y = O(R['OhdUp'](Y, ''))['trim']())[md(__sk_Rq.m, 'W8id')]()) || R['MePFh'] === L) ? R[md(__sk_Rq.F, 'frRp')](R['VmOtz'](Y, h), this['did']) : s;
      })['join'](';\x20') : this[mY(__sk_RX.V, '8Hpe')];
    }
  }
  return m[mY(__sk_RX.A, 'kJU#')]((R, O) => {
    var mi = mY;
    'Scxev' === M['cBPRq'] ? R['index'] = M['KdCNO'](O, 1) : F[mi(0x36b, __sk_Rr.M)]('❌\x20' + this['getAccountDisplayName']() + M['TLhDk'] + b['message']);
  }), m;
}
async function executeConcurrently(m, F, b) {
  var __sk_Rv = {
      M: 0x565,
      m: 'UzGo',
      F: 0x573,
      b: 0x24a,
      l: '0Ddi',
      o: 0x2ac,
      R: 'ZDJd'
    },
    __sk_Rk = {
      M: 0x33c,
      m: 'NQY)',
      F: 0x530
    },
    mN = __sk_MI,
    l = {};
  l[mN(__sk_Rv.M, __sk_Rv.m)] = mN(__sk_Rv.F, 'NQY)'), l['mTHvr'] = function (A, K) {
    return A >= K;
  }, l[mN(__sk_Rv.b, __sk_Rv.l)] = function (A, K) {
    return A === K;
  }, l['frpTp'] = function (A, K) {
    return A + K;
  }, l[mN(0xb5, 'w%uJ')] = mN(__sk_Rv.o, 'y[XJ');
  var R = l;
  const O = new Array(m['length']);
  let V = 0;
  return F = Array['from']({
    'length': Math[mN(0x56a, __sk_Rv.R)](F, m[mN(0x5bb, 'M)%K')])
  }, async function () {
    var __sk_Rz = {
        M: 0x1e9,
        m: 'LPN7'
      },
      my = mN,
      A = {
        'BSlXf': function (C) {
          return C();
        }
      };
    for (;;) {
      if ('bxZEo' !== my(0x2a4, 'LPN7')) {
        var K = V++;
        if (R['mTHvr'](K, m['length'])) return;
        var H = m[K];
        try {
          O[K] = await b(H, K);
        } catch (C) {
          if (R[my(0x3a8, 'dz)r')]('ByvpX', my(0x160, 'v7OE'))) return F['log']('❌\x20' + this['getAccountDisplayName']() + R['qqawD'] + b[my(0x469, 'bR&C')]), false;else console['log'](R['frpTp'](R[my(0x30c, 'v7OE')], 1 + K) + '）：' + C[my(__sk_Rk.M, '0Ddi')]), O[K] = null;
        }
      } else {
        var Y = [];
        for (const d of A['BSlXf'](R)) {
          var L = A(d);
          L ? Y['push'](L) : K[my(0x578, __sk_Rk.m)](my(__sk_Rk.F, 'duP7') + d);
        }
        return Y['forEach']((N, y) => {
          var me = my;
          N[me(__sk_Rz.M, __sk_Rz.m)] = y + 1;
        }), Y;
      }
    }
  }), await Promise['all'](F), O;
}
function formatAccountDisplay(m, F, b) {
  var __sk_O2 = {
      M: 0x434,
      m: 'WQ!j'
    },
    mj = __sk_MI,
    l = {};
  l['LMcnC'] = function (O, V) {
    return O + V;
  }, l['RdXuD'] = function (O, V) {
    return O + V;
  }, l[mj(0x3d6, '&fI4')] = function (O, V) {
    return O + V;
  };
  var R = l;
  return F = R['LMcnC'](mj(__sk_O2.M, 'E0S!') + (F || m), ']') + (b ? R['RdXuD'](R['EXBcO']('（', b), '）') : ''), colors['bright'] + colors['cyan'] + F + colors[mj(0x2ef, __sk_O2.m)];
}
async function processAccount(M) {
  var __sk_OO = {
      M: 0x597,
      m: 'tlkh',
      F: 0x625,
      b: 0xd8,
      l: '$aoM',
      o: 0x480,
      R: 0x1f7,
      O: 0x260,
      V: 'E$*v',
      A: 0x2cf,
      K: '3Qg6',
      H: 'UzGo',
      C: 0x372,
      s: 0x492,
      h: '&fI4',
      J: 0x359,
      Y: 0x56d,
      L: '$aoM',
      d: 0x5a0,
      i: 'pOZA',
      N: 0x3b9,
      y: 0x200,
      e: 0x461,
      j: 0x52d,
      D: 0xa9,
      t: 'M)%K',
      Q: 0x130,
      u: 0x557,
      I: 0x382,
      W: 'UzGo',
      T: 0x4e8,
      Z: 'NQY)',
      G: 0x392,
      E: 0x34c,
      a: '2g$W',
      U: 0x8a,
      c: 'W8id',
      n: '[nEQ',
      B: 'oizW',
      g: '$aoM',
      P: 0x4db,
      q: '%N&g',
      r: 0x618,
      X: 'nZ^k',
      x: 'WQ!j',
      S: 'xqZj'
    },
    mD = __sk_MI,
    m = {
      'WYyMJ': function (j, D) {
        return j + D;
      },
      'wufhA': '0123456789abcdef',
      'SzyMw': mD(0x486, '$aoM'),
      'Rkqlq': function (j, D, Q, I) {
        return j(D, Q, I);
      },
      'yAnOs': function (j, D) {
        return j + D;
      },
      'QAyCF': mD(0x1e6, 'tlkh'),
      'cYqma': mD(0x52c, '*phW'),
      'FJWuX': function (j, D) {
        return j !== D;
      },
      'MAIOz': mD(__sk_OO.M, 'dz)r'),
      'jXWth': function (j, D) {
        return j + D;
      },
      'CPQgd': mD(0x4bc, __sk_OO.m),
      'mUjMY': mD(0x17b, 'U@ca'),
      'OupiT': mD(__sk_OO.F, 'H]2s'),
      'cZxQx': function (j, D) {
        return j + D;
      },
      'fVTKG': mD(0x1cc, '$aoM'),
      'PmbKy': mD(0x38c, '*phW'),
      'FLFXh': function (j, D) {
        return j + D;
      },
      'IgwMs': function (j, D, Q, I) {
        return j(D, Q, I);
      },
      'jmAGI': function (j, D) {
        return j + D;
      },
      'FtRPc': mD(__sk_OO.b, 'i1Bf'),
      'uEXBD': function (j, D) {
        return j != D;
      },
      'oCeUX': function (j, D) {
        return j != D;
      },
      'OGqXi': '，UD:\x20',
      'DMUtD': function (j, D) {
        return j + D;
      },
      'debFM': mD(0x371, __sk_OO.l),
      'kgwRM': mD(__sk_OO.o, 'U@ca'),
      'UsxWO': function (j, D) {
        return j + D;
      },
      'KeoWq': mD(__sk_OO.R, '*phW'),
      'Jxqzb': '⚠️\x20',
      'qkREg': function (j, D) {
        return j - D;
      }
    };
  if (M['proxyUrl']) {
    if (m['SzyMw'] === 'zyYtJ') {
      const j = m['Rkqlq'](formatAccountDisplay, M['index'], null, M['remark']);
      var F = await testProxyConnectivity(M[mD(__sk_OO.O, 'bR&C')], j);
      if (console['log'](m['yAnOs']('\x20\x20-\x20', F['ok'] ? m['QAyCF'] : m['cYqma']) + ':\x20' + F['msg']), F['ok'] && F['ip'] && m['FJWuX'](m['MAIOz'], F['ip'])) usedProxyIPs['has'](F['ip']) ? console['log'](m[mD(0xce, __sk_OO.V)](mD(__sk_OO.A, '[sJs') + F['ip'], m[mD(0x3e3, __sk_OO.K)])) : usedProxyIPs[mD(0x5a6, __sk_OO.H)](F['ip']), console[mD(__sk_OO.C, 'H]2s')](m['mUjMY'] + j + m['OupiT'] + M[mD(__sk_OO.s, 'tlkh')]);else {
        if (!F['ok']) return console['log'](m['cZxQx']('❌\x20', j) + m['fVTKG']), {
          'index': M['index'],
          'remark': M[mD(0xa6, __sk_OO.K)] || m[mD(0xf3, __sk_OO.h)],
          'nickname': m['cZxQx'](mD(0x289, '0Q4Q'), M['index']),
          'initialCoin': 0x0,
          'finalCoin': 0x0,
          'coinChange': 0x0,
          'initialCash': 0x0,
          'finalCash': 0x0,
          'cashChange': 0x0,
          'error': m['FLFXh'](mD(__sk_OO.J, '%N&g'), F['msg']),
          'skipped': true
        };
      }
    } else {
      H['log'](m[mD(0x41b, __sk_OO.K)](mD(__sk_OO.Y, '&fI4'), Q['message']));
      var Q = m['wufhA'];
      let I = '';
      for (let W = 0; W < 32; W++) I += Q['charAt'](V[mD(0x527, __sk_OO.L)](A[mD(0x5e9, '@xcw')]() * Q['length']));
      return I;
    }
  } else {
    const Q = m[mD(__sk_OO.d, '0Q4Q')](formatAccountDisplay, M[mD(0x514, '3Qg6')], null, M['remark']);
    console[mD(0x50f, __sk_OO.i)](m[mD(__sk_OO.N, '2vEu')](m['mUjMY'], Q) + m[mD(0x468, '&fI4')]);
  }
  const b = formatAccountDisplay(M[mD(0x5ee, 'bR&C')], null, M[mD(__sk_OO.y, __sk_OO.V)]);
  console[mD(__sk_OO.e, 'E$*v')](mD(__sk_OO.j, 'oizW') + b + mD(__sk_OO.D, __sk_OO.t));
  var R,
    O,
    F = await getAccountBasicInfo(M[mD(__sk_OO.Q, 'zu(j')], M[mD(0x21d, 'Q*e#')], M[mD(__sk_OO.u, '0Ddi')]),
    V = F?.['nickname'] || mD(0x3fd, 'E0S!') + M['index'],
    A = (F ? (A = m['uEXBD'](null, F['totalCoin']) ? F['totalCoin'] : mD(__sk_OO.I, __sk_OO.W), K = m['oCeUX'](null, F[mD(__sk_OO.T, 'bR&C')]) ? F['allCash'] : mD(0x35d, __sk_OO.Z), O = formatAccountDisplay(M[mD(0x37e, '2vEu')], V, M[mD(0x3ef, 'oizW')]), R = F['ud'] ? m['OGqXi'] + F['ud'] : '', enableColors ? console['log'](m['DMUtD'](m['DMUtD'](m['WYyMJ']('✅\x20' + O + m[mD(__sk_OO.G, '2g$W')], formatSuccess(A)), m['kgwRM']), colorText(K, m['UsxWO'](colors['bright'], colors['yellow']))) + R) : console[mD(__sk_OO.E, __sk_OO.h)](m['jXWth']('✅\x20' + O, m['debFM']) + A + mD(0x5c6, '0Ddi') + K + R)) : (O = formatAccountDisplay(M['index'], V, M['remark']), console['log'](m['UsxWO']('❌\x20' + O, m['KeoWq']))), F?.['ud'] || null),
    K = new KuaishouAccount({
      ...M,
      'nickname': V,
      'tasksToExecute': tasksToExecute,
      'udFromLogin': A
    });
  if (F && (await K[mD(0xe3, __sk_OO.a)](), K['coinExceeded'])) {
    console['log'](m[mD(0x316, '&ikq')](m[mD(0x194, 'jO@S')] + K[mD(__sk_OO.U, '2g$W')](), mD(0x21e, __sk_OO.c)));
    const I = await getAccountBasicInfo(K['getCookieWithCurrentDid'](), M['proxyUrl'], M['index']),
      W = F?.['totalCoin'] || 0,
      T = I?.[mD(0x412, __sk_OO.n)] || 0,
      Z = T - W,
      G = F?.['allCash'] || 0,
      E = I?.[mD(0x5a8, __sk_OO.B)] || 0,
      U = m[mD(0x60d, __sk_OO.g)](E, G);
    return {
      'index': M[mD(__sk_OO.P, __sk_OO.q)],
      'remark': M['remark'] || mD(0x123, 'pOZA'),
      'nickname': V,
      'initialCoin': W,
      'finalCoin': T,
      'coinChange': Z,
      'initialCash': G,
      'finalCash': E,
      'cashChange': U,
      'stats': K['getTaskStats'](),
      'coinLimitExceeded': true
    };
  }
  await K[mD(0x11f, 'C$jf')]();
  const H = await getAccountBasicInfo(K['getCookieWithCurrentDid'](), M[mD(__sk_OO.r, __sk_OO.X)], M['index']),
    C = F?.[mD(0x1a6, __sk_OO.x)] || 0,
    J = H?.['totalCoin'] || 0,
    Y = m['qkREg'](J, C),
    L = F?.['allCash'] || 0,
    N = H?.[mD(0x3c1, __sk_OO.S)] || 0,
    y = N - L;
  return K['printTaskStats'](), {
    'index': M['index'],
    'remark': M['remark'] || m['PmbKy'],
    'nickname': V,
    'initialCoin': C,
    'finalCoin': J,
    'coinChange': Y,
    'initialCash': L,
    'finalCash': N,
    'cashChange': y,
    'stats': K[mD(0x614, 'E0S!')](),
    'coinLimitExceeded': K['coinExceeded'],
    'infoFetchFailed': !F
  };
}
function displayResultsTable(M) {
  var __sk_Ou = {
      M: 0x49a,
      m: 0x445,
      F: '(%(I',
      b: '%N&g',
      l: 0xae,
      o: 0x193,
      R: 'nZ^k',
      O: 0x196,
      V: '8Hpe',
      A: 0x3da,
      K: 'zu(j',
      H: 0x635,
      C: 'U)4u',
      s: 'W8id',
      h: 0x93,
      J: 'xqZj',
      Y: 0x21a,
      L: 0x15f,
      d: '0Ddi',
      i: 0x4a6,
      N: 'pOZA',
      y: 'duP7',
      e: 0x129,
      j: 'W8id',
      D: 0x626
    },
    __sk_OQ = {
      M: 0x3ab,
      m: 'M)%K',
      F: '0Ddi',
      b: 0x431,
      l: 0x619,
      o: 0x649,
      R: 'WQ!j',
      O: 0x426,
      V: '%N&g',
      A: '&ikq',
      K: 0x7d,
      H: 0x16e,
      C: 'oizW',
      s: 'duP7',
      h: 0x1d7,
      J: 0x3b8,
      Y: '0Q4Q',
      L: 0x19c,
      d: 'M)%K'
    },
    __sk_Oj = {
      M: '0Ddi'
    },
    __sk_Oe = {
      M: 0x5da,
      m: 'zu(j'
    },
    mt = __sk_MI,
    m = {
      'kmeAR': mt(__sk_Ou.M, 'w%uJ'),
      'gHXSc': function (V, A) {
        return V + A;
      },
      'YlcfJ': function (V, A, K) {
        return V(A, K);
      },
      'DbTmL': function (V, A) {
        return V + A;
      },
      'jGqiL': function (V, A) {
        return V + A;
      },
      'hVTHU': function (V, A, K) {
        return V(A, K);
      },
      'nzOKR': function (V, A, K) {
        return V(A, K);
      },
      'GaoVn': function (V, A) {
        return V + A;
      },
      'ayYyu': function (V, A, K) {
        return V(A, K);
      },
      'oPNIS': mt(0x54b, 'w%uJ'),
      'BXNbC': mt(0x1ca, 'i1Bf'),
      'xpWzC': function (V, A) {
        return V + A;
      },
      'TvlQL': function (V, A, K) {
        return V(A, K);
      },
      'HzapW': mt(__sk_Ou.m, 'xqZj'),
      'Chxaz': function (V, A) {
        return V + A;
      },
      'UVjOe': mt(0x59e, __sk_Ou.F),
      'GcTxc': mt(0x318, __sk_Ou.b),
      'jaqaV': function (V, A) {
        return V + A;
      },
      'zdSwD': mt(__sk_Ou.l, 'Q*e#'),
      'hhktI': function (V, A) {
        return V + A;
      },
      'Atugo': function (V, A) {
        return V + A;
      },
      'fyunc': mt(0x1d6, 'Q*e#'),
      'MDhJp': mt(0x126, '$E2a'),
      'yPQwY': mt(__sk_Ou.o, __sk_Ou.R),
      'dIEgF': mt(0x5f5, 'U)4u'),
      'chnJK': mt(__sk_Ou.O, 'U)4u'),
      'zZnxa': mt(0x1aa, __sk_Ou.V),
      'RAhPG': mt(0x255, 'frRp')
    };
  if (M['length']) {
    var F = M[mt(0xb0, 'ZDJd')]((J, Y) => J + (parseInt(Y['initialCoin']) || 0), 0),
      F = M[mt(0x60f, 'E0S!')]((J, Y) => J + (parseInt(Y['finalCoin']) || 0), 0) - F,
      b = M[mt(0x443, '&ikq')]((J, Y) => J + (parseFloat(Y['initialCash']) || 0), 0),
      b = M['reduce']((J, Y) => J + (parseFloat(Y[mt(0x5ac, 'E0S!')]) || 0), 0) - b;
    let V = 0,
      A = 0,
      K = 0;
    M['forEach'](J => {
      var mQ = mt;
      J['stats'] && Object[mQ(0x562, '(%(I')](J['stats'])[mQ(0xbb, __sk_Oj.M)](Y => {
        var mu = mQ;
        V += Y['success'] + Y['failed'], A += Y[mu(__sk_Oe.M, __sk_Oe.m)], K += Y['totalReward'];
      });
    });
    var R = 0 < V ? (A / V * 100)[mt(__sk_Ou.A, __sk_Ou.K)](1) : m['BXNbC'],
      O = (M[mt(0x533, '$E2a')](J => J['coinLimitExceeded'])['length'], M['filter'](J => J['skipped'])['length']);
    M['filter'](J => J['infoFetchFailed'])['length'], console['log']('\x0a\x0a' + '='['repeat'](80)), console['log'](m['DbTmL'](m[mt(0x605, 'zu(j')]('|', m[mt(__sk_Ou.H, '[nEQ')](centerText, m['HzapW'], 78)), '|')), console['log']('='['repeat'](80)), console['log'](m['Chxaz'](m[mt(0x4b9, '0Ddi')]('|' + (mt(0x18c, '&ikq') + M[mt(0x237, 'kJU#')])['padEnd'](22) + (m['UVjOe'] + O)['padEnd'](22) + m['DbTmL'](m['GcTxc'], V)[mt(0x436, 'NVQW')](22), m['GaoVn'](m[mt(0x4ad, 'U)4u')](m['zdSwD'], R), '%')['padEnd'](10)), '|')), console[mt(0x495, __sk_Ou.C)](m[mt(0x34e, __sk_Ou.s)](m[mt(__sk_Ou.h, __sk_Ou.J)](m['GaoVn']('|' + m['Atugo'](m[mt(0x58d, 'zu(j')], F)[mt(__sk_Ou.Y, '0Q4Q')](26), (mt(__sk_Ou.L, '(%(I') + K)['padEnd'](26)), m['DbTmL'](mt(0x4bd, __sk_Ou.d), b[mt(__sk_Ou.i, __sk_Ou.N)](2))['padEnd'](24)), '|')), console['log']('-'['repeat'](80));
    const H = [6, 16, 16, 12, 12, 12, 12, 12, 12, 10];
    let C = '|',
      h = ([mt(0x1ef, 'dz)r'), mt(0x173, 'i1Bf'), m['MDhJp'], mt(0x58e, __sk_Ou.y), mt(__sk_Ou.e, __sk_Ou.j), m['yPQwY'], m['dIEgF'], m['chnJK'], m['zZnxa'], mt(__sk_Ou.D, 'bR&C')]['forEach']((J, Y) => {
        C += centerText(J, H[Y]) + '|';
      }), console['log'](C), '|');
    H['forEach'](J => {
      if ('RAHVT' === m['kmeAR']) return this['taskStats'][m]['failed']++, false;else h += '-'['repeat'](J) + '|';
    }), console['log'](h), M[mt(0x10b, 'W8id')](J => {
      var mI = mt,
        Y = '|',
        Y = m['gHXSc'](Y += m['gHXSc'](m['YlcfJ'](centerText, J['index'], H[0]), '|'), centerText(J[mI(0xe5, '&ikq')], H[1]) + '|');
      let L = J['nickname'] || '-';
      J[mI(0x48c, 'WQ!j')] ? L += '\x20❌' : J[mI(__sk_OQ.M, __sk_OQ.m)] ? L += '\x20⚠️' : J[mI(0x607, __sk_OQ.F)] && (L += '\x20🔶'), Y = (Y = m['gHXSc'](m['DbTmL'](Y = m['DbTmL'](Y = m['DbTmL'](m[mI(__sk_OQ.b, 'jO@S')](Y = (Y += m['hVTHU'](centerText, L['substring'](0, H[2] - 2), H[2]) + '|') + centerText(J['initialCoin'], H[3]) + '|', centerText(J[mI(0x4c7, 'bR&C')], H[4])), '|'), m[mI(__sk_OQ.l, 'pOZA')](centerText, 0 <= J['coinChange'] ? '+' + J[mI(0x1f5, 'kJU#')] : J[mI(__sk_OQ.o, __sk_OQ.R)], H[5])) + '|', centerText(J[mI(0x546, __sk_OQ.m)], H[6])), '|')) + centerText(J[mI(__sk_OQ.O, __sk_OQ.V)], H[7]) + '|';
      var d = 0 <= J[mI(0x63b, __sk_OQ.A)] ? m[mI(0x96, '8Hpe')]('+', J['cashChange']['toFixed'](2)) : J['cashChange'][mI(0x25e, 'w%uJ')](2);
      Y += m[mI(__sk_OQ.K, 'ZDJd')](centerText, d, H[8]) + '|';
      let N = mI(__sk_OQ.H, __sk_OQ.C);
      J['skipped'] ? N = mI(0x20c, __sk_OQ.s) : J['coinLimitExceeded'] ? N = mI(__sk_OQ.h, 'E$*v') : J[mI(__sk_OQ.J, __sk_OQ.Y)] && (N = m[mI(__sk_OQ.L, 'v7OE')]), Y += centerText(N, H[9]) + '|', console[mI(0x1d4, __sk_OQ.d)](Y);
    }), console['log']('='['repeat'](80)), console[mt(0x63a, 'LPN7')]('|' + centerText(mt(0x430, __sk_Ou.R), 78) + '|'), console['log']('='['repeat'](80));
  } else console['log'](m['RAhPG']);
}
async function testSignService() {
  var __sk_OZ = {
      M: 0x5b8,
      m: 0x481,
      F: 0x4d1,
      b: '0Ddi',
      l: 0x188,
      o: 'U@ca',
      R: 'pOZA',
      O: 0x461,
      V: 'E$*v'
    },
    mW = __sk_MI,
    F = {
      'ovxYp': function (A, K) {
        return A !== K;
      },
      'YeFjR': mW(0x43b, 'E$*v'),
      'LXgvB': mW(__sk_OZ.M, 'W8id'),
      'kIowT': function (A, K, H, C) {
        return A(K, H, C);
      },
      'rZAPD': '/encsign',
      'dBCTZ': mW(0xf7, 'tlkh'),
      'iolAC': function (A, K) {
        return A + K;
      },
      'adoWE': mW(0x377, '[sJs'),
      'SQpnt': mW(0x4d5, 'U@ca')
    };
  try {
    if (F[mW(0x1f4, '&fI4')]('KjUbr', F['YeFjR'])) {
      var b = {};
      b['type'] = F['LXgvB'], b[mW(0x285, 'dz)r')] = 'dGVzdF9kYXRh', b['ud'] = 'test_user', b[mW(__sk_OZ.m, 'i1Bf')] = SCRIPT_VERSION;
      var l = {};
      l['Content-Type'] = 'application/json';
      var R = b,
        {
          response: O,
          body: V
        } = await F['kIowT'](sendRequest, {
          'method': 'POST',
          'url': API_CONFIG[mW(__sk_OZ.F, __sk_OZ.b)] + F[mW(__sk_OZ.l, __sk_OZ.o)],
          'body': JSON['stringify'](R),
          'headers': l,
          'timeout': 0x2710
        }, null, mW(0x1d3, 'W!#l'));
      return O ? V && V['status'] ? (console['log'](F['dBCTZ']), true) : (console['log'](F[mW(0xe8, __sk_OZ.R)](mW(0x2d0, 'ZDJd'), V?.['message'] || F[mW(0x4f0, 'W!#l')])), false) : (console[mW(__sk_OZ.O, __sk_OZ.V)](F['SQpnt']), false);
    } else return R = O[mW(0x281, '(%(I')][V], A ? K[mW(0x5cd, __sk_OZ.V)]() : H;
  } catch (K) {
    return console['log'](mW(0x28f, 'nZ^k') + K['message']), false;
  }
}
async function testNssigService() {
  var __sk_OU = {
      M: 0x36d,
      m: 'bR&C',
      F: 'w%uJ',
      b: '%N&g',
      l: 0x529,
      o: 0x53f,
      R: 'yoVP',
      O: 0x2ad,
      V: 0x177,
      A: 0x182,
      K: 0x22c,
      H: 'zu(j',
      C: 0x2fe,
      s: 'kJU#'
    },
    mT = __sk_MI,
    m = {
      'MeQXt': function (V, A) {
        return V + A;
      },
      'ysnGn': 'test=data&salt=test_salt',
      'jagvg': mT(0x4f6, '8Hpe'),
      'cymjX': 'test_user',
      'bDNdQ': function (V, A, K, H) {
        return V(A, K, H);
      },
      'ggHKz': mT(__sk_OU.M, __sk_OU.m),
      'gIoCw': mT(0x235, __sk_OU.F),
      'KdMlJ': function (V, A) {
        return V + A;
      }
    };
  try {
    var F = {};
    F['Content-Type'] = 'application/json';
    var b = getOrCreateDeviceId(),
      l = {
        'type': 'nssig',
        'path': mT(0x2f9, __sk_OU.b),
        'data': m['ysnGn'],
        'salt': m[mT(0x5f3, '0Q4Q')],
        'ud': m['cymjX'],
        'script_version': SCRIPT_VERSION,
        'device_id': b
      },
      {
        response: R,
        body: O
      } = await m['bDNdQ'](sendRequest, {
        'method': m[mT(__sk_OU.l, '[nEQ')],
        'url': m[mT(0x25b, 'zu(j')](API_CONFIG['toudeshishabi'], '/nssig'),
        'body': JSON['stringify'](l),
        'headers': F,
        'timeout': 0x2710
      }, null, mT(__sk_OU.o, __sk_OU.R));
    return R ? O && O['status'] ? (console['log'](mT(0x2f3, '0Bp5')), true) : (console['log'](mT(0x550, 'bR&C') + (O?.[mT(__sk_OU.O, '&fI4')] || mT(__sk_OU.V, 'ZDJd'))), false) : (console['log'](m['gIoCw']), false);
  } catch (V) {
    return mT(__sk_OU.A, 'xqZj') !== mT(__sk_OU.K, 'Q*e#') ? (F['log'](m['MeQXt']('❌\x20' + this['getAccountDisplayName']() + mT(0x558, 'E$*v'), b['message'])), null) : (console[mT(0x212, 'bR&C')](m[mT(0x302, __sk_OU.H)](mT(__sk_OU.C, __sk_OU.s), V[mT(0x33c, '0Ddi')])), false);
  }
}
(async () => {
  var __sk_Of = {
      M: 0x261,
      m: 'xqZj',
      F: 0x1ff,
      b: '*phW',
      l: 'UzGo',
      o: 0x4f3,
      R: 0x473,
      O: 0x34b,
      V: 'W!#l',
      A: 0x423,
      K: 0x133,
      H: '0Ddi',
      C: 'xqZj',
      s: 0x54c,
      h: 'v7OE',
      J: 0x645,
      Y: 'frRp',
      L: 0x360,
      d: 0x61b,
      i: 0x3ae,
      N: 'Q*e#',
      y: 0x5a2,
      e: 'H]2s',
      j: 'pOZA',
      D: 0x50a,
      t: 0x2af,
      Q: 'NVQW',
      u: 0x4a9,
      I: 'ZDJd',
      W: 0x352,
      T: 'dz)r',
      Z: 0x5bf,
      G: 0x2c9,
      E: 'C$jf',
      a: 0x528,
      U: '0Q4Q',
      c: '@xcw',
      n: 0x636,
      B: 'pOZA',
      g: 0x351,
      P: 0x521,
      q: 0x300,
      r: 0x5d6,
      X: 0x4a5,
      x: 0x3c6,
      S: '$aoM',
      w: 0x39f,
      p: '3Qg6',
      z: '&ikq',
      k: 0x11c,
      v: 'U@ca',
      f: 'tlkh',
      M0: 0x242,
      M1: 0x53d,
      M2: '(%(I',
      M3: 'E$*v',
      M4: 0x4a8,
      M5: 'WQ!j'
    },
    __sk_Ov = {
      M: 0x629,
      m: 0x42b,
      F: 'dz)r',
      b: 0x5f9,
      l: 0x3f6,
      o: 'yoVP',
      R: 0xd6,
      O: 0x15d,
      V: '0Ddi',
      A: 0x54a,
      K: 'NQY)',
      H: 0x62d,
      C: 0xe4,
      s: 0x37d,
      h: 0x27e,
      J: 0x1b9,
      Y: 'zu(j',
      L: 0x5de,
      d: '&fI4',
      i: 0x5d9,
      N: 'U@ca',
      y: 0x12b,
      e: '0Ddi',
      j: 0x155,
      D: 'pOZA',
      t: 0x38a,
      Q: 'y[XJ',
      u: 0x12c,
      I: 'xqZj',
      W: 0x3cf,
      T: 0x2b3,
      Z: 0x209,
      G: 0x3c8,
      E: 0x3f3,
      a: '@xcw',
      U: '[sJs',
      c: '*phW',
      n: 0x2a2,
      B: 0x1fe,
      g: 0x564,
      P: '&ikq',
      q: 0x42a,
      r: 'U)4u',
      X: '3Qg6',
      x: 0x106
    },
    mZ = __sk_MI,
    M = {
      'txZdV': function (l, R) {
        return l + R;
      },
      'yEgJG': mZ(0x256, 'yoVP'),
      'rdnsV': function (l, R) {
        return l + R;
      },
      'RUIxy': function (l, R) {
        return l + R;
      },
      'HzgPN': mZ(__sk_Of.M, 'w%uJ'),
      'cuNBF': function (l, R) {
        return l !== R;
      },
      'smOCw': mZ(0x3bc, __sk_Of.m),
      'Navrg': 'wCbZV',
      'nzgIO': 'GtLFE',
      'TVyPT': 'Hqwgi',
      'fAcmw': function (l, R) {
        return l + R;
      },
      'zflYd': mZ(0x31b, '*phW'),
      'VNvAc': mZ(__sk_Of.F, __sk_Of.b),
      'IABOk': mZ(0x599, '%N&g'),
      'bYEUx': function (l) {
        return l();
      },
      'LXEmD': mZ(0x457, 'bR&C'),
      'QakJj': mZ(0xc5, __sk_Of.l),
      'jtQMJ': '═══════════════════════════════════════════════════════════════',
      'nRsbY': function (l, R) {
        return l + R;
      },
      'Yxhxs': mZ(__sk_Of.o, 'v7OE'),
      'xqYbE': mZ(0xdb, 'dz)r'),
      'eIyFU': function (l, R) {
        return l + R;
      },
      'JHNfw': mZ(0x58f, 'WQ!j'),
      'JDziu': mZ(0x4e6, '[nEQ'),
      'OwWPV': function (l, R) {
        return l + R;
      },
      'MVkiZ': mZ(0x342, 'pOZA'),
      'FSmcA': mZ(__sk_Of.R, '3Qg6'),
      'TATje': mZ(__sk_Of.O, __sk_Of.V),
      'zSFxM': function (l, R) {
        return l + R;
      },
      'RyOqH': mZ(0x109, 'Q*e#'),
      'AGBnZ': function (l, R, O, V) {
        return l(R, O, V);
      },
      'LRpOj': mZ(__sk_Of.A, 'W8id')
    };
  console['log'](M['VNvAc']);
  var m = getOrCreateDeviceId(),
    m = (console['log'](M['IABOk'] + m), await testSignService()),
    F = await M[mZ(__sk_Of.K, __sk_Of.H)](testNssigService),
    m = (m && F || (console[mZ(0x1f2, __sk_Of.C)](mZ(0x266, 'H]2s')), console['log'](mZ(0x174, '[nEQ')), console[mZ(__sk_Of.s, __sk_Of.h)](mZ(__sk_Of.J, __sk_Of.Y)), console['log'](mZ(__sk_Of.L, '$E2a')), console['log'](mZ(__sk_Of.d, '0Ddi')), console['log'](M['LXEmD']), process[mZ(__sk_Of.i, __sk_Of.N)](1)), getAllAccountConfigs());
  m[mZ(__sk_Of.y, 'NVQW')] || (console['error'](M[mZ(0x571, __sk_Of.e)]), console[mZ(0x50f, __sk_Of.j)](mZ(0x5e8, 'UzGo')), process['exit'](1)), console['log'](M[mZ(__sk_Of.D, 'jO@S')]), console['log'](mZ(__sk_Of.t, __sk_Of.Q)), console['log'](M[mZ(__sk_Of.u, __sk_Of.I)](M['Yxhxs'], COIN_LIMIT)), console['log'](M['xqYbE'] + LOW_REWARD_THRESHOLD), console['log'](M[mZ(__sk_Of.W, __sk_Of.T)](mZ(0x2e8, 'LPN7'), LOW_REWARD_LIMIT)), console['log'](M['fAcmw'](M[mZ(__sk_Of.Z, '2vEu')] + accountCount, M['JDziu'])), console[mZ(__sk_Of.G, __sk_Of.E)](mZ(0x5b5, 'bR&C') + (SKIP_LIVE_ADS ? M['OwWPV'](mZ(__sk_Of.a, __sk_Of.U), SKIP_LIVE_MAX_RETRIES) + mZ(0x31d, __sk_Of.c) : mZ(0x20d, 'UzGo'))), console[mZ(0x20a, '0Bp5')](mZ(__sk_Of.n, '2g$W') + tasksToExecute['join'](',\x20')), tasksToExecute[mZ(0x113, 'kJU#')](M['MVkiZ']) && console['log'](mZ(0x489, __sk_Of.B) + LOOK_COUNT), tasksToExecute[mZ(0x29a, 'i1Bf')](M['FSmcA']) && console['log'](M[mZ(__sk_Of.g, '$aoM')](mZ(__sk_Of.P, 'bR&C'), BOX_COUNT)), tasksToExecute['includes']('food') && console['log'](mZ(__sk_Of.q, 'NQY)') + FOOD_COUNT), tasksToExecute['includes']('kbox') && console[mZ(__sk_Of.r, '8Hpe')](M['rdnsV'](mZ(__sk_Of.X, '@xcw'), KBOX_COUNT)), tasksToExecute['includes']('search') && console[mZ(__sk_Of.x, __sk_Of.S)](M[mZ(0x48b, '$E2a')] + SEARCH_COUNT), tasksToExecute[mZ(__sk_Of.w, __sk_Of.p)](mZ(0x400, __sk_Of.z)) && console['log'](M['zSFxM'](mZ(__sk_Of.k, __sk_Of.v), FOLLOW_COUNT)), tasksToExecute['includes'](mZ(0x404, __sk_Of.f)) && console['log'](M[mZ(0x427, 'duP7')] + SEARCH_FOLLOW_COUNT), console['log'](mZ(0x366, '*phW')), console[mZ(0x63a, 'LPN7')]('═══════════════════════════════════════════════════════════════\x0a'), console['log'](M['zSFxM'](mZ(0x171, 'LPN7'), m['length']) + mZ(__sk_Of.M0, __sk_Of.p));
  const b = [];
  await M[mZ(__sk_Of.M1, '@xcw')](executeConcurrently, m, Math['max'](1, Math['min'](MAX_CONCURRENCY, m[mZ(0x5ca, __sk_Of.M2)])), async l => {
    var mG = mZ,
      R = {
        'przxJ': function (A, K) {
          return A === K;
        },
        'WXNxU': function (A, K) {
          return M['rdnsV'](A, K);
        },
        'ZjWRG': mG(__sk_Ov.M, 'M)%K')
      };
    if (M['cuNBF']('qOdsn', M['smOCw'])) try {
      if (M['Navrg'] === M[mG(__sk_Ov.m, 'y[XJ')]) {
        var __sk_Ok = {
          M: 0x1b8
        };
        R['stats'] && O[mG(0x1c4, __sk_Ov.F)](V[mG(__sk_Ov.b, 'E$*v')])['forEach'](Y => {
          var mE = mG;
          C += Y['success'] + Y['failed'], s += Y['success'], h += Y[mE(__sk_Ok.M, 'W8id')];
        });
      } else {
        var O = await processAccount(l),
          V = {};
        V[mG(__sk_Ov.l, 'NVQW')] = l[mG(0x208, __sk_Ov.o)], V['remark'] = l[mG(__sk_Ov.R, '0Q4Q')] || mG(__sk_Ov.O, __sk_Ov.V), V[mG(__sk_Ov.A, __sk_Ov.K)] = O?.[mG(__sk_Ov.H, '8Hpe')] || mG(__sk_Ov.C, '[nEQ') + l[mG(0x51e, 'M)%K')], V[mG(__sk_Ov.s, 'kJU#')] = O?.['initialCoin'] || 0, V[mG(__sk_Ov.h, 'zu(j')] = O?.['finalCoin'] || 0, V['coinChange'] = O?.['coinChange'] || 0, V['initialCash'] = O?.[mG(0xc9, 'H]2s')] || 0, V['finalCash'] = O?.[mG(0x7e, 'zu(j')] || 0, V['cashChange'] = O?.[mG(0x85, '[sJs')] || 0, V[mG(__sk_Ov.J, __sk_Ov.Y)] = O?.[mG(0x31f, 'M)%K')] || {}, V[mG(__sk_Ov.L, 'bR&C')] = O?.['coinLimitExceeded'] || false, V['skipped'] = O?.['skipped'] || false, V[mG(0xb8, __sk_Ov.d)] = O?.['infoFetchFailed'] || false, V[mG(__sk_Ov.i, '3Qg6')] = O?.['error'] || null, b[mG(0x48d, 'U@ca')](V);
      }
    } catch (K) {
      M['TVyPT'] !== mG(0x4c1, __sk_Ov.N) ? (j['ad']?.['adDataV2']?.['inspirePersonalize']?.['awardValue'] ? D = l(Q['ad'][mG(__sk_Ov.y, __sk_Ov.e)]['inspirePersonalize']['awardValue']) || 0 : u['ad']?.['awardCoin'] ? I = W(T['ad']['awardCoin']) || 0 : Z['ad']?.['adDataV2']?.['inspireAdInfo']?.[mG(0x5d5, 'M)%K')] && (G = E(a['ad'][mG(0x42d, '&fI4')]['inspireAdInfo'][mG(__sk_Ov.j, __sk_Ov.D)]) || 0), R['przxJ'](0, U) && (c['log'](R[mG(__sk_Ov.t, __sk_Ov.Q)](mG(__sk_Ov.u, __sk_Ov.I), this[mG(__sk_Ov.W, 'E$*v')]()) + mG(__sk_Ov.T, '[nEQ')), n['log'](mG(0x121, __sk_Ov.N), B['ad']?.['adDataV2'] ? mG(0x28e, __sk_Ov.N) : mG(0x1c5, 'ZDJd')), g['log']('\x20\x20-\x20adData.ad.awardCoin:', P['ad']?.['awardCoin'] || mG(__sk_Ov.Z, __sk_Ov.N)), q['ad']?.['adDataV2']) && (r[mG(0x2fb, 'tlkh')]('\x20\x20-\x20inspirePersonalize:', X['ad'][mG(__sk_Ov.G, 'U@ca')]['inspirePersonalize'] ? mG(__sk_Ov.E, __sk_Ov.a) : mG(0x1e0, '&fI4')), x['log'](mG(0x477, __sk_Ov.U), S['ad'][mG(0x4aa, 'zu(j')]['inspireAdInfo'] ? mG(0x45b, 'C$jf') : R[mG(0x4e9, '@xcw')]))) : (console['log'](M['fAcmw'](M['fAcmw'](mG(0x5ad, __sk_Ov.c) + l['index'], ']') + (l['remark'] ? '（' + l['remark'] + '）' : ''), M['zflYd']) + K['message']), b[mG(0x38b, 'E$*v')]({
        'index': l['index'],
        'remark': l[mG(__sk_Ov.n, 'w%uJ')] || M[mG(__sk_Ov.B, 'dz)r')],
        'nickname': mG(0x28a, 'NVQW') + l['index'],
        'initialCoin': 0x0,
        'finalCoin': 0x0,
        'coinChange': 0x0,
        'initialCash': 0x0,
        'finalCash': 0x0,
        'cashChange': 0x0,
        'error': K[mG(0x494, 'H]2s')],
        'skipped': true
      }));
    } else K['log'](M[mG(__sk_Ov.g, 'tlkh')](M['txZdV'](M['yEgJG'] + H['index'] + ']', C['remark'] ? M[mG(0x628, __sk_Ov.P)](M[mG(__sk_Ov.q, __sk_Ov.r)]('（', s['remark']), '）') : '') + mG(0x470, __sk_Ov.X), h['message'])), J['push']({
      'index': Y[mG(__sk_Ov.x, 'w%uJ')],
      'remark': L['remark'] || M[mG(0x1e3, 'yoVP')],
      'nickname': mG(0x217, '2g$W') + d[mG(0x1e9, 'LPN7')],
      'initialCoin': 0x0,
      'finalCoin': 0x0,
      'coinChange': 0x0,
      'initialCash': 0x0,
      'finalCash': 0x0,
      'cashChange': 0x0,
      'error': i['message'],
      'skipped': true
    });
  }), b['sort']((l, R) => l['index'] - R[mZ(0x162, '(%(I')]), console['log'](mZ(0x590, __sk_Of.M3), '✅'), console['log'](M[mZ(__sk_Of.M4, __sk_Of.M5)]), displayResultsTable(b);
})();